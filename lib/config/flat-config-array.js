...
/**
 * The shape of the `ConfigError` instances thrown by `@eslint/config-array`.
 * @typedef {Object} ConfigError
 * @property {string} name Always `"ConfigError"`.
 * @property {string} message The error message, which ends with a period.
 * @property {number} index The index of the offending config object.
 */
 ...
 * @param {ConfigError} error The original error.
...
/*
 * These symbols key per-instance bookkeeping that is deliberately hidden from
 * the array contents. They are typed as `any` because `FlatConfigArray`
 * inherits `Array`'s numeric index signature, and TypeScript therefore rejects
 * every symbol-keyed read and write on the instance.
 */
...
class FlatConfigArray extends ConfigArray {
	 ...
	 * @param {Iterable<Object>|Function|Object} configs An array of configuration information.
	 * @param {Object} [options] The options to use for the config array instance.
	 * @param {string} [options.basePath] The absolute path of the directory that
	 *      relative paths in the config are resolved against.
	 * @param {boolean} [options.shouldIgnore] Determines if `ignores` fields should be honored.
	 * @param {Object|Array<Object>} [options.baseConfig] The config to insert before `configs`.
	 */
	constructor(
		configs,
		...
		if (/** @type {any} */ (baseConfig)[Symbol.iterator]) {
			this.unshift(.../** @type {Array<Object>} */ (baseConfig));
		} else {
	 ...
	 * @param {Object} [context] The context to use to normalize the array.
	 * @returns {Promise<ConfigArray>} A promise that resolves when the array is normalized.
	 */
	normalize(context) {
	 ...
	 * any ConfigError exceptions with additional details.
	 * @param {Object} [context] The context to use to normalize the array.
	 * @returns {ConfigArray} The current instance.
	...
	normalizeSync(context) {
		try {
			...
			if (/** @type {ConfigError} */ (error).name === "ConfigError") {
				throw wrapConfigErrorWithDetails(
	 ...
	 * normalization.
	 * @param {Record<string, unknown>} config The config to preprocess.
	 * @returns {Record<string, unknown>} The preprocessed config.
	 */
	 ...
	 * @param {Record<string, unknown>} config The config to finalize.
	 * @returns {Config} The finalized config.
	 * @throws {TypeError} If the config is invalid.
	 */
