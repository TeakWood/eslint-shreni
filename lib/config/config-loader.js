 ...
 * @throws If `filePath` is not a non-empty string.
 */
 ...
 * @param loaderOptions The options to use when loading configuration files.
 * @throws If no configuration exists.
 */
 ...
 * @param filePath The file path to check.
 * @returns `true` if the file is a TypeScript file, `false` if it's not.
 */
 ...
 * @returns `true` if the ESLint is running Bun, `false` if it's not.
 */
 ...
 * @returns `true` if the ESLint is running in Deno, `false` if it's not.
 */
 ...
 * @returns `true` if native TypeScript support is enabled, otherwise `false`.
 * @since 9.24.0
 */
 ...
 * @param filePath The absolute file path to load.
 * @param fileURL The file URL to load.
 * @param mtime The last modified timestamp of the file.
 ...
 * @param filePath The absolute file path of the module to import.
 * @param fileURL The file URL to load.
 * @param mtime The last modified timestamp of the file.
 * @returns - A {@linkcode Promise | promise} that resolves to the imported ESLint config.
 * @since 9.24.0
 */
 ...
 * @param filePath The filename to load from.
 * @param hasUnstableNativeNodeJsTSConfigFlag The flag to indicate whether the `unstable_native_nodejs_ts_config` flag is enabled.
 * @returns The config loaded from the config file.
 */
...
class ConfigLoader {
	 ...
	 * @param options The options to use when loading configuration files.
	 */
	constructor(options) {
	 ...
	 * @param fromDirectory The directory from which to start searching.
	 * @returns Location information for
	 *      the config file.
	 */
	 ...
	 * @param configFilePath The absolute path to the config file to use if not overridden.
	 * @param basePath The base path to use for relative paths in the config file.
	 * @returns The config array for `eslint`.
	 */
	 ...
	 * @param fileOrDirPath The file or directory path to get the config file path for.
	 * @returns The config file path or `undefined` if not found.
	 * @throws If `fileOrDirPath` is not a non-empty string.
	 * @throws If `fileOrDirPath` is not an absolute path.
	 */
	 ...
	 * @param filePath The path of the file or directory to retrieve config for.
	 * @returns A configuration object for the file.
	 * @throws If no configuration for `filePath` exists.
	 */
	 ...
	 * This is the same logic used by the ESLint CLI executable to determine
	 * configuration for each file it processes.
	 * @param dirPath The path of the directory to retrieve config for.
	 * @returns A configuration object for the directory.
	 */
	 ...
	 * @returns A configuration object for the file.
	 * @throws If `filePath` is not a non-empty string.
	 * @throws If `filePath` is not an absolute path.
	 * @throws If the config file was not already loaded.
	 */
	 ...
	 * intended to be used in locations where we know the config file has already
	 * been loaded and we just need to get the configuration for a file.
	 * @param fileOrDirPath The path of the directory to retrieve a config object for.
	 * @returns A configuration object for the directory.
	 * @throws If `dirPath` is not a non-empty string.
	 * @throws If `dirPath` is not an absolute path.
	 * @throws If the config file was not already loaded.
	 */
	 ...
	 * @returns A promise that fulfills with an object containing the jiti module's createJiti function and version.
	 */
	 ...
	 * @param options.useConfigFile The path to the config file to use.
	 * @param options.cwd Path to a directory that should be considered as the current working directory.
	 * @param [options.fromDirectory] The directory from which to start searching. Defaults to `cwd`.
	 * @returns Location information for
	 *      the config file.
	 */
