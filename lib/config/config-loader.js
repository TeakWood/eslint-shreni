...
/**
 * The options accepted by `ConfigLoader`.
 * @typedef {Object} ConfigLoaderOptions
 * @property {string} cwd The directory that relative paths are resolved against.
 * @property {string|false} [configFile] The path to the config file to use, or
 *      `false` to skip looking for one entirely.
 * @property {Array<Object>|Object|null} [baseConfig] The config objects to place
 *      before the ones read from the config file. `null` means none, which is
 *      what the `ESLint` class passes when the option is unset.
 * @property {boolean} [ignoreEnabled] Determines if `ignores` fields outside of
 *      `baseConfig` should be honored.
 * @property {Array<string>|null} [ignorePatterns] Additional ignore patterns, which
 *      are appended after everything else so they win.
 * @property {Object|Array<Object>|null} [overrideConfig] The config objects to append
 *      after the ones read from the config file.
 * @property {boolean} [hasUnstableNativeNodeJsTSConfigFlag] Whether the
 *      `unstable_native_nodejs_ts_config` flag is enabled.
 * @property {Array<Object>} [defaultConfigs] The configs contributed by the
 *      `plugins` option.
 * @property {WarningService} [warningService] The service used to emit warnings.
 */

/**
 * `ConfigLoaderOptions` after the constructor has filled in `warningService`.
 * @typedef {ConfigLoaderOptions & { warningService: WarningService }} ResolvedConfigLoaderOptions
 */

/**
 * Where a config file was found and what its configs are relative to.
 * @typedef {Object} ConfigFileInfo
 * @property {string} [configFilePath] The absolute path to the config file, or
 *      `undefined` when no config file is in use.
 * @property {string} basePath The base path to use for relative paths in the
 *      config file.
 */
 ...
 * @param {unknown} filePath The file path to check.
...
function assertConfigurationExists(configFilePath, loaderOptions) {
	...
	if (!configFilePath && useConfigFile !== false) {
		const error = /** @type {Error & { messageTemplate?: string }} */ (
			new Error("Could not find config file.")
		);
...
function isRunningInBun() {
	return !!(/** @type {Record<string, unknown>} */ (globalThis).Bun);
}
...
function isRunningInDeno() {
	return !!(/** @type {Record<string, unknown>} */ (globalThis).Deno);
}
...
function isNativeTypeScriptSupportEnabled() {
	/*
	 * Typed as `Array<unknown>` because `process.features.typescript` is
	 * `string | boolean`, which `Array<string>#includes()` would reject.
	 */
	/** @type {Array<unknown>} */
	const nativeTypeScriptModes = ["strip", "transform"];

	// eslint-disable-next-line n/no-unsupported-features/node-builtins -- it's still an experimental feature.
	return nativeTypeScriptModes.includes(process.features.typescript);
}
 ...
 * @returns {Promise<unknown>} The configuration loaded from the file.
 * @throws {Error} If `jiti` is missing or too old to load the file.
 * @since 9.24.0
 */
async function loadTypeScriptConfigFileWithJiti(filePath, fileURL, mtime) {
	...
	const config = /** @type {{ default?: unknown } | undefined} */ (
		await jiti.import(fileURL.href)
	);
 ...
 * @param {URL} fileURL The file URL to load.
 * @param {number} mtime The last modified timestamp of the file.
 * @returns {Promise<unknown>} - A {@linkcode Promise | promise} that resolves to the imported ESLint config.
 * @since 9.24.0
 */
 ...
 * @returns {Promise<unknown>} The config loaded from the config file.
 * @throws {Error} If the `unstable_native_nodejs_ts_config` flag is enabled but
 *      native TypeScript support is unavailable.
 */
...
class ConfigLoader {
	 ...
	 * An entry is the in-flight promise until the calculation settles, which is
	 * what `getCachedConfigArrayForPath()` duck-types on.
	 * @type {Map<string|undefined, FlatConfigArray|Promise<FlatConfigArray>>}
	 */
	#configArrays = new Map();

	/**
	 * Map of absolute directory names to the config file paths for those directories.
	 * An entry is the in-flight promise until the lookup settles, which is what
	 * `getCachedConfigArrayForPath()` duck-types on.
	 * @type {Map<string, ConfigFileInfo|Promise<ConfigFileInfo>>}
	 */
	...
	constructor(options) {
		this.#options = options.warningService
			? /** @type {ResolvedConfigLoaderOptions} */ (options)
	 ...
	 * @returns {Promise<ConfigFileInfo>} Location information for
	 *      the config file.
	 */
	async #locateConfigFileToUse(fromDirectory) {
		...
		if (this.#configFilePaths.has(fromDirectory)) {
			/*
			 * The `has()` check above guarantees an entry, which `get()`'s
			 * return type cannot express.
			 */
			return /** @type {ConfigFileInfo|Promise<ConfigFileInfo>} */ (
				this.#configFilePaths.get(fromDirectory)
			);
		}
	 ...
	 * @param {string|undefined} configFilePath The absolute path to the config file to use if not overridden.
	...
	async #calculateConfigArray(configFilePath, basePath) {
		...
		if (this.#configArrays.has(configFilePath)) {
			 ...
			 * return type cannot express.
			 */
			return /** @type {FlatConfigArray|Promise<FlatConfigArray>} */ (
				this.#configArrays.get(configFilePath)
			);
		}
	...
	getCachedConfigArrayForPath(fileOrDirPath) {
		...
		if (
			typeof (
				/** @type {Promise<ConfigFileInfo>} */ (configFilePathInfo).then
			) === "function"
		) {
			throw new Error(
		...
		/*
		 * The `has()` check and the `then` check above together guarantee a
		 * settled `ConfigFileInfo` rather than `undefined` or a promise.
		 */
		const { configFilePath } = /** @type {ConfigFileInfo} */ (
			configFilePathInfo
		);
		...
		if (
			!configArray ||
			typeof (
				/** @type {Promise<FlatConfigArray>} */ (configArray).then
			) === "function"
		) {
			throw new Error(
		...
		// the `then` check above rules out the in-flight promise
		return /** @type {FlatConfigArray} */ (configArray);
	}

	/**
	 * Used to import the jiti dependency. This method is exposed internally for testing purposes.
	 * @returns {Promise<{ createJiti: import("jiti").createJiti, version: string }>} A promise that fulfills with an object containing the jiti module's createJiti function and version.
	 */
	 ...
	 * @param {string|false} [options.useConfigFile] The path to the config file to use.
	 ...
	 * @param {string|undefined} configFilePath The absolute path to the config file to use if not overridden.
	 * @param {string} basePath The base path to use for relative paths in the config file.
	 * @param {ResolvedConfigLoaderOptions} options The options to use when loading configuration files.
	 * @returns {Promise<FlatConfigArray>} The config array for `eslint`.
	 */
