...
/** @typedef {import("../config/flat-config-array.js").FlatConfigArray} FlatConfigArray */
/** @typedef {import("../services/warning-service.js").WarningService} WarningService */
/**
 * The retry helper from `@humanwhocodes/retry`, typed by the one method used
 * here rather than by importing the class. The package ships separate CJS and
 * ESM declarations whose `Retrier` classes carry distinct private fields, so a
 * nominal reference matches only whichever half the importer resolved.
 * @typedef {{ retry: <T>(fn: () => Promise<T>, options?: { signal?: AbortSignal }) => Promise<T> }} Retrier
 */
/** @typedef {import("@humanfs/types").HfsWalkEntry} HfsWalkEntry */

/**
 * A resolved config for one file, as produced by `FlatConfigArray#getConfig()`.
 * `Config` copies the user's config onto itself with `Object.assign()`, so the
 * keys read here are not declared on the class and are restated.
 * @typedef {InstanceType<typeof import("../config/config.js").Config> & Record<string, any>} ResolvedConfig
 */

/**
 * The fix option as the user supplies it: a flag, or a predicate deciding which
 * messages to fix.
 * @typedef {boolean | ((message: LintMessage) => boolean)} FixOption
 */

/**
 * A rule category that `fixTypes` can name, mirroring a rule's `meta.type` plus
 * the pseudo-type used for unused disable directives.
 * @typedef {"directive" | "problem" | "suggestion" | "layout"} FixType
 */

/**
 * The options the `ESLint` constructor accepts, before validation. The shape is
 * open because `processOptions()` is what rejects the keys that don't belong.
 * @typedef {Record<string, any>} ESLintOptions
 */

/**
 * The options after `processOptions()` has validated and normalized them. These
 * are the options every helper below is handed.
 * @typedef {Object} ProcessedESLintOptions
 * @property {boolean} allowInlineConfig Whether directive comments are honored.
 * @property {Object|null} baseConfig The config placed before the config file's.
 * @property {boolean} cache Whether lint results are cached.
 * @property {string} cacheLocation Where the cache is stored.
 * @property {"metadata"|"content"} cacheStrategy How cache entries are invalidated.
 * @property {number|"auto"|"off"} concurrency How many worker threads to use.
 * @property {string|false} configFile The config file to use, or `false` to skip lookup.
 * @property {Object|null} overrideConfig The config appended after the config file's.
 * @property {string} cwd The absolute path everything is resolved against.
 * @property {boolean} errorOnUnmatchedPattern Whether an unmatched pattern throws.
 * @property {FixOption} fix Whether, and which, problems are fixed.
 * @property {Array<FixType>|null} fixTypes The rule categories eligible for fixing.
 * @property {Array<string>} flags The feature flags to enable.
 * @property {boolean} globInputPaths Whether glob patterns are interpreted.
 * @property {boolean} ignore Whether `ignores` outside `baseConfig` are honored.
 * @property {Array<string>|null} ignorePatterns The additional ignore patterns.
 * @property {boolean} stats Whether timing statistics are collected.
 * @property {boolean} passOnNoPatterns Whether an empty pattern list is a no-op.
 * @property {boolean} warnIgnored Whether ignored files produce a warning result.
 * @property {(ruleId: string, severity: number) => boolean} ruleFilter Selects the rules to run.
 * @property {boolean} applySuppressions Whether the suppressions file is applied.
 * @property {string} suppressionsLocation Where the suppressions file lives.
 */

/**
 * The glob patterns to search one base directory for, in both their resolved
 * and their as-the-user-typed forms.
 * @typedef {Object} Search
 * @property {Array<string>} patterns The absolute path glob patterns to match.
 * @property {Array<string>} rawPatterns The patterns as the user inputted them.
 */

/**
 * Accumulates the time spent reading files, so a worker can subtract it from
 * its linting time.
 * @typedef {Object} ReadFileCounter
 * @property {bigint} duration The nanoseconds spent reading files so far.
 */
 ...
 * @param {unknown} value The value to check.
 * @returns {boolean} `true` if `value` is an empty array or an array of
 *      non-empty strings.
 */
...
function isPositiveInteger(value) {
	// The cast is sound because `Number.isInteger()` is only true of numbers.
...
async function globMatch({ basePath, pattern }) {
	...
	const walkSettings = {
		/**
		 * Decides whether to descend into a directory.
		 * @param {HfsWalkEntry} entry The directory to consider.
		 * @returns {boolean} `true` to walk the directory's entries.
		 */
		...
		/**
		 * Decides whether an entry counts as a match.
		 * @param {HfsWalkEntry} entry The entry to consider.
		 * @returns {boolean} `true` if the entry matches the pattern.
		 */
		entryFilter(entry) {
	...
	if (await hfs.isDirectory(basePath)) {
		/*
		 * `walk()` is declared to return an `AsyncIterable`, but what it
		 * actually returns is an async generator. Advancing it once is enough:
		 * the filters above set `found` as a side effect of that single step.
		 */
		const walk = /** @type {AsyncGenerator<HfsWalkEntry>} */ (
			hfs.walk(basePath, walkSettings)
		);

		return walk.next().then(() => found);
	}
...
async function globSearch({
	...
	const walk = hfs.walk(basePath, {
		/**
		 * Skips directories that no pattern can reach, or that are ignored.
		 * @param {HfsWalkEntry} entry The directory to consider.
		 * @returns {Promise<boolean>} `true` to walk the directory's entries.
		 */
		...
		/**
		 * Keeps the files a pattern matches and a config covers.
		 * @param {HfsWalkEntry} entry The entry to consider.
		 * @returns {Promise<boolean>} `true` if the entry should be linted.
		 */
	...
	if (errorOnUnmatchedPattern && unmatchedPatterns.size > 0) {
		throw new UnmatchedSearchPatternsError({
			...
			unmatchedPatterns: [...unmatchedPatterns].map(
				/*
				 * Every member of `unmatchedPatterns` was seeded from
				 * `relativeToPatterns`'s own keys, so the lookup always hits.
				 */
				pattern =>
					/** @type {string} */ (relativeToPatterns.get(pattern)),
			),
			patterns,
			rawPatterns,
		});
 ...
 * @param {Object} options The options for this function.
 * @param {Map<string, Search>} options.searches
 *      A map of absolute path glob patterns to match, keyed by base directory.
...
async function globMultiSearch({
	...
	/*
	 * Second loop for `fulfilled` results. Every rejection was either rethrown
	 * or converted into a thrown error by the loop above, so anything still
	 * here is fulfilled.
	 */
	return results.flatMap(
		result =>
			/** @type {PromiseFulfilledResult<Array<string>>} */ (result).value,
	);
}
...
async function findFiles({
	...
	/*
	 * The cast types the map through its entries: argument inference from the
	 * array literal otherwise wins over the contextual type and pins
	 * `rawPatterns` to `never[]`.
	 */
	...
	stats.forEach((stat, index) => {
		...
		if (stat) {
			...
			if (stat.isDirectory()) {
				...
				// The entry was just created above if it was missing.
				({ patterns: globbyPatterns, rawPatterns } =
					/** @type {Search} */ (searches.get(filePath)));
		...
		if (globInputPaths && isGlobPattern(pattern)) {
			...
			// The entry was just created above if it was missing.
			({ patterns: globbyPatterns, rawPatterns } = /** @type {Search} */ (
	...
	// The glob search is the last promise pushed, so it is the last result.
	const globbyResults = /** @type {Array<string>} */ (
		(await Promise.all(promises)).at(-1)
	);
 ...
 * @param {string} [configStatus] A status that determines why the file is ignored
...
function createIgnoreResult(filePath, baseDir, configStatus) {
	...
	return {
		...
		messages: [
			/*
			 * An ignore warning is about the file as a whole, so unlike every
			 * other message it carries no position within the file.
			 */
			/** @type {LintMessage} */ ({
				ruleId: null,
				fatal: false,
...
class ESLintInvalidOptionsError extends Error {
	/**
	 * Creates a new instance.
	 * @param {Array<string>} messages The reasons the options were rejected.
	 */
 ...
 * @returns {ProcessedESLintOptions} The normalized options.
 */
 ...
 * @param {ProcessedESLintOptions} eslintOptions The processed ESLint options.
 * @param {string} cacheFilePath The path to the cache file.
 * @returns {LintResultCache|null} A new lint result cache or `null`.
 */
 ...
 * @param {LintMessage} message The message to check.
 * @param {ResolvedConfig} config The config for the file that generated the message.
 * @param {Set<FixType>} fixTypes The fix types to check.
...
function shouldMessageBeFixed(message, config, fixTypes) {
	...
	const ruleType = rule ? rule.meta?.type : void 0;

	return ruleType !== void 0 && fixTypes.has(ruleType);
}

/**
 * Creates a fixer function based on the provided fix, fixTypesSet, and config.
 * @param {FixOption} fix The original fix option.
 * @param {Set<FixType>|null} fixTypesSet A set of fix types to filter messages for fixing.
 * @param {ResolvedConfig|undefined} config The config for the file that generated the message.
 * @returns {FixOption} The fixer function or the original fix value.
 */
function getFixerForFixTypes(fix, fixTypesSet, config) {
	...
	return message =>
		shouldMessageBeFixed(
			...
			// A fixer is only built for a file some config matched.
 ...
 * @param {string} [config.filePath] The path to the file of `text`. If this is undefined, it uses `<text>`.
 * @param {FlatConfigArray} config.configs The config.
 * @param {FixOption} config.fix If `true` then it does fix.
 * @param {boolean} config.allowInlineConfig If `true` then it uses directive comments.
 * @param {(ruleId: string, severity: number) => boolean} config.ruleFilter A predicate function to filter which rules should be run.
 ...
 * @param {LintResultCache|null} lintResultCache The result cache or `null`.
 * @param {ReadFileCounter|null} readFileCounter Used to keep track of the time spent reading files.
 ...
 * @returns {Promise<LintResult|undefined>} The lint result.
 */
 ...
 * @param {Array<string>} flags The flags defined via the API.
 * @returns {Array<string>} The merged flags to use.
 */
 ...
 * @param {Record<string, Object>} [optionPlugins] The plugins specified in the ESLint options.
 * @returns {Array<Object>} The default configs.
 */
 ...
 * @param {ProcessedESLintOptions} eslintOptions The processed ESLint options.
 * @param {Array<Object>} defaultConfigs The default configs.
