...
/** @typedef {import("../shared/types.js").DeprecatedRuleInfo} DeprecatedRuleInfo */
/** @typedef {import("../shared/types.js").LintResult} LintResult */
/** @typedef {import("../shared/types.js").RuleMeta} RuleMeta */
...
/** @typedef {InstanceType<typeof import("../linter").Linter>} Linter */
/** @typedef {InstanceType<typeof import("../cli-engine/lint-result-cache")>} LintResultCache */
/** @typedef {import("./eslint-helpers.js").ESLintOptions} ESLintOptions */
/** @typedef {import("./eslint-helpers.js").ProcessedESLintOptions} ProcessedESLintOptions */
/** @typedef {import("./eslint-helpers.js").ResolvedConfig} ResolvedConfig */
/** @typedef {import("./worker.js").WorkerMessage} WorkerMessage */

/**
 * The state an `ESLint` instance keeps outside itself, so that none of it is
 * reachable through the public API.
 * @typedef {Object} ESLintPrivateMembers
 * @property {ProcessedESLintOptions} options The validated, normalized options.
 * @property {Linter} linter The linter that does the actual work.
 * @property {string} cacheFilePath Where lint results are cached.
 * @property {LintResultCache|null} lintResultCache The cache, when caching is on.
 * @property {Array<Object>} defaultConfigs The configs contributed by the `plugins` option.
 * @property {FlatConfigArray|null} configs The last config array used.
 * @property {ConfigLoader} configLoader The loader that finds and builds configs.
 * @property {WarningService} warningService The service that emits warnings.
 * @property {SuppressionsService|null} suppressionsService The suppressions file reader, when suppressions are applied.
 */

/**
 * A formatter, as returned by `loadFormatter()`.
 * @typedef {Object} LoadedFormatter
 * @property {(results: Array<LintResult>, resultsMeta?: Object) => string | Promise<string>} format Formats the given results.
 */
...
/**
 * Reads the private state belonging to an `ESLint` instance.
 * @param {ESLint} eslint The instance whose state to read.
 * @returns {ESLintPrivateMembers} The instance's private state.
 */
function getPrivateMembers(eslint) {
	/*
	 * The constructor is what fills the map, so every instance that anything
	 * else can reach already has an entry.
	 */
	return /** @type {ESLintPrivateMembers} */ (privateMembers.get(eslint));
}
 ...
 * @returns {Record<string, RuleMeta|undefined>} metadata for all enabled rules.
 */
function createRulesMeta(rules) {
	return Array.from(rules).reduce((retVal, [id, rule]) => {
		retVal[id] = rule.meta;
		return retVal;
	}, /** @type {Record<string, RuleMeta|undefined>} */ ({}));
}

/**
 * Gets the replacement rule names from a deprecated rule's metadata.
 * @param {RuleMeta} meta The rule metadata.
 * @returns {Array<string>} Replacement rule names.
 */
function getDeprecatedRuleReplacements(meta) {
	...
	return replacedBy.map((/** @type {unknown} */ replacement) => {
		if (typeof replacement !== "object" || replacement === null) {
			return "";
		}

		/*
		 * `replacedBy` entries come from a plugin's own metadata, so the two
		 * names below are read defensively rather than trusted.
		 */
		const { plugin, rule } =
			/** @type {{ plugin?: { name?: unknown }, rule?: { name?: unknown } }} */ (
				replacement
			);
...
/** @type {WeakMap<ResolvedConfig, ReadonlyArray<DeprecatedRuleInfo>>} */
 ...
 * @returns {ReadonlyArray<DeprecatedRuleInfo>} The used deprecated rule list.
 */
function getOrFindUsedDeprecatedRules(eslint, maybeFilePath) {
	const {
		options: { cwd },
		configLoader,
	} = getPrivateMembers(eslint);
	...
	// Populated just above for any config that is not already cached.
	return config
		? /** @type {ReadonlyArray<DeprecatedRuleInfo>} */ (
 ...
 * @param {ESLint} eslint The ESLint instance.
 * @param {Array<LintResult>} results The linting results to process.
 * @returns {Array<LintResult>} The processed linting results.
 */
function processLintReport(eslint, results) {
	const descriptor = {
		...
		/**
		 * Resolves the deprecated rules lazily, against the result's own file.
		 * @this {LintResult}
		 * @returns {ReadonlyArray<DeprecatedRuleInfo>} The used deprecated rules.
		 */
		get() {
 ...
 * @param {Object} options Config file location options.
 * @param {string|false} [options.configFile] The config file to use, or `false`
 *      to skip looking for one.
 * @param {string} options.cwd The directory to search up from.
 * @returns {Promise<{ configFilePath: string, basePath: string }>} Location
 *      information for the config file.
 * @throws {Error} If no config file was found.
 */
 ...
 * @param {unknown} [cause] The original error that led to this symptom error being thrown. Might not always be available.
 ...
 * @param {ProcessedESLintOptions["fix"]} fix The fix option.
 * @param {string} filePath The file for which to retrieve lint results.
 * @param {ResolvedConfig} config The config of the file.
 ...
 * @param {Array<string>} filePaths File paths to lint.
 * @param {number} maxWorkers The maximum number of workers to run.
...
function calculateAutoWorkerCount(eslint, filePaths, maxWorkers) {
	...
	for (const filePath of filePaths) {
		...
		if (
			countAllMatched ||
			/*
			 * `countAllMatched` is false only when there is a cache, so the
			 * short circuit above is what guarantees the cache is present.
			 */
			needsReprocessing(
				/** @type {LintResultCache} */ (lintResultCache),
				fix,
				filePath,
				config,
			)
...
function calculateWorkerCount(
	...
	const { concurrency } = getPrivateMembers(eslint).options;
	switch (concurrency) {
		case "off":
			return 0;
		case "auto": {
 ...
 *
 * Typed as `any` because there is no JSDoc way to declare a symbol-keyed
 * member, and the options object this keys into is keyed by string.
 */
const disableCloneabilityCheck = /** @type {any} */ (
	Symbol("Do not check for uncloneable options.")
);
 ...
 * @returns {Promise<Array<LintResult|undefined>>} Lint results.
 */
async function runWorkers(
	...
	/** @type {Array<LintResult|undefined>} */
	 ...
	 * @param {(value?: unknown) => void} resolve_ Called when the worker thread terminates successfully.
	 * @param {(reason: Error) => void} reject Called when the worker thread terminates with an error.
	 * @returns {void}
	 */
	function workerExecutor(resolve_, reject) {
		...
		worker.once(
			"message",
			(/** @type {WorkerMessage} */ indexedResults) => {
				...
				for (const result of indexedResults) {
					...
					// The index has served its purpose once the slot is chosen.
					delete (/** @type {{ index?: number }} */ (result).index);
...
async function lintFilesWithMultithreading(
	...
	const { configLoader, lintResultCache } = getPrivateMembers(eslint);
...
async function lintFilesWithoutMultithreading(eslint, filePaths) {
	...
	const retrier = new Retrier(
		(/** @type {NodeJS.ErrnoException} */ error) =>
			fileRetryCodes.has(/** @type {string} */ (error.code)),
		{
			concurrency: 100,
		},
	);
...
function validateOptionCloneability(options) {
	...
	// ESLint tags its errors with a code the CLI can branch on.
	/** @type {Error & { code?: string }} */ (error).code =
		"ESLINT_UNCLONEABLE_OPTIONS";
	throw error;
}
...
class ESLint {
	 ...
	 * @param {Array<LintResult>} results The lint results.
	 * @returns {Promise<void>} Returns a promise that is used to track side effects.
	 * @throws {Error} If `results` is not an array of objects.
	 */
				...
				.map(r =>
					retrier.retry(() =>
						fs.writeFile(
							...
							// Filtered to results with a string output above.
	 ...
	 * @param {Array<LintResult>} results The results to filter.
	 * @returns {Array<LintResult>} The filtered results.
	 */
	 ...
	 * @returns {Promise<ESLint>} The new ESLint instance.
	 * @throws {TypeError} If `optionsURL` is not a `URL`.
	 */
	 ...
	 * @param {Array<LintResult>} results The results to fetch rules meta for.
	 * @returns {Record<string, RuleMeta|undefined>} A mapping of ruleIds to rule meta objects.
	...
	hasFlag(flag) {
		...
		return getPrivateMembers(this).linter.hasFlag(flag);
	}

	/**
	 * Executes the current configuration on an array of file and directory names.
	 * @param {string|Array<string>} patterns An array of file and directory names.
	 * @returns {Promise<Array<LintResult>>} The results of linting the file patterns given.
	 * @throws {Error} If `patterns` is neither a non-empty string nor an array of them.
	 */
	async lintFiles(patterns) {
		/*
		 * Each branch below either leaves this holding an array or replaces it
		 * with one, so by the time it is used it is always an array.
		 */
		let normalizedPatterns = /** @type {Array<string>} */ (patterns);
		const {
			cacheFilePath,
			lintResultCache,
			options: eslintOptions,
		...
		if (workerCount) {
			...
			results = await lintFilesWithMultithreading(
				...
				workerCount,
				/*
				 * A non-zero worker count means concurrency is on, and the
				 * constructor sets this field whenever it is.
				 */
				/** @type {ESLintOptions|string} */ (this.#optionsOrURL),
				() =>
		...
		/*
		 * The constructor builds the service exactly when `applySuppressions`
		 * is on, which the early return above has already established.
		 */
		const service = /** @type {SuppressionsService} */ (
			suppressionsService
		);
		const { results: suppressedResults } = service.applySuppressions(
			unsuppressedResults,
			await service.load(),
		);
	 ...
	 * @returns {Promise<Array<LintResult>>} The results of linting the string of code given.
	 * @throws {Error} If `code` or any of the options is invalid.
	 */
	...
	async loadFormatter(name = "stylish") {
		...
		// grab our options
		const { cwd } = getPrivateMembers(this).options;
		...
		try {
			/*
			 * `import()` is declared to take a string, but Node.js accepts a
			 * `URL` too, which is what makes Windows paths load correctly.
			 */
			const formatterURL = /** @type {string} */ (
				/** @type {unknown} */ (pathToFileURL(formatterPath))
			);

			formatter = (await import(formatterURL)).default;
		} catch (ex) {
			// Rewriting the message means treating the thrown value as an error.
			const error = /** @type {Error} */ (ex);

			// check for formatters that have been removed
			if (isBuiltInFormatter && removedFormatters.has(name)) {
				error.message = `The ${name} formatter is no longer part of core ESLint. Install it manually with \`npm install -D eslint-formatter-${name}\``;
			} else {
				error.message = `There was a problem loading formatter: ${formatterPath}\nError: ${error.message}`;
			}
		...
		return {
			...
			format(results, resultsMeta) {
				/** @type {Record<string, RuleMeta|undefined>|null} */
				let rulesMeta = null;
	 ...
	 * @returns {Promise<ResolvedConfig|undefined>} A configuration object for the file
	 *      or `undefined` if there is no configuration data for the object.
	 * @throws {Error} If `filePath` is not a non-empty string, or no config file was found.
	 */
	async calculateConfigForFile(filePath) {
		if (!isNonEmptyString(filePath)) {
			throw new Error("'filePath' must be a non-empty string");
		}
		const options = getPrivateMembers(this).options;
		...
		if (!configs) {
			...
			// The CLI reads this to pick the message template to print.
			/** @type {Error & { messageTemplate?: string }} */ (
				error
