...
/*
 * `optionator` publishes no type declarations and has no `@types` package, so
 * tsc cannot infer a type for it. Only the three members ESLint uses are
 * described, by the `OptionatorInstance` typedef below, and the module is read
 * through that type so the `any` never reaches the emitted declarations.
 */
/**
 * Builds an optionator instance from a settings object.
 * @type {(settings: Object) => OptionatorInstance}
 */
...
/** @typedef {import("./shared/translate-cli-options.js").CLIOptions} CLIOptions */

/**
 * The options optionator produces from a command line.
 *
 * `CLIOptions` describes the subset `translateOptions()` forwards to the
 * `ESLint` constructor. The members below are the ones the CLI acts on itself,
 * plus `_`, which collects the positional file patterns.
 *
 * `maxWarnings` is restated as required rather than left optional as it is on
 * `CLIOptions`: the option below carries `default: "-1"`, so a parse always
 * produces it, and the CLI compares it numerically without a guard.
 * @typedef {CLIOptions & {
 *   _: Array<string>,
 *   maxWarnings: number,
 *   color?: boolean,
...
/**
 * The subset of an optionator instance that ESLint relies on.
 * @typedef {Object} OptionatorInstance
 * @property {(args: Array<string> | string | Object) => ParsedCLIOptions} parse Parses the given arguments.
 * @property {() => string} generateHelp Produces the help text for every option.
 * @property {(optionName: string) => string} generateHelpForOption Produces expanded help text for one option.
 */
 ...
 * @returns {OptionatorInstance} The optionator instance.
 */
