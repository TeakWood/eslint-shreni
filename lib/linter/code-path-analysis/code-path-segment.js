...
/**
 * The hidden state of a segment.
 *
 * Installed by `Object.defineProperty` in the constructor so that it is
 * non-enumerable — segments are handed to rules, and this slot is not part of
 * that surface. No structural inference can see a property declared that way,
 * so the class declares it explicitly (see the constructor).
 * @typedef {Object} SegmentInternal
 * @property {boolean} used Whether the segment has been attached to the code path.
 * @property {CodePathSegment[]} loopedPrevSegments Previous segments coming from the end of a loop.
 * @property {string[]} [nodes] The dumped nodes. Present only while debug
 * dumping is enabled — `debug-helpers.js` is the only reader.
 */
