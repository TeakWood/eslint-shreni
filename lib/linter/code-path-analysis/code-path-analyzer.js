...
function preprocess(analyzer, node) {
	...
	/**
	 * ESCAPE HATCH: `code-path-state.js` is annotated by a later bead
	 * (`y6r.13`). Its `pushForkContext` documents the one parameter without
	 * marking it optional, so inference makes it required even though that
	 * file's own callers omit it (`code-path-state.js:1417`, `:1579`) and this
	 * one does too. Widening the receiver here leaves the reserved file
	 * untouched; delete the intersection once it is annotated.
	 * @type {CodePathState & { pushForkContext(forkLeavingPath?: boolean): void }}
	 */
	const state = CodePath.getState(codePath);
	const parent = node.parent;
