...
/**
 * @typedef {InstanceType<typeof import("./id-generator.js")>} IdGenerator
 */
...
/**
 * The kind of choice a `ChoiceContext` represents.
 *
 * `"&&"`, `"||"` and `"??"` come from a `LogicalExpression` or from the
 * corresponding logical assignment with its trailing `=` removed; `"test"` from
 * an `IfStatement` or a `ConditionalExpression`; `"loop"` from a `while`,
 * `do-while` or `for` statement. This is not a discriminant — every kind uses
 * the same shape — but stating the vocabulary is what makes the `default:
 * throw new Error("unreachable")` arms in `popChoiceContext()` and
 * `makeLogicalRight()` mean something.
 * @typedef {"&&" | "||" | "??" | "test" | "loop"} ChoiceKind
 */

/**
 * The `type` of the AST node that opened a loop context.
 *
 * This is the discriminant of `LoopContext`: `pushLoopContext()` maps each
 * value to exactly one class, and `popLoopContext()` and `makeContinue()` read
 * it back.
 * @typedef {"WhileStatement" | "DoWhileStatement" | "ForStatement" | "ForInStatement" | "ForOfStatement"} LoopContextType
 */

/**
 * The context of the loop currently being traversed.
 *
 * A closed, discriminated union rather than one wide shape. The five loop forms
 * genuinely track different things — a `do-while` re-tests at the bottom and so
 * has `entrySegments` and a `continueForkContext` that nothing else has; a
 * `for` carries five separate segment pointers that no other form does — and
 * the code already discriminates on `type` where it has to (`popLoopContext()`,
 * `makeContinue()`). Modelling it as one optional-everything object would make
 * every one of those reads possibly-undefined and would let a `for-of` context
 * be handed to `makeDoWhileTest()` without complaint.
 *
 * The per-form `make*` methods do NOT discriminate; each is called by
 * `CodePathAnalyzer` only while the matching loop is on top of the stack, so
 * each asserts its own variant. See the note on `CodePathState#loopContext`.
 * @typedef {WhileLoopContext | DoWhileLoopContext | ForLoopContext | ForInLoopContext | ForOfLoopContext} LoopContext
 */

/**
 * Where traversal currently is inside a `try` statement.
 *
 * `TryContext#position` moves `"try"` -> `"catch"` -> `"finally"`, skipping
 * whichever blocks the statement does not have.
 * @typedef {"try" | "catch" | "finally"} TryPosition
 */

/**
 * The one method of `ForkContext` that this file's return/throw sinks need.
 * @typedef {(segments: CodePathSegment[]) => void} AddSegments
 */

/**
 * A plain array of segments that has been given an `add()` method.
 *
 * `CodePathState#returnedForkContext` and `#thrownForkContext` are arrays, not
 * `ForkContext`s — they are handed to rules through `CodePath#returnedSegments`
 * and `#thrownSegments` — but `getReturnContext()` and `getThrowContext()` may
 * return either them or a real `ForkContext` belonging to an enclosing `try`.
 * The constructor bolts `add()` on so the two are interchangeable at those call
 * sites, and this states the resulting shape.
 * @typedef {CodePathSegment[] & {add: AddSegments}} SegmentSink
 */

/**
 * Somewhere the segments of a `return` statement can be sent.
 *
 * Either a `TryContext` with a `finally` block — in which case the `return`
 * forks into that block — or the `CodePathState` itself, which collects the
 * code path's final segments. Described structurally because that is all the
 * call sites use, and because the two candidates store different things there
 * (a `ForkContext` and a `SegmentSink`).
 * @typedef {Object} ReturnSink
 * @property {{add: AddSegments}} returnedForkContext Where returning segments go.
 */

/**
 * Somewhere the segments of a `throw` statement can be sent. See `ReturnSink`.
 * @typedef {Object} ThrowSink
 * @property {{add: AddSegments}} thrownForkContext Where thrown segments go.
 */
...
class BreakContext {
	 ...
	 * @param {BreakContext | null} upperContext The previous `BreakContext`.
...
class ChainContext {
	/**
	 * Creates a new instance.
	 * @param {ChainContext | null} upperContext The previous `ChainContext`.
	 */
...
class ChoiceContext {
	/**
	 * Creates a new instance.
	 * @param {ChoiceContext | null} upperContext The previous `ChoiceContext`.
	 * @param {ChoiceKind} kind The kind of choice. If it's a logical or assignment expression, this
 ...
 *
 * The type parameter is what makes `LoopContext` a discriminated union: each
 * subclass fixes it to its own `type` literal, so a `context.type ===
 * "DoWhileStatement"` test narrows the union to `DoWhileLoopContext` and that
 * form's `entrySegments` and `continueForkContext` become readable. Declaring
 * `type` as the whole `LoopContextType` union here and letting the subclasses
 * pass a literal to `super()` would not do that — the property's declared type
 * is this class's, so every subclass would still read `type` as the union and
 * no `case` arm would narrow anything.
...
class LoopContextBase {
	 ...
	 * @param {Type} type The AST node's `type` for the loop.
	...
	constructor(upperContext, type, label, breakContext) {
		...
		/**
		 * The segments a `continue` targeting this loop jumps to.
		 *
		 * Declaration only, and deliberately declared on the base rather than
		 * on the four subclasses that assign it: `makeContinue()` is the one
		 * place that tells loop forms apart by the PRESENCE of this pointer
		 * instead of by `type`, so it has to be readable on every member of
		 * the union. On `DoWhileLoopContext` it stays `undefined` — that form
		 * re-tests at the bottom of the loop and routes `continue` through
		 * `continueForkContext` instead — which is why the declared type
		 * admits `undefined` and the four other forms narrow it back out.
		 * @type {CodePathSegment[] | null | undefined}
		 */
		// eslint-disable-next-line no-unused-expressions -- Declares the shared slot read by `makeContinue()`; evaluates to `undefined` and does nothing.
		this.continueDestSegments;
	}
}
...
class WhileLoopContext extends LoopContextBase {
	...
	constructor(upperContext, label, breakContext) {
		 ...
		 *
		 * ESCAPE HATCH: the cast is load-bearing, not decorative. TypeScript
		 * does not read
		 * `this.x = void 0` as a property declaration at all — a leading
		 * `@type` tag does not change that — so without it the slot simply
		 * does not exist on the class. Writing `undefined` instead of
		 * `void 0` would also declare it, at the cost of departing from the
		 * idiom this file uses throughout.
		 */
		this.test = /** @type {boolean | undefined} */ (void 0);
 ...
 *
 * This is the one loop form that never gets a `continueDestSegments` pointer;
 * see the declaration of that slot on `LoopContextBase`.
 * @extends {LoopContextBase<"DoWhileStatement">}
 */
class DoWhileLoopContext extends LoopContextBase {
	...
	constructor(upperContext, label, breakContext, forkContext) {
		 ...
		 * the loop. Used to catch infinite or skipped loops.
		 *
		 * ESCAPE HATCH: see `WhileLoopContext#test` for why the cast is
		 * load-bearing.
		 */
		this.test = /** @type {boolean | undefined} */ (void 0);
 ...
 * @extends {LoopContextBase<"ForStatement">}
 */
 ...
 * @extends {LoopContextBase<"ForInStatement">}
 */
 ...
 * @extends {LoopContextBase<"ForOfStatement">}
 */
...
class SwitchContext {
	/**
	 * Creates a new instance.
	 * @param {SwitchContext | null} upperContext The previous context.
...
class TryContext {
	/**
	 * Creates a new instance.
	 * @param {TryContext | null} upperContext The previous context.
 ...
 * @returns {LoopContext | null} A loop-context for a `continue` statement.
 */
 ...
 * @returns {BreakContext | null} A context for a `break` statement.
 */
 ...
 * @param {CodePathState} state The state to search within
 * @returns {ReturnSink} A context for a `return` statement.
 */
function getReturnContext(state) {
	...
	while (context) {
		if (context.hasFinalizer && context.position !== "finally") {
			/*
			 * ESCAPE HATCH: `TryContext#returnedForkContext` is declared
			 * nullable because it genuinely is `null` for a `try` without a
			 * `finally` block — that is the one thing its constructor branches
			 * on. This loop only ever returns a context whose `hasFinalizer` is
			 * `true`, which is exactly the condition under which the fork
			 * context was created, so it is non-null here. The `hasFinalizer`
			 * test above is what makes this safe: do not remove it.
			 */
			return /** @type {ReturnSink} */ (/** @type {unknown} */ (context));
		}
		context = context.upper;
	}
 ...
 * @param {CodePathState} state The state to search within.
 * @returns {ThrowSink} A context for a `throw` statement.
 */
 ...
 * @param {CodePathSegment[]} elements An array to remove the specific element.
 * @param {CodePathSegment} value The value to be removed.
 * @returns {void}
 */
 ...
 * @param {ForLoopContext} context A loop context to modify.
 ...
 *
 * ESCAPE HATCH, applying to every `*Context` stack pointer below. Each is
 * declared nullable, because each genuinely is `null` before its first `push*`
 * and again after the outermost `pop*`. The `pop*` and `make*` methods
 * nevertheless dereference them without a guard, because `CodePathAnalyzer` is
 * the only caller and it only calls each method between the matching `push*`
 * and `pop*` — an ordering the checker cannot see. Every one of those reads
 * therefore asserts the context it needs, and for the loop stack the assertion
 * also selects the variant the caller has established (see `LoopContext`).
 *
 * The assertions are deliberately at the reads rather than hidden by declaring
 * the fields non-nullable: `getBreakContext()`, `getContinueContext()`,
 * `getReturnContext()` and `getThrowContext()` all walk these stacks and test
 * for the end of one, and typing the fields non-null would make that
 * termination test read as dead code.
 */
class CodePathState {
	/**
	 * Creates a new instance.
	 ...
	 *   path segments.
	 * @param {LoopedCallback} onLooped A callback function to notify looping.
	 */
	constructor(idGenerator, onLooped) {
		...
		/*
		 * ESCAPE HATCH, for both sinks below. Each is declared as a
		 * `SegmentSink` — an array that also has `add()` — but the `add()`
		 * method is bolted on a few statements further down, so an empty array
		 * literal cannot satisfy that shape at the point of assignment. The
		 * widening goes through `unknown` because `never[]` and `SegmentSink`
		 * do not overlap enough for a direct assertion. Moving the two `add`
		 * assignments up here instead would not help: they close over
		 * `finalSegments` and over each other.
		 */
		 ...
		 * @type {SegmentSink}
		 */
		this.returnedForkContext = /** @type {SegmentSink} */ (
			/** @type {unknown} */ ([])
		);
		 ...
		 * @type {SegmentSink}
		 */
		this.thrownForkContext = /** @type {SegmentSink} */ (
			/** @type {unknown} */ ([])
		);
	 ...
	 * @returns {CodePathSegment[]} The head segments of the current fork context.
	 */
	get headSegments() {
	 ...
	 * @returns {ForkContext | null} The enclosing fork context, or `null` at the root.
	 */
	...
	popForkContext() {
		...
		/*
		 * ESCAPE HATCH: `ForkContext#upper` is `null` only for the root
		 * context, which is created in the constructor and never popped —
		 * every `popForkContext()` is paired with a `pushForkContext()` by
		 * `CodePathAnalyzer`.
		 */
		this.forkContext = /** @type {ForkContext} */ (lastContext.upper);
	 ...
	 */
	forkPath() {
		// ESCAPE HATCH: forking only happens inside a pushed context, whose `upper` is the context it was pushed from. See `popForkContext()`.
		const parentForkContext = /** @type {ForkContext} */ (
			this.parentForkContext
		);

		this.forkContext.add(parentForkContext.makeNext(-1, -1));
	}
	 ...
	 */
	forkBypassPath() {
		// ESCAPE HATCH: see `forkPath()`.
		const parentForkContext = /** @type {ForkContext} */ (
			this.parentForkContext
		);

		this.forkContext.add(parentForkContext.head);
	}
	 ...
	 *     a -> b -> foo();
	 *     a -> b -> bar();
	 * @param {ChoiceKind} kind A kind string.
	 ...
	 */
	popChoiceContext() {
		// ESCAPE HATCH: only called between the matching `pushChoiceContext()` and here. See the note on the class.
		const poppedChoiceContext = /** @type {ChoiceContext} */ (
			this.choiceContext
		);
		...
		switch (poppedChoiceContext.kind) {
			...
			case "??":
				...
				if (poppedChoiceContext.isForkingAsResult) {
					/*
					 * ESCAPE HATCH: `isForkingAsResult` is set only when this
					 * context is the left (test) operand of another choice, so
					 * reaching here establishes that the popped context had a
					 * parent — which is now on top of the stack.
					 */
					const parentContext = /** @type {ChoiceContext} */ (
						this.choiceContext
					);
	 ...
	 */
	makeLogicalRight() {
		// ESCAPE HATCH: only called between the matching `pushChoiceContext()` and `popChoiceContext()`. See the note on the class.
		const currentChoiceContext = /** @type {ChoiceContext} */ (
			this.choiceContext
		);
		const forkContext = this.forkContext;
	 ...
	 */
	makeIfConsequent() {
		// ESCAPE HATCH: only called between the `"test"` `pushChoiceContext()` and its pop. See the note on the class.
		const context = /** @type {ChoiceContext} */ (this.choiceContext);
		const forkContext = this.forkContext;
	 ...
	 */
	makeIfAlternate() {
		// ESCAPE HATCH: see `makeIfConsequent()`.
		const context = /** @type {ChoiceContext} */ (this.choiceContext);
		const forkContext = this.forkContext;
	 ...
	 */
	popChainContext() {
		// ESCAPE HATCH: only called on leaving a `ChainExpression`, which pushed one on entry. See the note on the class.
		const context = /** @type {ChainContext} */ (this.chainContext);
	 ...
	 */
	popSwitchContext() {
		// ESCAPE HATCH: only called on leaving a `SwitchStatement`, which pushed one on entry. See the note on the class.
		const context = /** @type {SwitchContext} */ (this.switchContext);
		...
		if (!context.lastIsDefault) {
			if (context.defaultBodySegments) {
				 ...
				 *
				 * ESCAPE HATCH: `defaultSegments` is assigned in the same
				 * `isDefaultCase` branch of `makeSwitchCaseBody()` that
				 * assigns `defaultBodySegments`, and before it, so a non-null
				 * `defaultBodySegments` implies a non-null `defaultSegments`.
				 * The checker sees two independent nullable pointers.
				 */
				disconnectSegments(
					/** @type {CodePathSegment[]} */ (context.defaultSegments),
		 ...
		 *
		 * ESCAPE HATCH: `forkCount` counts the contexts `makeSwitchCaseBody()`
		 * pushed for this statement, so there are exactly that many to unwind
		 * before reaching the one the `switch` started in. See
		 * `popForkContext()` for why the root is never reached.
		 */
		for (let i = 0; i < context.forkCount; ++i) {
			this.forkContext = /** @type {ForkContext} */ (
				this.forkContext.upper
			);
		}
	...
	makeSwitchCaseBody(isCaseBodyEmpty, isDefaultCase) {
		// ESCAPE HATCH: only called for a `SwitchCase`, which is always inside the `SwitchStatement` that pushed the context. See the note on the class.
		const context = /** @type {SwitchContext} */ (this.switchContext);
	 ...
	 */
	popTryContext() {
		// ESCAPE HATCH: only called on leaving a `TryStatement`, which pushed one on entry. See the note on the class.
		const context = /** @type {TryContext} */ (this.tryContext);
		...
		/*
		 * ESCAPE HATCH: `returnedForkContext` is `null` exactly when the `try`
		 * has no `finally` block. Such a statement must have a `catch`, and
		 * `makeCatchBlock()` leaves `position` at `"catch"` — which the early
		 * return above has already handled. Everything from here on runs only
		 * for a `try` with a `finally`, so the fork context exists.
		 */
		const originalReturnedForkContext = /** @type {ForkContext} */ (
		...
		// ESCAPE HATCH: the `try` pushed the context being unwound here. See `popForkContext()`.
		this.forkContext = /** @type {ForkContext} */ (this.forkContext.upper);
		...
		if (!context.lastOfTryIsReachable && !context.lastOfCatchIsReachable) {
			/*
			 * ESCAPE HATCH, and a latent defect this annotation exposed rather
			 * than introduced. `makeUnreachable()` takes two indices and
			 * returns segments; every other call in this file is
			 * `replaceHead(makeUnreachable(-1, -1))`. Called with no arguments
			 * both indices are `undefined`, so `createSegments()` computes
			 * `NaN` bounds, produces one segment per fork with no previous
			 * segments, and the result is then discarded — which leaves this
			 * statement doing nothing except consuming `count` segment ids.
			 * The evident intent was to replace the head with unreachable
			 * segments.
			 *
			 * NOT FIXED HERE: segment ids are observable to rules through
			 * `CodePathSegment#id`, so both correcting the call and deleting
			 * it renumber every segment created after this point. That is a
			 * behaviour change, and this bead is an annotation. The receiver
			 * is widened so the arity mismatch is described rather than
			 * silently accepted; upstream carries the same line.
			 */
			const forkContext =
				/** @type {{makeUnreachable(startIndex?: number, endIndex?: number): CodePathSegment[]}} */ (
					this.forkContext
				);
	 ...
	 */
	makeCatchBlock() {
		// ESCAPE HATCH: a `CatchClause` is only reached inside the `TryStatement` that pushed the context. See the note on the class.
		const context = /** @type {TryContext} */ (this.tryContext);
		const forkContext = this.forkContext;
		const originalThrownForkContext = context.thrownForkContext;
	 ...
	 */
	makeFinallyBlock() {
		// ESCAPE HATCH: a `finally` block is only reached inside the `TryStatement` that pushed the context. See the note on the class.
		const context = /** @type {TryContext} */ (this.tryContext);
		let forkContext = this.forkContext;

		/*
		 * ESCAPE HATCH: reaching a `finally` block means the statement has one,
		 * which is the condition under which `TryContext` creates
		 * `returnedForkContext`. See `popTryContext()`.
		 */
		const originalReturnedForkContext = /** @type {ForkContext} */ (
			context.returnedForkContext
		);
	...
	makeFirstThrowablePathInTryOrCatchBlock() {
		...
		/*
		 * ESCAPE HATCH: `getThrowContext()` returns either an enclosing
		 * `TryContext` or this state itself, and the first guard below returns
		 * for the second case — but a `===` test against `this` narrows
		 * nothing for the checker. This is the only caller that needs more
		 * than somewhere to put segments: the reads below (`empty`,
		 * `position`, `hasFinalizer`) exist only on a `TryContext`, which is
		 * why the helper still describes its result by the sink surface alone.
		 * The `context === this` test is what makes this safe: do not remove
		 * it, and keep it first.
		 */
		const tryContext = /** @type {TryContext} */ (
			/** @type {unknown} */ (context)
		);

		if (context === this || !tryContext.thrownForkContext.empty) {
			return;
		}

		if (
			tryContext.position !== "try" &&
			(tryContext.position !== "catch" || !tryContext.hasFinalizer)
		) {
			return;
		}

		tryContext.thrownForkContext.add(forkContext.head);
		forkContext.replaceHead(forkContext.makeNext(-1, -1));
	}
	 ...
	 * @param {LoopContextType} type The type of the node which was triggered. One of
	 *   `WhileStatement`, `DoWhileStatement`, `ForStatement`, `ForInStatement`,
	 *   and `ForStatement`.
	 * @param {string | null} label A label of the node which was triggered.
	 * @throws (Unreachable - unknown type.)
	 * @returns {void}
	 */
	...
	popLoopContext() {
		/*
		 * ESCAPE HATCH: only called on leaving a loop, which pushed one on
		 * entry. See the note on the class. Unlike the `make*` methods below,
		 * this one does not assert a variant — the `switch` on `context.type`
		 * discriminates the union, which is what that field is for.
		 */
		const context = /** @type {LoopContext} */ (this.loopContext);
		...
		switch (context.type) {
			...
			case "ForStatement":
				 ...
				 *
				 * ESCAPE HATCH: `continueDestSegments` starts `null` and is
				 * filled in while the loop is traversed — by `makeWhileTest()`
				 * for a `while`, and by whichever of `makeForTest()`,
				 * `makeForUpdate()` or `makeForBody()` runs for a `for`. All of
				 * those precede the loop being popped, so it is set by now.
				 */
				makeLooped(
					this,
					forkContext.head,
			...
			case "DoWhileStatement": {
				...
				for (let i = 0; i < segmentsList.length; ++i) {
					/*
					 * ESCAPE HATCH: `entrySegments` is assigned by
					 * `makeDoWhileBody()`, and a `do-while` always has a body,
					 * so it is set before the loop can be popped.
					 */
					makeLooped(
						this,
						segmentsList[i],
			...
			case "ForOfStatement":
				 ...
				 *
				 * ESCAPE HATCH: `leftSegments` is assigned by
				 * `makeForInOfLeft()`, and the left of a `for-in`/`for-of` is
				 * not optional, so it is set before the loop can be popped.
				 */
				makeLooped(
					this,
					forkContext.head,
					/** @type {CodePathSegment[]} */ (context.leftSegments),
				);
				break;
	...
	makeWhileTest(test) {
		/*
		 * ESCAPE HATCH: only called for the test of a `WhileStatement`, so the
		 * context on top of the stack is the `WhileLoopContext` that statement
		 * pushed. This is the assertion the note on the class describes: it
		 * both establishes that a loop context exists and selects the variant
		 * the caller has already committed to.
		 */
		const context = /** @type {WhileLoopContext} */ (this.loopContext);
		const forkContext = this.forkContext;
	 ...
	 */
	makeWhileBody() {
		// ESCAPE HATCH: see `makeWhileTest()`; `pushLoopContext("WhileStatement")` also pushed the `"loop"` choice context.
		const context = /** @type {WhileLoopContext} */ (this.loopContext);
		const choiceContext = /** @type {ChoiceContext} */ (this.choiceContext);
		const forkContext = this.forkContext;
	 ...
	 */
	makeDoWhileBody() {
		// ESCAPE HATCH: see `makeWhileTest()`. Only called for the body of a `DoWhileStatement`.
		const context = /** @type {DoWhileLoopContext} */ (this.loopContext);
		const forkContext = this.forkContext;
	...
	makeDoWhileTest(test) {
		// ESCAPE HATCH: see `makeWhileTest()`. Only called for the test of a `DoWhileStatement`.
		const context = /** @type {DoWhileLoopContext} */ (this.loopContext);
		const forkContext = this.forkContext;
	 ...
	 */
	makeForTest(test) {
		// ESCAPE HATCH: see `makeWhileTest()`. Only called for the test of a `ForStatement`.
		const context = /** @type {ForLoopContext} */ (this.loopContext);
		const forkContext = this.forkContext;
	 ...
	 */
	makeForUpdate() {
		// ESCAPE HATCH: see `makeWhileTest()`; `pushLoopContext("ForStatement")` also pushed the `"loop"` choice context.
		const context = /** @type {ForLoopContext} */ (this.loopContext);
		const choiceContext = /** @type {ChoiceContext} */ (this.choiceContext);
		const forkContext = this.forkContext;
	 ...
	 */
	makeForBody() {
		// ESCAPE HATCH: see `makeForUpdate()`.
		const context = /** @type {ForLoopContext} */ (this.loopContext);
		const choiceContext = /** @type {ChoiceContext} */ (this.choiceContext);
		const forkContext = this.forkContext;
		...
		if (!bodySegments) {
			...
			/*
			 * ESCAPE HATCH: reaching here means the loop has no test
			 * expression, and every path to a `for` body without one has
			 * already assigned `endOfInitSegments` — the `else` branch of
			 * `makeForUpdate()` when there is an update expression, and the
			 * `else` branch just above when there is neither.
			 */
			prevForkContext.add(
				/** @type {CodePathSegment[]} */ (context.endOfInitSegments),
			);
	...
	makeForInOfLeft() {
		/*
		 * ESCAPE HATCH: see `makeWhileTest()`. The two `for-in`/`for-of`
		 * contexts have the same shape, so the assertion selects them as a
		 * pair and nothing below needs to tell them apart.
		 */
		const context = /** @type {ForInLoopContext | ForOfLoopContext} */ (
			this.loopContext
		);
		const forkContext = this.forkContext;
	 ...
	 */
	makeForInOfRight() {
		// ESCAPE HATCH: see `makeForInOfLeft()`.
		const context = /** @type {ForInLoopContext | ForOfLoopContext} */ (
			this.loopContext
		);
		const forkContext = this.forkContext;
		...
		/*
		 * ESCAPE HATCH: `prevSegments` is assigned by `makeForInOfLeft()`,
		 * which always runs first — the left of a `for-in`/`for-of` is not
		 * optional and is traversed before the right.
		 */
		temp.add(/** @type {CodePathSegment[]} */ (context.prevSegments));
	...
	makeForInOfBody() {
		...
		/*
		 * ESCAPE HATCH: `endOfLeftSegments` and `leftSegments` are assigned by
		 * `makeForInOfRight()` and `makeForInOfLeft()` respectively, both of
		 * which precede the body in the traversal. See `makeForInOfRight()`.
		 */
		temp.add(/** @type {CodePathSegment[]} */ (context.endOfLeftSegments));
	 ...
	 * @returns {BreakContext} The removed context.
	 */
	popBreakContext() {
		// ESCAPE HATCH: only called on leaving a statement that pushed one on entry. See the note on the class.
		const context = /** @type {BreakContext} */ (this.breakContext);
		const forkContext = this.forkContext;
	...
	makeContinue(label) {
		...
		if (context) {
			if (context.continueDestSegments) {
				...
				/*
				 * ESCAPE HATCH: this is the one place that tells loop forms
				 * apart by the PRESENCE of `continueDestSegments` instead of
				 * by `type`, and TypeScript cannot narrow a union that way.
				 * `DoWhileLoopContext` is the only form that never gets that
				 * pointer — it re-tests at the bottom, so `continue` has to
				 * fork to the test rather than jump to a known segment — so
				 * this branch is reachable only for a `do-while`.
				 */
				const doWhileContext = /** @type {DoWhileLoopContext} */ (
					context
				);

				doWhileContext.continueForkContext.add(forkContext.head);
			}
		}
		forkContext.replaceHead(forkContext.makeUnreachable(-1, -1));
	}
