...
class BreakContext {
	 ...
	 * @param upperContext The previous `BreakContext`.
	 * @param breakable Indicates if we are inside a statement where
	 *      `break` without a label will exit the statement.
	 * @param label The label for the statement.
	 * @param forkContext The current fork context.
	 */
...
class ChainContext {
	/**
	 * Creates a new instance.
	 * @param upperContext The previous `ChainContext`.
	 */
...
class ChoiceContext {
	/**
	 * Creates a new instance.
	 * @param upperContext The previous `ChoiceContext`.
	 * @param kind The kind of choice. If it's a logical or assignment expression, this
	 *      is `"&&"` or `"||"` or `"??"`; if it's an `if` statement or
	 *      conditional expression, this is `"test"`; otherwise, this is `"loop"`.
	 * @param isForkingAsResult Indicates if the result of the choice
	 *      creates a fork.
	 * @param forkContext The containing `ForkContext`.
	 */
...
class LoopContextBase {
	/**
	 * Creates a new instance.
	 * @param upperContext The previous `LoopContext`.
	 * @param type The AST node's `type` for the loop.
	 * @param label The label for the loop from an enclosing `LabeledStatement`.
	 * @param breakContext The context for breaking the loop.
	 */
...
class DoWhileLoopContext extends LoopContextBase {
	 ...
	 * @param label The label for the loop from an enclosing `LabeledStatement`.
	 * @param breakContext The context for breaking the loop.
	 * @param forkContext The enclosing fork context.
	 */
...
class SwitchContext {
	/**
	 * Creates a new instance.
	 * @param upperContext The previous context.
	 * @param hasCase Indicates if there is at least one `case` statement.
...
class TryContext {
	 ...
	 * Creates a new instance.
	 * @param upperContext The previous context.
	 * @param hasFinalizer Indicates if the `try` statement has a
	 *      `finally` block.
	 * @param forkContext The enclosing fork context.
	 */
 ...
 * @param dest A destination array (`returnedSegments` or `thrownSegments`).
 * @param others Another destination array (`returnedSegments` or `thrownSegments`).
 * @param all The unified destination array (`finalSegments`).
 ...
 * @param label The label of a `continue` statement.
 * @returns A loop-context for a `continue` statement.
 */
 ...
 * @param state The state to search within.
 * @param label The label of a `break` statement.
 * @returns A context for a `break` statement.
 */
 ...
 * @param state The state to search within
 * @returns A context for a `return` statement.
 */
 ...
 * @param state The state to search within.
 * @returns A context for a `throw` statement.
 */
 ...
 * @param elements An array to remove the specific element.
 ...
 * @param prevSegments Forward segments to disconnect.
 * @param nextSegments Backward segments to disconnect.
 */
 ...
 * @param unflattenedFromSegments Segments which are source.
 * @param unflattenedToSegments Segments which are destination.
 */
 ...
 * @param context A loop context to modify.
 * @param choiceContext A choice context of this loop.
 * @param head The current head paths.
 */
...
class CodePathState {
	/**
	 * Creates a new instance.
	 * @param idGenerator An id generator to generate id for code
	 *   path segments.
	 * @param onLooped A callback function to notify looping.
	 */
	 ...
	 * @param forkLeavingPath A flag which shows being in a
	 *   "finally" block.
	 ...
	 * @param hasCase `true` if the switch statement has one or more
	 *   case parts.
	 ...
	 * @param isCaseBodyEmpty `true` if the body is empty.
	 * @param isDefaultCase `true` if the body is the default case.
	 */
	 ...
	 * @param hasFinalizer `true` if the try statement has a
	 *   `finally` block.
	 */
	 ...
	 * @param type The type of the node which was triggered. One of
	 *   `WhileStatement`, `DoWhileStatement`, `ForStatement`, `ForInStatement`,
	 *   and `ForStatement`.
	 * @param label A label of the node which was triggered.
	 ...
	 * @param test The test value (only when constant).
	 */
	makeWhileTest(test) {
		const context = this.loopContext;
		const forkContext = this.forkContext;
	 ...
	 * @param label A label of the break statement.
	 */
	makeBreak(label) {
		const forkContext = this.forkContext;
	 ...
	 * It makes new unreachable segment, then it set the head with the segment.
	 * @param label A label of the continue statement.
	 */
	makeContinue(label) {
		const forkContext = this.forkContext;
