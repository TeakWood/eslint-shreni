...
/** @typedef {import("./id-generator.js")} IdGenerator */

/**
 * The `type` of the AST node that starts a loop.
 * @typedef {"WhileStatement" | "DoWhileStatement" | "ForStatement" | "ForInStatement" | "ForOfStatement"} LoopType
 */

/**
 * The kind of a `ChoiceContext`. Logical and logical-assignment expressions use
 * the operator itself, `if` statements and conditional expressions use `"test"`,
 * and loops use `"loop"`.
 * @typedef {"&&" | "||" | "??" | "test" | "loop"} ChoiceKind
 */

/**
 * Any of the five loop contexts. Each one narrows the inherited `type` property
 * to a single literal, so this is a discriminated union and a `switch` over
 * `context.type` narrows to the matching class.
 * @typedef {WhileLoopContext | DoWhileLoopContext | ForLoopContext | ForInLoopContext | ForOfLoopContext} LoopContext
 */

/**
 * The loop contexts that jump to a known destination on `continue`. That is
 * every loop except `do-while`, whose test runs after the body and which
 * therefore collects `continue` paths in a fork context instead.
 * @typedef {WhileLoopContext | ForLoopContext | ForInLoopContext | ForOfLoopContext} ContinuableLoopContext
 */

/**
 * The callback the analyzer registers to hear about loops being created.
 * @typedef {(fromSegment: CodePathSegment, toSegment: CodePathSegment) => void} LoopedCallback
 */

/**
 * An array of final segments that also carries `ForkContext`'s `add()` method.
 * `CodePathState` bolts `add()` onto its `returnedForkContext` and
 * `thrownForkContext` arrays so that `getReturnContext()` and
 * `getThrowContext()` can hand back either one and callers can treat the two
 * interchangeably.
 * @typedef {Array<CodePathSegment> & { add: (segments: Array<CodePathSegment>) => void }} FinalSegments
 */
 ...
 *
 * The class is generic over the loop's node `type` so that each subclass pins it
 * to a single literal. That is what makes `LoopContext` a discriminated union:
 * a `switch` over `context.type` narrows to the matching subclass without any
 * cast.
 * @template {LoopType} [T=LoopType]
 */
class LoopContextBase {
	/**
	 * Creates a new instance.
	 * @param {LoopContext | null} upperContext The previous `LoopContext`.
	 * @param {T} type The AST node's `type` for the loop.
...
class WhileLoopContext extends LoopContextBase {
	...
	constructor(upperContext, label, breakContext) {
		 ...
		 *
		 * The type is carried by the cast rather than a `@type` tag because
		 * TypeScript does not synthesize a property declaration from an
...
class TryContext {
	...
	constructor(upperContext, hasFinalizer, forkContext) {
		 ...
		 *
		 * This is non-null exactly when `hasFinalizer` is `true`.
 ...
 * @param {Array<CodePathSegment>} dest A destination array (`returnedSegments` or `thrownSegments`).
 * @param {Array<CodePathSegment>} others Another destination array (`returnedSegments` or `thrownSegments`).
 * @param {Array<CodePathSegment>} all The unified destination array (`finalSegments`).
 * @param {Array<CodePathSegment>} segments Segments to add.
 * @returns {void}
 */
 ...
 * @param {CodePathState} state The state to search within
 * @returns {{ returnedForkContext: ForkContext | FinalSegments }} A context for a `return` statement.
 */
function getReturnContext(state) {
	...
	while (context) {
		if (context.hasFinalizer && context.position !== "finally") {
			/*
			 * `TryContext#returnedForkContext` is created exactly when the
			 * context has a `finally` block, which the check above has just
			 * established, so it is non-null here.
			 */
			return /** @type {{ returnedForkContext: ForkContext }} */ (
				context
			);
		}
		context = context.upper;
 ...
 * @param {Array<CodePathSegment>} elements An array to remove the specific element.
 ...
 * @param {Array<CodePathSegment>} prevSegments Forward segments to disconnect.
 * @param {Array<CodePathSegment>} nextSegments Backward segments to disconnect.
 * @returns {void}
 */
 ...
 * @param {Array<CodePathSegment>} unflattenedFromSegments Segments which are source.
 * @param {Array<CodePathSegment>} unflattenedToSegments Segments which are destination.
 * @returns {void}
 */
 ...
 * @param {Array<CodePathSegment>} head The current head paths.
 * @returns {void}
 */
...
class CodePathState {
	...
	constructor(idGenerator, onLooped) {
		...
		/*
		 * The context stacks below all start out empty and are pushed and popped
		 * as the analyzer walks the AST. Every method that reads one is only
		 * reached while the node type that pushed it is being traversed, so the
		 * context is always present by the time it is read — but that invariant
		 * lives in `code-path-analyzer.js` and is not something TypeScript can
		 * see. Readers therefore cast the `null` away, and the loop readers
		 * additionally cast to the specific loop context their node type pushes.
		 */
		 ...
		 * segments are also contained in `finalSegments`. The `add()` method
		 * that completes the `FinalSegments` shape is attached just below.
		 * @type {FinalSegments}
		 */
		this.returnedForkContext = /** @type {FinalSegments} */ (
			/** @type {Array<CodePathSegment>} */ ([])
		);
		 ...
		 * @type {FinalSegments}
		 */
		this.thrownForkContext = /** @type {FinalSegments} */ (
			/** @type {Array<CodePathSegment>} */ ([])
		);
	 ...
	 * @returns {Array<CodePathSegment>} The head segments of the current fork context.
	 */
	get headSegments() {
	 ...
	 * @returns {ForkContext | null} The parent fork context, or `null` at the root.
	 */
	...
	popForkContext() {
		...
		// A context is only popped after a matching push, so `upper` is set.
	 ...
	 */
	forkPath() {
		// Forking only happens inside a pushed context, which always has a parent.
		...
		switch (poppedChoiceContext.kind) {
			...
			case "??":
				...
				if (poppedChoiceContext.isForkingAsResult) {
					/*
					 * `isForkingAsResult` is only set when this context is the
					 * test expression of an enclosing choice context, so the
					 * parent exists.
					 */
	...
	popSwitchContext() {
		...
		if (!context.lastIsDefault) {
			if (context.defaultBodySegments) {
				 ...
				 */
				/*
				 * `defaultSegments` is assigned alongside `defaultBodySegments`
				 * in `makeSwitchCaseBody()`, so it is non-null whenever
				 * `defaultBodySegments` is.
				 */
				disconnectSegments(
					/** @type {Array<CodePathSegment>} */ (
		...
		for (let i = 0; i < context.forkCount; ++i) {
			/*
			 * Each iteration undoes one `pushForkContext()` from
			 * `makeSwitchCaseBody()`, so `upper` is set every time.
			 */
	...
	popTryContext() {
		...
		/*
		 * Reaching this point means the traversal has left a `finally` block,
		 * so the context has a finalizer and `returnedForkContext` is non-null.
		 */
		...
		// `makeFinallyBlock()` pushed a fork context, so `upper` is set.
	...
	makeFinallyBlock() {
		...
		/*
		 * This method only runs for a `try` statement that has a `finally`
		 * block, so `returnedForkContext` is non-null.
		 */
		const originalReturnedForkContext = /** @type {ForkContext} */ (
			context.returnedForkContext
		);
	...
	makeFirstThrowablePathInTryOrCatchBlock() {
		...
		/*
		 * `getThrowContext()` returns either `this` or a `TryContext`. The
		 * `context === this` check below rules out the former before any
		 * `TryContext`-only property is read, but TypeScript cannot follow that
		 * through the `||`, so the narrowed alias is spelled out here.
		 */
		const tryContext = /** @type {TryContext} */ (context);
	 ...
	 * @param {LoopType} type The type of the node which was triggered. One of
	...
	popLoopContext() {
		...
		switch (context.type) {
			...
			case "ForStatement":
				 ...
				 *
				 * `continueDestSegments` is assigned while traversing the loop —
				 * by `makeWhileTest()` for `while`, and by `makeForTest()` or
				 * `makeForBody()` for `for` — so it is set by the time the loop
				 * is popped.
				 */
				makeLooped(
					this,
			...
			case "DoWhileStatement": {
				...
				for (let i = 0; i < segmentsList.length; ++i) {
					/*
					 * `entrySegments` is assigned by `makeDoWhileBody()`, which
					 * always runs before the loop is popped.
					 */
					makeLooped(
						this,
						segmentsList[i],
			...
			case "ForOfStatement":
				 ...
				 *
				 * `leftSegments` is assigned by `makeForInOfLeft()`, which
				 * always runs before the loop is popped.
				 */
				makeLooped(
					this,
	...
	makeForBody() {
		...
		if (!bodySegments) {
			...
			/*
			 * With no test segments, the init expression is where the body
			 * path starts, and `endOfInitSegments` has been assigned by
			 * `makeForTest()`, `makeForUpdate()`, or the branch above.
			 */
			prevForkContext.add(
				/** @type {Array<CodePathSegment>} */ (
					context.endOfInitSegments
				),
			);
	...
	makeForInOfRight() {
		...
		// `prevSegments` is assigned by `makeForInOfLeft()`, which runs first.
		temp.add(/** @type {Array<CodePathSegment>} */ (context.prevSegments));
	...
	makeForInOfBody() {
		...
		/*
		 * `endOfLeftSegments` and `leftSegments` are assigned by
		 * `makeForInOfLeft()` and `makeForInOfRight()`, both of which run
		 * before the loop body is entered.
		 */
		temp.add(
			/** @type {Array<CodePathSegment>} */ (context.endOfLeftSegments),
		);
		...
		makeLooped(
			this,
			forkContext.head,
			/** @type {Array<CodePathSegment>} */ (context.leftSegments),
		);
	...
	makeContinue(label) {
		...
		if (context) {
			/*
			 * Every loop context except `do-while` carries
			 * `continueDestSegments`. Reading it off the union means naming one
			 * of the branches, so the destination is read through the
			 * non-`do-while` shape and the `else` below handles `do-while`,
			 * where the property is absent and therefore falsy.
			 */
			const continueDestSegments = /** @type {ContinuableLoopContext} */ (
				context
			).continueDestSegments;

			if (continueDestSegments) {
				makeLooped(this, forkContext.head, continueDestSegments);
				...
				/*
				 * `do-while` is the only loop without `continueDestSegments`:
				 * its test runs after the body, so `continue` paths are
				 * collected in a fork context instead.
				 */
				/** @type {DoWhileLoopContext} */ (
					context
				).continueForkContext.add(forkContext.head);
			}
		}
		forkContext.replaceHead(forkContext.makeUnreachable(-1, -1));
	}
