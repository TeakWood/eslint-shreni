...
/**
 * A factory that creates one segment of a fork.
 *
 * This is `CodePathSegment.newNext`, `.newUnreachable` or `.newDisconnected`:
 * `createSegments` takes whichever one the caller wants as a wrapper around
 * `new CodePathSegment()`.
 * @typedef {(id: string, allPrevSegments: CodePathSegment[]) => CodePathSegment} SegmentFactory
 */
 ...
 *      that should be specified as previous segments for the newly created segments.
 * @param {SegmentFactory} create A function that creates new `CodePathSegment`
 *      instances in a particular way. See the `CodePathSegment.new*` methods.
 * @returns {CodePathSegment[]} An array of the newly-created segments.
 */
...
class ForkContext {
	 ...
	 * @param {ForkContext | null} upper The preceding fork context. `null` for
	 *      a root context.
	 ...
	 * @returns {CodePathSegment[]} The head segments, or an empty array when
	 * the context has none.
	 */
	get head() {
		const list = this.segmentsList;

		// The `length === 0` test is what makes the `at(-1)` read safe.
		return list.length === 0
			? []
			: /** @type {CodePathSegment[]} */ (list.at(-1));
	}

	/**
	 * Indicates if the context contains no segments.
	 * @returns {boolean} `true` if there are no segments.
	 */
	get empty() {
	 ...
	 * @returns {boolean} `true` if any head segment is reachable.
	 */
	get reachable() {
	 ...
	 * @param {CodePathSegment[]} segments The segments to add.
	 * @returns {void}
	 */
	add(segments) {
		assert(
	 ...
	 * @param {CodePathSegment[]} replacementHeadSegments The new head segments.
	 * @returns {void}
	 */
