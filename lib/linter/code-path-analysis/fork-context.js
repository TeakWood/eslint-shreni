...
/**
 * A factory that creates a `CodePathSegment`. These are the
 * `CodePathSegment.new*` static methods.
 * @typedef {(id: string, allPrevSegments: Array<CodePathSegment>) => CodePathSegment} SegmentFactory
 */
...
class ForkContext {
	 ...
	 * @returns {Array<CodePathSegment>} The head segments, or an empty array
	 *      when the context has no segments yet.
	 */
	get head() {
		const list = this.segmentsList;

		// `at(-1)` is non-nullish because the list is known to be non-empty here.
		return list.length === 0
			? []
			: /** @type {Array<CodePathSegment>} */ (list.at(-1));
	}
	 ...
	 * @returns {boolean} `true` if any segment is reachable.
	 */
	get reachable() {
	...
	makeUnreachable(startIndex, endIndex) {
		/*
		 * `CodePathState#popTryContext()` calls this with no arguments. The
		 * resulting `NaN` range makes `createSegments()` skip its inner loop
		 * entirely, producing segments with no previous segments at all, which
		 * is what that call site wants.
		 */
		return createSegments(
			this,
