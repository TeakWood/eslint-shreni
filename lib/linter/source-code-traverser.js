...
/** @typedef {import("./esquery.js").ESQueryParsedSelector} ESQueryParsedSelector */
/** @typedef {import("./source-code-visitor.js").SourceCodeVisitor} SourceCodeVisitor */
...
/**
 * The slice of a `Language` object this traverser reads. Languages are supplied
 * by plugins, so only the members used here are pinned.
 * @typedef {Object} TraversableLanguage
 * @property {ESQueryOptions["visitorKeys"]} [visitorKeys] The visitor keys for the language's AST.
 * @property {ESQueryOptions["matchClass"]} [matchesSelectorClass] Tests whether a node belongs to an esquery class such as `:function`.
 * @property {string} [nodeTypeKey] The property holding a node's type, when it is not `type`.
 */

/**
 * A step that enters or exits a node.
 * @typedef {Object} VisitTraversalStep
 * @property {1} kind Discriminant marking this as a visit step.
 * @property {number} phase `1` when entering the node, `2` when exiting it.
 * @property {ASTNode} target The node being entered or exited.
 * @property {Array<any>} [args] The arguments to hand the visitor functions.
 */

/**
 * A step that calls a named method on the visitor directly.
 * @typedef {Object} CallTraversalStep
 * @property {2} kind Discriminant marking this as a call step.
 * @property {string} target The name of the method to call.
 * @property {Array<any>} args The arguments to hand the method.
 */

/** @typedef {VisitTraversalStep | CallTraversalStep} TraversalStep */

/**
 * The slice of a source code object this traverser reads.
 * @typedef {Object} TraversableSourceCode
 * @property {ESQueryOptions["visitorKeys"]} [visitorKeys] The visitor keys for this file's AST.
 * @property {() => Iterable<TraversalStep>} traverse Produces the steps to walk the file.
 */
...
class ESQueryHelper {
	...
	constructor(visitor, esqueryOptions) {
		 ...
		 * @type {Map<string, Array<ESQueryParsedSelector>>}
		 */
		 ...
		 * enter phase of traversal.
		 * @type {Array<ESQueryParsedSelector>}
		 */
		...
		visitor.forEachName(rawSelector => {
			...
			if (selector.nodeTypes) {
				...
				selector.nodeTypes.forEach(nodeType => {
					...
					// Sound: the branch above guarantees the entry exists.
					/** @type {Array<ESQueryParsedSelector>} */ (
						typeMap.get(nodeType)
					).push(selector);
				});
				return;
			}
	 ...
	 * @param {Array<ASTNode>} ancestry The ancestry of the node being checked.
	 ...
	 * @returns {Array<string>} An array of selectors that match the node.
	 */
...
class SourceCodeTraverser {
	 ...
	 * @type {TraversableLanguage}
	 */
	#language;

	/**
	 * Map of languages to instances of this class.
	 * @type {WeakMap<TraversableLanguage, SourceCodeTraverser>}
	 */
	static instances = new WeakMap();

	/**
	 * Creates a new instance.
	 * @param {TraversableLanguage} language The language of the source code being traversed.
	 */
	...
	/**
	 * Returns the shared instance for a language, creating it if needed.
	 * @param {TraversableLanguage} language The language of the source code being traversed.
	 * @returns {SourceCodeTraverser} The traverser for the language.
	 */
	static getInstance(language) {
		...
		// Sound: the branch above guarantees the entry exists.
		return /** @type {SourceCodeTraverser} */ (
			this.instances.get(language)
		);
	}

	/**
	 * Traverses the given source code synchronously.
	 * @param {TraversableSourceCode} sourceCode The source code to traverse.
	 * @param {SourceCodeVisitor} visitor The emitter to use for events.
	 * @param {Object} [options] Options for traversal.
	 * @param {Iterable<TraversalStep>} [options.steps] The steps to take during traversal.
	 * @returns {void}
	...
	traverseSync(sourceCode, visitor, { steps } = {}) {
		const esquery = new ESQueryHelper(visitor, {
			...
			/*
			 * `vk.getKeys` hands back a `readonly string[]`, which esquery
			 * declares as mutable but only ever iterates.
			 */
			fallback: /** @type {ESQueryOptions["fallback"]} */ (
				/** @type {unknown} */ (vk.getKeys)
			),
			...
			switch (step.kind) {
				case STEP_KIND_VISIT: {
					try {
						...
						/*
						 * A `catch` binding is `unknown` under `strict`. Rules
						 * throw `Error` instances and the linter reads the
						 * `currentNode` stamped here when reporting a crash.
						 */
						/** @type {{ currentNode?: ASTNode }} */ (
							error
				...
				default:
					// `step` is `never` here, so the kind has to be read back out.
					throw new Error(
						`Invalid traversal step found: "${/** @type {{ kind: unknown }} */ (step).kind}".`,
					);
			}
		}
	}
