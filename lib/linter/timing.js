...
/**
 * Total time in milliseconds spent inside each rule, keyed by rule ID.
 ...
 * display the data
 *
 * ESCAPE HATCH: `rows` is `any[][]`. Reason: each row starts life as
 * `[ruleId, milliseconds]` and is then mutated in place into
 * `[ruleId, formattedTime, formattedPercentage]` — the element at index 1
 * changes from `number` to `string`. A tuple type describes one of those two
 * shapes but not the transition between them, and typing the union would make
 * both the arithmetic and the formatting illegal.
