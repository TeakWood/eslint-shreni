...
function hasArgumentsWithAccessors(node) {
	return node.arguments
		.filter((/** @type {ASTNode} */ arg) => arg.type === "ObjectExpression")
		.some(hasAccessors);
}
 ...
 * @param {ASTNode} node The node that the rule warns on
 * @param {SourceCode} sourceCode in context sourcecode object
 ...
 * @returns {Array<Token>} The parenthesis tokens of the node. This is sorted by the location.
 */
function getParenTokens(node, leftArgumentListParen, sourceCode) {
	// Every argument node spans at least one token, so neither lookup can miss.
	const parens = [
 ...
 * @param {ASTNode} node The node that the rule warns on, i.e. the Object.assign call
 * @param {SourceCode} sourceCode sourceCode of the Object.assign call
 * @returns {FixFunction} autofixer - replaces the Object.assign with a spread object.
 */
function defineFixer(node, sourceCode) {
	/**
	 * Rewrites the `Object.assign` call into an object literal, one edit at a
	 * time. This satisfies `FixFunction` because a generator's result is an
	 * `Iterable<EditInfo>`.
	 * @param {RuleFixer} fixer The fixer.
	 * @returns {Generator<EditInfo>} The edits that perform the rewrite.
	 */
	return function* (fixer) {
		/*
		 * The node is a call, so its argument list has both parentheses: the
		 * `(` after the callee and the `)` that is the call's last token.
		 */
		const leftParen = /** @type {Token} */ (
			sourceCode.getTokenAfter(node.callee, isOpeningParenToken)
		);
		const rightParen = /** @type {Token} */ (sourceCode.getLastToken(node));
		...
		for (const argNode of node.arguments) {
			...
			/*
			 * `getParenTokens()` always starts from the argument's first and
			 * last tokens, so the array it returns holds at least two entries
			 * and neither `shift()` nor `pop()` can come up empty.
			 */
			const left = /** @type {Token} */ (innerParens.shift());
			const right = /** @type {Token} */ (innerParens.pop());

			if (argNode.type === "ObjectExpression") {
				/*
				 * An object literal spans at least `{` and `}`, so skipping one
				 * token back from its last one still lands on a token; and the
				 * argument is inside a call, so the `)` of the argument list
				 * always follows it.
				 */
				const maybeTrailingComma = /** @type {Token} */ (
					sourceCode.getLastToken(argNode, 1)
				);
				const maybeArgumentComma = /** @type {Token} */ (
					sourceCode.getTokenAfter(right)
				);
				...
				/*
				 * Both are passed to `fixer.removeRange()`, which takes a
				 * two-element tuple; a hoisted array literal would widen to
				 * `number[]`.
				 */
				const leftRange = /** @type {Range} */ ([
					left.range[0],
					getEndWithSpaces(left, sourceCode),
				]);
				const rightRange = /** @type {Range} */ ([
					Math.max(
...
module.exports = {
	...
	create(context) {
		...
		return {
			/**
			 * Checks every `Object.assign()` call in the file.
