...
module.exports = {
	...
	create(context) {
		...
		function report(node, expectedOperator) {
			...
			if (isTypeOfBinary(node) || areLiteralsAndSameType(node)) {
				context.report({
					...commonReportParams,
					/**
					 * Replaces the operator with the expected one.
					 * @param {RuleFixer} fixer The fixer to use.
					 * @returns {EditInfo} The edit that replaces the operator.
					 */
					fix(fixer) {
		...
		return {
			/**
			 * Checks a binary expression for a loose equality operator.
			 * @param {ASTNode} node The binary expression node.
			 * @returns {void}
			 */
