...
module.exports = {
	...
	create(context) {
		...
		function report(node) {
			// A `for` statement's body is always preceded by the `)` that closes its header.
			const closingParen = /** @type {Token} */ (
		 ...
		 * @returns {Array<ASTNode>} An array of modifying expressions.
		 */
		function getModifyingExpressions(node, counter) {
			if (node.type === "SequenceExpression") {
				return node.expressions.flatMap((/** @type {ASTNode} */ expr) =>
		...
		return {
			/**
			 * Checks that the update clause moves the counter towards the test.
