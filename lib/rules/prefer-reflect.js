...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {ASTNode} node The node that violates the rule.
		...
		return {
			/**
			 * Checks a call expression for a method that `Reflect` replaces.
			...
			CallExpression(node) {
				...
				if (
					...
					/*
					 * `hasReflectSubstitute` is an `Object.hasOwn()` check against
					 * `reflectSubstitutes`, and `existingNames` declares exactly the
					 * same keys, so both lookups are guaranteed to hit.
					 */
					const name =
						/** @type {keyof typeof reflectSubstitutes} */ (
							methodName
						);

					report(node, existingNames[name], reflectSubstitutes[name]);
				}
			},
			/**
			 * Checks a unary expression for a `delete` that `Reflect.deleteProperty` replaces.
