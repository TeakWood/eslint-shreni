...
module.exports = {
	...
	create(context) {
		...
		return {
			/**
			 * Reports method chains that are deeper than the configured depth
			 * and are not broken onto separate lines.
			...
			"CallExpression:exit"(node) {
				...
				if (
					...
					/*
					 * The object of a member expression is always followed by
					 * the `.`, `?.` or `[` that introduces the property, so a
					 * token that is not a closing paren always exists here.
					 */
					const firstTokenAfterObject = /** @type {Token} */ (
					...
					context.report({
						...
						/**
						 * Inserts a line break before the property access.
