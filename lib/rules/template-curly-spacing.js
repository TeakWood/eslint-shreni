...
module.exports = {
	...
	create(context) {
		...
		function checkSpacingBefore(token) {
			...
			/*
			 * The token starts with `}`, so it closes an embedded expression and
			 * is therefore never the first token of the program.
			 */
		...
		function checkSpacingAfter(token) {
			...
			/*
			 * The token ends with `${`, so an embedded expression follows it and
			 * it is therefore never the last token of the program.
			 */
		...
		return {
			/**
			 * Checks the spacing around the given template element.
			 * @param {ASTNode} node The `TemplateElement` node to check.
			 * @returns {void}
			 */
			TemplateElement(node) {
				// A template element is always made up of exactly one token.
