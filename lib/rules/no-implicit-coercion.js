 ...
 * @param {ASTNode} node The BinaryExpression node to be walked up on
...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {boolean} shouldSuggest Whether this report should offer a suggestion
		 * @param {boolean} shouldFix Whether this report should fix the node
		 * @returns {void}
		 */
		...
		return {
			/**
			 * Checks a unary expression for `!!foo`, `~foo.indexOf(bar)`, `+foo` and `-(-foo)`.
			 * @param {ASTNode} node The `UnaryExpression` to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks a binary expression for `1 * foo`, `foo - 0` and `"" + foo`.
			 * Uses `:exit` to prevent double reporting.
			 * @param {ASTNode} node The `BinaryExpression` to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks an assignment expression for `foo += ""`.
			 * @param {ASTNode} node The `AssignmentExpression` to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks a template literal for the `` `${foo}` `` string shorthand.
			 * @param {ASTNode} node The `TemplateLiteral` to check.
			 * @returns {void}
			 */
