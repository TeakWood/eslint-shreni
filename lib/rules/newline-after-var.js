...
module.exports = {
	...
	create(context) {
		...
		function getLastToken(node) {
			// A `VariableDeclaration` always has a last token, and a `;` last token always has a token before it.
		 ...
		 * @param {number} commentStartLine The line number on which the comment starts
		...
		function checkForBlankLine(node) {
			...
			if (mode === "never" && noNextLineToken && !hasNextLineComment) {
				context.report({
					...
					/**
					 * Removes the blank line between the declaration and the next token.
			...
			if (
				...
				context.report({
					...
					messageId: "expected",
					/**
					 * Inserts the blank line after the declaration.
					 * @param {RuleFixer} fixer The fixer to build the edit with.
					 * @returns {EditInfo} The edit.
					 */
					fix(fixer) {
