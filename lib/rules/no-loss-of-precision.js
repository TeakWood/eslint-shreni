...
class ScientificNotation {
	...
	/**
	 * Creates a number in scientific notation.
	 * @param {string} coefficient The digits of the coefficient.
	...
	/**
	 * Renders the number in `<d>.<ddd>e<n>` form.
	 * @returns {string} The scientific notation as a string.
	 */
	toString() {
 ...
 * @param {ASTNode} node the number `Literal` node
...
module.exports = {
	 ...
	 * @returns {RuleVisitor} The visitor that reports imprecise number literals.
	 */
	create(context) {
		return {
			/**
			 * Reports a number literal whose value cannot be stored exactly.
			 * @param {ASTNode} node The `Literal` node being visited.
			 * @returns {void}
			 */
			Literal(node) {
