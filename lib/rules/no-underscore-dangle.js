	...
	create(context) {
		...
		function checkForDanglingUnderscoreInFunctionParameters(node) {
			if (!allowFunctionParams) {
				node.params.forEach((/** @type {ASTNode} */ param) => {
					const { type } = param;
					let nodeToCheck;
		...
		function checkForDanglingUnderscoreInVariableExpression(node) {
			sourceCode.getDeclaredVariables(node).forEach(variable => {
				/*
				 * `variable` was declared by `node`, so one of its definitions
				 * necessarily came from `node`. `Definition["node"]` and
				 * `Definition["name"]` are bare ESTree nodes — no `parent`, and
				 * `range` and `loc` optional — while `node` is the rules-layer
				 * `ASTNode`. They describe the same objects at runtime but have
				 * no declared overlap, so both hops go through `unknown`.
				 */
				const definition = /** @type {Definition} */ (
					variable.defs.find(
						def => /** @type {unknown} */ (def.node) === node,
					)
				);
				const identifierNode = /** @type {ASTNode} */ (
					/** @type {unknown} */ (definition.name)
				);
