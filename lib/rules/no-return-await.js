...
module.exports = {
	...
	create(context) {
		...
		function reportUnnecessaryAwait(node) {
			context.report({
				// An `AwaitExpression` always begins with its `await` keyword token.
				node: /** @type {Token} */ (
				...
				suggest: [
					{
						...
						/**
						 * Removes the `await` keyword and the space that follows it.
						 * @param {RuleFixer} fixer The fixer to apply the change with.
						 * @returns {EditInfo | null} The fix, or `null` if the `await` spans a line break.
						 */
						fix(fixer) {
		...
		return {
			/**
			 * Reports an `await` in tail position, where it has no effect.
			 * @param {ASTNode} node The `AwaitExpression` node.
			 * @returns {void}
			 */
