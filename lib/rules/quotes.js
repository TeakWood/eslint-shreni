...
/**
 * One of the three quote styles this rule can enforce, together with the
 * conversion it performs when it fixes a string literal into that style.
 * `convert` is called as a method so that it can read `quote` off the setting
 * it belongs to, which is why it declares its own `this`.
 * @typedef {Object} QuoteSetting
 * @property {string} quote The quote character this style uses.
 * @property {string} alternateQuote The quote character allowed under `avoidEscape` when the string contains `quote`.
 * @property {string} description The name of the style as it appears in the report message.
 * @property {(this: QuoteSetting, str: string) => string} convert Rewrites a quoted string's raw text into this style.
 */
...
/*
 * `convert` is not in the literal because all three styles share one
 * implementation, which is attached to each of them immediately below. The
 * assertion records that two-step construction: the settings are complete
 * before this module finishes loading, so no rule can observe them without it.
 */
const QUOTE_SETTINGS =
	/** @type {Record<"double" | "single" | "backtick", QuoteSetting>} */ ({
		double: {
			quote: '"',
			alternateQuote: "'",
...
module.exports = {
	...
	create(context) {
		const quoteOption = context.options[0],
			// `meta.schema` restricts the first option to one of the three style names.
			settings =
				QUOTE_SETTINGS[
					/** @type {"double" | "single" | "backtick"} */ (
						quoteOption || "double"
					)
				],
		...
		return {
			/**
			 * Checks that a string literal uses the configured quote style.
				...
				if (settings && typeof val === "string") {
					...
					if (!isValid) {
						context.report({
							...
							/**
							 * Requotes the string literal into the configured style.
							 * @param {RuleFixer} fixer The fixer to use.
							 * @returns {EditInfo | null} The fix, or `null` when requoting would not be safe.
							 */
							fix(fixer) {
								if (
									quoteOption === "backtick" &&
			...
			/**
			 * Checks that a template literal is only used where backticks are
			 * expected or where a template literal feature is in use.
			...
			TemplateLiteral(node) {
				...
				context.report({
					...
					/**
					 * Requotes the template literal into the configured style.
					 * @param {RuleFixer} fixer The fixer to use.
					 * @returns {EditInfo | null} The fix, or `null` when requoting would change the code's behavior.
					 */
					fix(fixer) {
						if (
