...
/**
 * An `eslint-scope` variable, together with the `eslintUsed` flag the linter
 * attaches to it after scope analysis has run. It is set by
 * `markVariableAsUsed()` and by `exported` directive comments, and is absent
 * on variables nothing marked.
 ...
 * `Definition#name` here — as bare ESTree nodes: no `parent`, and `range` and
 * `loc` optional. The linter populates all three before any rule runs, so each
 * of these is the same object a visitor would have received, and is
...
function getDestructuringHost(reference) {
	...
	let node = asNode(reference.identifier).parent;
 ...
 * @returns {ASTNode | null} An Identifier node if the variable should change
 *      to const. Otherwise, null.
 */
function getIdentifierIfShouldBeConst(variable, ignoreReadBeforeAssign) {
			...
			if (
				...
				if (leftNode.type === "ObjectPattern") {
					...
					hasOuterVariables = properties
						.filter((/** @type {ASTNode} */ prop) => prop.value)
						.map((/** @type {ASTNode} */ prop) => prop.value.name)
						.some((/** @type {string} */ name) =>
	...
	const shouldBeConst =
		...
		canBecomeVariableDeclaration(asNode(writer.identifier));
	...
	if (isReadBeforeInit) {
		return asNode(variable.defs[0].name);
	}

	// `shouldBeConst` is only `true` when `writer` is non-null.
	return asNode(/** @type {Reference} */ (writer).identifier);
}
 ...
 * This is used to detect a mix of reassigned and never reassigned in a
 * destructuring.
 * @param {Array<Variable>} variables Variables to group by destructuring.
 * @param {boolean} ignoreReadBeforeAssign
 *      The value of `ignoreReadBeforeAssign` option.
 * @returns {Map<ASTNode, Array<ASTNode | null>>} Grouped identifier nodes.
 */
function groupByDestructuring(variables, ignoreReadBeforeAssign) {
	/** @type {Map<ASTNode, Array<ASTNode | null>>} */
		...
		for (let j = 0; j < references.length; ++j) {
			...
			if (group) {
				if (identifierMap.has(group)) {
					// Guarded by the `has()` call above.
					/** @type {Array<ASTNode | null>} */ (
						identifierMap.get(group)
					).push(identifier);
				} else {
 ...
 * @param {ASTNode | null} node The node to search from.
 * @param {string} type The type field of the parent node.
 * @param {(node: ASTNode) => boolean} shouldStop A predicate that returns true if the traversal should stop, and false otherwise.
 * @returns {ASTNode | null} The closest ancestor with the specified type; null if no such ancestor exists.
 */
...
module.exports = {
	...
	create(context) {
		 ...
		 * be 2 or more.
		 * @param {Array<ASTNode | null>} nodes
		 *      References which are grouped by destructuring to report.
		 * @returns {void}
		 */
		function checkGroup(nodes) {
			// `filter(Boolean)` drops the nullable entries described above.
			const nodesToReport = /** @type {Array<ASTNode>} */ (
			...
			if (
				...
				if (
					...
					if (
						...
						varDeclParent.declarations.forEach(
							(/** @type {ASTNode} */ declaration) => {
				...
				nodesToReport.forEach(node => {
					context.report({
						...
						fix: shouldFix
							? fixer => {
									/*
									 * `shouldFix` is only truthy when `varDeclParent`
									 * is non-null, and the declaration always has the
									 * keyword token its own `kind` names.
									 */
		...
		return {
			/**
			 * Reports every collected group once the whole file is analyzed.
			 * @returns {void}
			 */
			"Program:exit"() {
			...
			/**
			 * Collects the variables a `let` declaration declares.
