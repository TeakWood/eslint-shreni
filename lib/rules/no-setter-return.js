...
/**
 * The per-function state this rule stacks as it walks the file. One entry is
 * pushed for every function node, and `upper` links it to the entry for the
 * enclosing function so that `:exit` can pop back to it.
 * @typedef {Object} FuncInfo
 * @property {FuncInfo | null} upper The entry for the enclosing function, or `null` at the top level.
 * @property {boolean} isSetter Whether this function is used as a setter.
 */
...
module.exports = {
	...
	create(context) {
		...
		function exitFunction() {
			 ...
			 * called `enterFunction`, so the stack always holds an entry here.
			 */
		...
		return {
			...
			/**
			 * Tracks an arrow function and reports its implicit return, if any.
			...
			ArrowFunctionExpression(node) {
				...
				// `enterFunction()` has just pushed this function's entry.
				const currentFuncInfo = /** @type {FuncInfo} */ (funcInfo);

				if (currentFuncInfo.isSetter && node.expression) {
			...
			/**
			 * Reports a `return` statement with an argument inside a setter.
