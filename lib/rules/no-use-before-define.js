...
/**
 * The rule's options, after the `"nofunc"` shorthand and the bare-object form
 * have both been folded into one shape. Every member is present once
 * `parseOptions()` has run, because `meta.defaultOptions` supplies the object
 * form and the shorthand is expanded by hand.
 * @typedef {Object} UseBeforeDefineOptions
 * @property {boolean} functions Whether to report references to functions declared later.
 * @property {boolean} classes Whether to report references to classes declared later.
 * @property {boolean} variables Whether to report references to variables declared later.
 * @property {boolean} allowNamedExports Whether to allow a named export to name a binding declared later.
 * @property {boolean} enums Whether to report references to TypeScript enums declared later.
 * @property {boolean} typedefs Whether to report references to TypeScript type aliases declared later.
 * @property {boolean} ignoreTypeReferences Whether to skip references that appear in a type position.
 */
 ...
 * `Definition#name`, `Definition#node` and `Scope#block` — as bare ESTree
 * nodes: no `parent`, and `range` and `loc` optional. The linter populates all
 * three before any rule runs, so each of these is the same object a visitor
 * would have received and is reinterpreted rather than re-checked.
 ...
 * @returns {UseBeforeDefineOptions} The parsed options.
 */
 ...
 * `VariableDeclarator#init`, which may be `null` — hence the leading `node &&`
 * guard, which the parameter type deliberately does not restate.
...
function isInClassStaticInitializerRange(node, location) {
	return /** @type {Array<ASTNode>} */ (node.body).some(
		classMember =>
...
function isClassStaticInitializerScope(scope) {
	...
	if (scope.type === "class-field-initializer") {
		...
		const propertyDefinition = asNode(scope.block).parent;
...
function isFromSeparateExecutionContext(reference) {
	// Only ever reached for references `shouldCheck()` has already resolved.
	...
	while (variable.scope.variableScope !== scope.variableScope) {
		if (isClassStaticInitializerScope(scope.variableScope)) {
			// A class static initializer scope always sits inside a class scope.
			scope = /** @type {Scope} */ (scope.variableScope.upper);
		} else {
			return true;
		}
	}
...
function isEvaluatedDuringInitialization(reference) {
	...
	const location = asNode(reference.identifier).range[1];
	const definition = /** @type {Variable} */ (reference.resolved).defs[0];

	if (definition.type === "ClassName") {
		// `ClassDeclaration` or `ClassExpression`
		const classDefinition = asNode(definition.node);
	...
	let node = asNode(definition.name).parent;
...
function isClassRefInClassDecorator(variable, reference) {
	...
	/*
	 * `decorators` is not part of ESTree, so it is only reachable through the
	 * rules-layer node shape.
	 */
	if (
		!asNode(variable.defs[0].node).decorators ||
		asNode(variable.defs[0].node).decorators.length === 0
	) {
		return false;
	}

	for (const deco of asNode(variable.defs[0].node).decorators) {
		if (
			asNode(reference.identifier).range[0] >= deco.range[0] &&
			asNode(reference.identifier).range[1] <= deco.range[1]
		) {
			return true;
		}
	}
...
module.exports = {
	...
	create(context) {
		...
		function shouldCheck(reference) {
			...
			/*
			 * `eslint-scope` types `Definition#type` as the closed set of
			 * definition kinds it creates itself; the TypeScript-only kinds
			 * below are contributed by `typescript-eslint`'s scope analyser.
			 */
			const definitionType = /** @type {string} */ (
		...
		function checkReferencesInScope(scope) {
			scope.references.filter(shouldCheck).forEach(reference => {
				...
				// `shouldCheck()` has already rejected unresolved references.
				const variable = /** @type {Variable} */ (reference.resolved);
				const definitionIdentifier = asNode(variable.defs[0].name);

				if (
					identifier.range[1] < definitionIdentifier.range[1] ||
		...
		return {
			/**
			 * Checks every reference in the program.
