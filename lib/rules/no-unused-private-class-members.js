...
/**
 * The bookkeeping this rule keeps for one declared private class member.
 * `isUsed` is absent until a reference proves the member is read, so that a
 * nested class can tell "never referenced" from "referenced but write-only".
 * @typedef {Object} TrackedMember
 * @property {ASTNode} declaredNode The `PropertyDefinition` or `MethodDefinition` that declares the member.
 * @property {boolean} hasReference Whether the member is referenced anywhere, even write-only.
 * @property {boolean} isAccessor Whether the member is a getter or a setter.
 * @property {boolean} [isUsed] Whether a reference to the member counts as a use.
 */
...
module.exports = {
	...
	create(context) {
		...
		/** @type {Array<Map<string, TrackedMember>>} */
		...
		function startsOnOwnLine(nodeOrToken) {
			...
			return tokenBefore.loc.end.line !== nodeOrToken.loc.start.line;
		}

		/**
		 * Gets leading comments that are directly attached to a class member.
		 * @param {ASTNode} classMemberNode The class member node
		 * @returns {Array<Comment>} Leading comments to remove with the member
		 */
		...
		function sharesLineWithAnotherToken(classMemberNode) {
			const previousToken = /** @type {Token} */ (
				sourceCode.getTokenBefore(classMemberNode)
			);
			const nextToken = /** @type {Token} */ (
				sourceCode.getTokenAfter(classMemberNode)
			);
		 ...
		 * @param {ASTNode} classMemberNode The class member node
		 * @returns {Array<Comment>} Trailing comments to remove with the member
		 */
		 ...
		 * @param {ASTNode} classMemberNode The member that would be removed
		 * @returns {Range} The text range to remove
		 */
		function getMemberRemovalRange(classMemberNode) {
			...
			const lastItemToRemove =
				trailingComments.length > 0
					? /** @type {Comment} */ (trailingComments.at(-1))
					: classMemberNode;
			...
			const nextToken = /** @type {Token} */ (
				sourceCode.getTokenAfter(lastItemToRemove, {
					includeComments: true,
				})
			);
		...
		return {
			/**
			 * Collects all declared members up front and assumes they are all unused.
			 * @param {ASTNode} classBodyNode The `ClassBody` node.
			 * @returns {void}
			 */
			ClassBody(classBodyNode) {
				/** @type {Map<string, TrackedMember>} */
			...
			/**
			 * Processes all usages of the private identifier and removes a member from
			 * `declaredAndUnusedPrivateMembers` if we deem it used.
			 * @param {ASTNode} privateIdentifierNode The `PrivateIdentifier` node.
			 * @returns {void}
			 */
			PrivateIdentifier(privateIdentifierNode) {
				...
				const memberDefinition = /** @type {TrackedMember} */ (
					classBody.get(privateIdentifierNode.name)
				);
			...
			/**
			 * Post-processes the class members and reports any remaining members.
			 ...
			 */
			"ClassBody:exit"() {
				const unusedPrivateMembers =
					/** @type {Map<string, TrackedMember>} */ (
						trackedClasses.shift()
					);
