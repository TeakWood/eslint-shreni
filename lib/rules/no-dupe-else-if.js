 ...
 * @param {(a: ASTNode, b: ASTNode) => boolean} comparator A function to compare two elements, should return `true` if they are equal.
 * @param {Array<ASTNode>} arrA The array to compare from.
 * @param {Array<ASTNode>} arrB The array to compare against.
 ...
 * @returns {Array<ASTNode>} Array of conditions that makes the node when joined by the operator.
 */
...
/**
 * Splits the given node by the `||` operator.
 * @type {(node: ASTNode) => Array<ASTNode>}
 */
...
module.exports = {
	...
	create(context) {
		...
		/**
		 * Determines whether the first given array of conditions is a subset of the second one.
		 * @type {(arrA: Array<ASTNode>, arrB: Array<ASTNode>) => boolean}
		 */
		const isSubset = isSubsetByComparator.bind(null, equal);

		return {
			/**
			 * Checks an `if` statement for a condition that is already covered by
			 * an earlier condition in the same if-else-if chain.
