...
/** @typedef {import("espree").EspreeToken} EspreeToken */
...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {Array<EspreeToken>} tokens The espree-tokenized node key
			...
			if (key.type === "Literal" && typeof key.value === "string") {
				...
				if (
					...
					context.report({
						...
						/**
						 * Strips the redundant quotes from the key.
				...
				context.report({
					...
					/**
					 * Adds quotes around the reserved-word key.
					 * @param {RuleFixer} fixer The fixer to use.
					 * @returns {EditInfo} The fix.
					 */
				...
				context.report({
					...
					data: { property: key.value },
					/**
					 * Adds quotes around the numeric key.
					 * @param {RuleFixer} fixer The fixer to use.
					 * @returns {EditInfo} The fix.
					 */
					fix: fixer => fixer.replaceText(key, getQuotedKey(key)),
		...
		function checkOmittedQuotes(node) {
			...
			if (
				...
				context.report({
					...
					/**
					 * Adds quotes around the unquoted key.
					 * @param {RuleFixer} fixer The fixer to use.
					 * @returns {EditInfo} The fix.
					 */
					fix: fixer => fixer.replaceText(key, getQuotedKey(key)),
		...
		function checkConsistency(node, checkQuotesRedundancy) {
			...
			// `properties` reaches this rule through `ASTNode`'s index signature, so the element type has to be restated here.
			...
			if (
				...
				quotedProps.forEach(property => {
					context.report({
						...
						/**
						 * Strips the redundant quotes from the property's key.
						 * @param {RuleFixer} fixer The fixer to use.
						 * @returns {EditInfo} The fix.
						 */
						fix: fixer =>
				...
				unquotedProps.forEach(property => {
					context.report({
						...
						/**
						 * Adds quotes around the property's key.
						 * @param {RuleFixer} fixer The fixer to use.
						 * @returns {EditInfo} The fix.
						 */
						fix: fixer =>
		...
		return {
			/**
			 * Checks a single property's key against the configured mode.
			...
			/**
			 * Checks an object literal's keys for consistent quoting.
