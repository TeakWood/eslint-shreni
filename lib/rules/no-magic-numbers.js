...
/**
 * The rule's option object once `meta.defaultOptions` has filled in every member.
 * @typedef {Object} NormalizedOptions
 * @property {boolean} detectObjects Whether numbers used in object properties are reported too.
 * @property {boolean} enforceConst Whether numbers assigned to a non-`const` declaration are reported.
 * @property {Array<number | string>} ignore The values never reported, bigints written as strings such as `"1n"`.
 * @property {boolean} ignoreArrayIndexes Whether numbers used as array indexes are allowed.
 * @property {boolean} ignoreDefaultValues Whether numbers used as default values are allowed.
 * @property {boolean} ignoreClassFieldInitialValues Whether numbers used as class field initializers are allowed.
 * @property {boolean} ignoreEnums Whether numbers used as TypeScript enum member values are allowed.
 * @property {boolean} ignoreNumericLiteralTypes Whether numbers used as TypeScript numeric literal types are allowed.
 * @property {boolean} ignoreReadonlyClassProperties Whether numbers used to initialize `readonly` class properties are allowed.
 * @property {boolean} ignoreTypeIndexes Whether numbers used as TypeScript indexed access types are allowed.
 */
 ...
 * @param {number | string} x The value to normalize.
 * @returns {number | bigint} The normalized value.
 */
...
module.exports = {
	 ...
	 */
	create(context) {
		// `meta.schema` validated the shape and `meta.defaultOptions` supplied every member.
		const options = /** @type {NormalizedOptions} */ (context.options[0]);
		const {
			detectObjects,
			enforceConst,
			ignore: rawIgnore,
		 ...
		 * @param {ASTNode} fullNumberNode `Literal` or `UnaryExpression` full number node
		 * @param {number | bigint} value Value expressed by the fullNumberNode
		...
		return {
			/**
			 * Reports a numeric literal that is not allowed by the options.
