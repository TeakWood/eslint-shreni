...
module.exports = {
	...
	create(context) {
		...
		return {
			/**
			 * Checks a regular expression literal for the required Unicode flag.
			 * @param {ASTNode} node The `Literal` node with a `regex` property.
			 * @returns {void}
			 */
			"Literal[regex]"(node) {
					...
					context.report({
						...
						suggest: isValidWithUnicodeFlag(
							...
							? [
									{
										/**
										 * Adds the required flag to the regular expression literal.
							...
							suggest:
								...
								isValidWithUnicodeFlag(
									...
									? [
											{
												/**
												 * Adds the required flag to the `RegExp()` call's flags argument.
												 * @param {RuleFixer} fixer The fixer to use.
												 * @returns {EditInfo | null} The fix, or `null` when there is no safe edit.
												 */
												fix(fixer) {
													const replaceFlag =
														requireFlag ?? "u";
													...
													if (flagsNode) {
														if (
															...
															/*
															 * A call with a `flagsNode` is only reported when
															 * `getStringIfConstant()` returned a string for it, so
															 * `flags` is never `null` on this path.
															 */
															if (
																/** @type {string} */ (
																	flags
																...
																if (
																	...
																	(flagsNode
																		.expressions
																		.length ||
																		// A `TemplateLiteral`'s `quasis` are always `TemplateElement` nodes.
																		/** @type {Array<ASTNode>} */ (
																			flagsNode.quasis
																		).some(
																			({
													...
													/*
													 * `refNode` is a `RegExp(…)` call or `new RegExp(…)`, so it
													 * always ends in `(` `)` and therefore always has a token
													...
													const penultimateToken =
														/** @type {Token} */ (
															sourceCode.getLastToken(
																refNode,
																{ skip: 1 }, // skip closing parenthesis
															)
														);
