...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {ASTNode} node node to evaluate
		 * @returns {ASTNode | null} node that is the function expression of the given IIFE, or null if none exist
		 */
		...
		return {
			/**
			 * Checks that an IIFE is wrapped in parens in the configured style.
			...
			CallExpression(node) {
				...
				if (!isCallExpressionWrapped && !isFunctionExpressionWrapped) {
					context.report({
						...
						/**
						 * Wraps the configured node in parens.
					...
					context.report({
						...
						/**
						 * Moves the parens so that they wrap only the function expression.
						 * @param {RuleFixer} fixer The fixer to use.
						 * @returns {EditInfo} The fix.
						 */
						fix(fixer) {
							...
							if (
								...
								/*
								 * `isWrappedInGroupingParens()` just reported that
								 * the call expression is followed by a close paren.
								 */
								const parenAfter = /** @type {Token} */ (
					...
					context.report({
						...
						/**
						 * Moves the invocation inside the parens that wrap the function expression.
						 * @param {RuleFixer} fixer The fixer to use.
						 * @returns {EditInfo} The fix.
						 */
						fix(fixer) {
