...
/**
 * Casts an `eslint-scope` node to the rules-layer `ASTNode` shape.
 *
 * `eslint-scope` types `Reference#identifier`, `Definition#name` and
 * `Definition#node` as bare ESTree nodes: no `parent`, and `range` and `loc`
 * optional. The nodes it stores are the very ones the parser produced, so they
 * carry all three, and the linter has filled them in before any rule runs.
...
function getEnclosingFunctionScope(scope) {
	...
	while (currentScope.type !== "function" && currentScope.type !== "global") {
		// The global scope has no `upper`, and the loop stops there.
		currentScope = /** @type {Scope} */ (currentScope.upper);
	}
	return currentScope;
}
...
function isDeclarationInitialized(node) {
	return node.declarations.every(
		(/** @type {ASTNode} */ declarator) => declarator.init !== null,
	);
}
 ...
 *      an `Identifier`.
 * @returns {ASTNode | null} A scope node. This is one of `Program`, `BlockStatement`,
 ...
 * @returns {(variable: Variable) => boolean} The predicate function which checks whether a given
...
function isUsedFromOutsideOf(scopeNode) {
	...
	function isOutsideOfScope(reference) {
		...
		const id = asNode(reference.identifier).range;
 ...
 * @returns {(variable: Variable) => boolean} The predicate function.
 * @private
 */
function hasReferenceInTDZ(node) {
	...
	return variable => {
		const id = asNode(variable.defs[0].name);
...
function hasReferenceBeforeDeclaration(variable) {
	const declarationStart = asNode(variable.defs[0].node).range[0];

	return variable.references.some(reference => {
		const referenceStart = asNode(reference.identifier).range[0];
...
module.exports = {
	...
	create(context) {
		...
		function canFix(node) {
			...
			// Every node is inside a `Program`, which is a scope node.
			const scopeNode = /** @type {ASTNode} */ (getScopeNode(node));
		...
		function report(node) {
			context.report({
				...
				fix(fixer) {
					// A `var` declaration always has a `var` keyword token.
					const varToken = /** @type {Token} */ (
