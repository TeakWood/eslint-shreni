...
/** @typedef {import("./utils/lazy-loading-rule-map.js").RuleLoaderEntry} RuleLoaderEntry */

/*
 * Every rule module is a plain object literal, so its `meta.type` widens to
 * `string` instead of the literal union `RuleModule` declares. An annotated
 * rule also declares `create(context)` against the rules-layer `RuleContext`,
 * which is deliberately sharper than the one `RuleModule` names. The entries
 * are therefore widened to the loader shape here rather than at each of the
 * ~290 call sites, and the hop through `unknown` is what lets that widening
 * cross those two differences.
 */
const ruleLoaders = /** @type {Array<RuleLoaderEntry>} */ (
	/** @type {unknown} */ (
		Object.entries({
			...
			"multiline-comment-style": () =>
				require("./multiline-comment-style"),
			...
			"no-multiple-empty-lines": () =>
				require("./no-multiple-empty-lines"),
			...
			"no-restricted-properties": () =>
				require("./no-restricted-properties"),
			...
			"no-unexpected-multiline": () =>
				require("./no-unexpected-multiline"),
			...
			"object-property-newline": () =>
				require("./object-property-newline"),
...
module.exports = new LazyLoadingRuleMap(ruleLoaders);
