 ...
 * @returns {boolean | number} A truthy value if the node will start with a template
 * curly. The `TemplateLiteral` branch short-circuits on `expressions.length`, so it
 * yields `0` rather than `false` for a literal with no placeholders; every caller
 * reads the result in a boolean position.
 */
 ...
 * @param {ASTNode} node The node that will be fixed to a template literal
 * @returns {boolean | number} A truthy value if the node will end with a template
 * curly. The `TemplateLiteral` branch short-circuits on `expressions.length`, so it
 * yields `0` rather than `false` for a literal with no placeholders; every caller
 * reads the result in a boolean position.
 */
...
module.exports = {
	...
	create(context) {
		...
		/**
		 * The start offsets of the top-level concatenations already reported, so
		 * that each one is reported once however many string literals it holds.
		 ...
		 * @param {ASTNode | Token} node1 The first node
		 * @param {ASTNode | Token} node2 The second node
		 ...
		 * @param {string | null} textBeforeNode Text that should appear before the node
		 * @param {string | null} textAfterNode Text that should appear after the node
		...
		function getTemplateLiteral(
			...
			if (isConcatenation(currentNode) && hasStringLiteral(currentNode)) {
				/*
				 * `isConcatenation()` already established that `currentNode` is a
				 * `+` binary expression, so a `+` token exists between its operands.
				 */
				const plusSign = /** @type {Token} */ (
		 ...
		 * @returns {EditInfo | null} A fix for this binary expression, or `null` if
		 * converting it would change the meaning of an escape sequence
		 */
		...
		return {
			/**
			 * Resets the per-file record of already reported concatenations.
			 * @returns {void}
			 */
			Program() {
