...
module.exports = {
	...
	create(context) {
		...
		function isParenthesized(node) {
			 ...
			 */
			return eslintUtils.isParenthesized(
				1,
			...
			switch (parent.type) {
				...
				case "ConditionalExpression":
					...
					if (
						...
						/*
						 * `getPrecedence()` only reads `.type`, so a bare node
						 * shape is enough to ask for assignment-level precedence.
						 */
						const assignmentNode = /** @type {ASTNode} */ ({
							type: "AssignmentExpression",
						});

						return precedence(node) < precedence(assignmentNode);
					}
		...
		return {
			/**
			 * Reports a `!!` double negation that sits in a boolean context.
			...
			UnaryExpression(node) {
				...
				if (isInFlaggedContext(parent)) {
					context.report({
						...
						/**
						 * Replaces `!!expr` with `expr`, parenthesizing it if the
						 * surrounding precedence requires it.
						 * @param {RuleFixer} fixer The fixer to build the edit with.
						 * @returns {EditInfo | null} The fix, or `null` if it is unsafe.
						 */
						fix(fixer) {
							...
							// `node.argument` is an expression, so it spans at least one token.
							const firstReplacementToken = /** @type {Token} */ (
			...
			/**
			 * Reports a `Boolean(...)` call that sits in a boolean context.
			...
			CallExpression(node) {
				...
				if (isInFlaggedContext(node)) {
					context.report({
						...
						/**
						 * Unwraps the redundant `Boolean(...)` call, collapsing the
						 * zero-argument forms to the boolean they evaluate to.
						 * @param {RuleFixer} fixer The fixer to build the edit with.
						 * @returns {EditInfo | null} The fix, or `null` if it is unsafe.
						 */
						fix(fixer) {
