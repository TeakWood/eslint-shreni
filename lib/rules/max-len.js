...
/**
 * An entry in the list the rule measures comment lines against. It is either a
 * real comment token, or — for a JSX empty expression that fills a single-line
 * `JSXExpressionContainer` — the container node standing in for it.
 * @typedef {Comment | ASTNode} CommentLike
 */
	...
	create(context) {
		...
		function computeLineLength(line, tabWidth) {
			...
			line.replace(
				/\t/gu,
				/**
				 * `String#replace` declares its replacer as returning the text
				 * to substitute with, but this one runs purely for the side
				 * effect below and the resulting string is thrown away, so it
				 * returns nothing and has to be asserted into the lib type.
				 * @type {(substring: string, ...args: Array<any>) => string}
				 */
				(
					(match, offset) => {
		...
		/**
		 * The options object must be the last option specified… The values stay
		 * `any` because `meta.schema` is what validates them, and restating the
		 * schema here would only duplicate it in a form nothing keeps in sync.
		 ...
		 * @param {CommentLike} comment The comment to inspect
		 ...
		 * @param {CommentLike} comment The comment to remove
		 ...
		 * @returns {node is ASTNode} True if the node is a JSXEmptyExpression contained in a single line JSXExpressionContainer.
		 */
		 ...
		 * @param {Record<number, Array<Token>>} object the object to mutate
		 ...
		 * @returns {Array<Token>} An array of string nodes.
		 */
		function getAllStrings() {
			/*
			 * `getNodeByRangeIndex()` is only consulted for a `JSXText` token.
			 * Such a token is always nested inside a JSX element or fragment,
			 * never first in the program, so the index before it is still
			 * covered by that enclosing node and the lookup cannot be `null`.
			 */
		 ...
		 * @returns {Array<Token>} An array of template literal nodes.
		 */
		 ...
		 * @returns {Array<Token>} An array of RegExp literal nodes.
		 */
		 ...
		 * @param {Array<Token>} arr array of AST nodes
		 * @returns {Record<number, Array<Token>>} accululated AST nodes
		 */
		function groupArrayByLineNumber(arr) {
			/** @type {Record<number, Array<Token>>} */
			const obj = {};
		 ...
		 * @returns {Array<CommentLike>} An array of comment nodes
		 */
