...
/**
 * A `Literal` node, or the synthetic stand-in `getNormalizedLiteral()` builds
 * for the node shapes this rule treats as literals. The synthetic form carries
 * only what the callers read, so it is not an `ASTNode`.
 * @typedef {ASTNode | {type: "Literal", value: unknown, raw: string}} NormalizedLiteral
 */
 ...
 * @returns {NormalizedLiteral | null} One of the following options.
...
module.exports = {
	...
	create(context) {
		...
		/**
		 * Maps each comparison operator to the operator that means the same thing
		 * with the operands swapped. Indexed by a token's `value`, so it needs a
		...
		function getFlippedString(node) {
			/*
			 * A BinaryExpression always has its operator between the two operands,
			 * and each operand contributes at least one token, so all three lookups
			 * land on a token.
			 */
			...
			const lastLeftToken = /** @type {Token} */ (
				sourceCode.getTokenBefore(operatorToken)
			);
			const firstRightToken = /** @type {Token} */ (
		...
		return {
			/**
			 * Reports a comparison whose literal is on the unexpected side.
			...
			BinaryExpression(node) {
				...
				if (
					...
					context.report({
						...
						/**
						 * Flips the sides of the comparison.
