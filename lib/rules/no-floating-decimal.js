...
module.exports = {
	...
	create(context) {
		...
		return {
			/**
			 * Reports a numeric literal whose decimal point has no digit on one side.
				...
				if (typeof node.value === "number") {
					if (node.raw.startsWith(".")) {
						context.report({
							...
							/**
							 * Inserts the missing leading zero.
							 * @param {RuleFixer} fixer The fixer to build the edit with.
							 * @returns {EditInfo} The edit that adds the zero.
							 */
							fix(fixer) {
								const tokenBefore =
					...
					if (node.raw.indexOf(".") === node.raw.length - 1) {
						context.report({
							...
							/**
							 * Appends the missing trailing zero.
							 * @param {RuleFixer} fixer The fixer to build the edit with.
							 * @returns {EditInfo} The edit that adds the zero.
							 */
