 ...
 * `checkPrecedingSpace()` also passes the `{` token of a `SwitchStatement`,
 * which is why this accepts a token as well as a node.
...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {ASTNode | Token} node The AST node of a BlockStatement, or the `{` token of a `SwitchStatement`.
		 * @returns {void} undefined.
		 */
		function checkPrecedingSpace(node) {
			// Comments are only returned when `includeComments` is passed.
			const precedingToken = /** @type {Token | null} */ (
			...
			if (
				...
				// `isTokenOnSameLine()` only reads `loc`, which a node has too.
				...
				if (requireSpace && !hasSpace) {
					context.report({
						...
						/**
						 * Inserts the missing space before the block.
					...
					context.report({
						...
						/**
						 * Removes the unexpected space before the block.
						 * @param {RuleFixer} fixer The fixer to use.
						 * @returns {EditInfo} The fix.
						 */
						fix(fixer) {
		...
		function checkSpaceBeforeCaseBlock(node) {
			...
			// A `SwitchStatement` is delimited by braces, so both lookups land on a token.
			if (cases.length > 0) {
				openingBrace = /** @type {Token} */ (
