...
/**
 * The compiled patterns for one of the two comment styles this rule
 * distinguishes, `"block"` and `"line"`.
 * @typedef {Object} StyleRule
 * @property {RegExp} beginRegex Matches a valid beginning of a comment.
 * @property {RegExp} endRegex Matches a valid ending of a block comment.
 * @property {boolean} hasExceptions `true` if exception patterns were configured.
 * @property {RegExp} captureMarker Captures the marker at the beginning of a comment.
 * @property {Set<string>} markers The markers configured for this style.
 */
 ...
 * @param {Array<string>} markers A marker list.
 * @returns {Array<string>} A marker list.
 */
 ...
 * @param {Array<string>} exceptions An exception pattern list.
...
module.exports = {
	...
	create(context) {
		...
		const styleRules = ["block", "line"].reduce(
			(/** @type {Record<string, StyleRule>} */ rule, type) => {
				const markers = parseMarkersOption(
					(config[type] && config[type].markers) ||
						config.markers ||
						[],
				);
				const exceptions =
		 ...
		 * @param {Comment} node A comment node to check.
		 * @param {string} messageId An error message to report.
		 * @param {RegExpExecArray | null} match An array of match results for markers.
		...
		function reportBegin(node, messageId, match, refChar) {
			...
			context.report({
				node,
				/**
				 * Corrects the spacing at the beginning of the comment.
				...
				fix(fixer) {
					...
					/*
					 * In `never` mode `checkCommentForSpace()` only reports once
					 * the begin pattern has matched, so `match` is never `null`
					 * on this path.
					 */
					const beginMatch = /** @type {RegExpExecArray} */ (match);

					end += beginMatch[0].length;
					return fixer.replaceTextRange(
						[start, end],
						commentIdentifier +
							(beginMatch[1] ? beginMatch[1] : ""),
					);
				},
				messageId,
				data: { refChar },
		 ...
		 * @param {Comment} node A comment node to check.
		 * @param {string} messageId An error message to report.
		 * @param {RegExpExecArray} [match] An array of the matched whitespace characters.
		 * @returns {void}
		 */
		function reportEnd(node, messageId, match) {
			context.report({
				node,
				/**
				 * Corrects the spacing at the end of the comment.
				 * @param {RuleFixer} fixer The fixer to use.
				 * @returns {EditInfo} The fix.
				 */
				fix(fixer) {
					...
					/*
					 * `match` is only omitted by the `requireSpace` caller, which
					 * returned above, so it is always present on this path.
					 */
					const end = node.range[1] - 2,
						start =
							end -
							/** @type {RegExpExecArray} */ (match)[0].length;
