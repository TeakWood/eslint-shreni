...
module.exports = {
	...
	create(context) {
		...
		function getLastTokenOnLine(node) {
			/*
			 * A directive is an expression statement holding a string literal, so it
			 * always has a last token, and that token always has a token before it.
			 */
			const lastToken = /** @type {Token} */ (
				sourceCode.getLastToken(node)
			);
			const secondToLastToken = /** @type {Token} */ (
		...
		function hasNewlineAfter(node) {
			...
			// This is only called for directives that are followed by something, so there is always a token after.
		...
		function reportError(node, location, expected) {
			context.report({
				...
				/**
				 * Inserts or removes the newline around the directive.
		...
		function checkDirectives(node) {
			...
			// The empty prologue returned early above, so there is a last directive.
			const lastDirective = /** @type {ASTNode} */ (directives.at(-1));
			const statements =
