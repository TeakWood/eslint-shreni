 ...
 * require is the one unavoidable implicit `any` in this file. Neither a `@type`
 * annotation nor a cast suppresses `TS7016`, which is reported on the module
 * specifier itself; the only alternatives are a `.d.ts` or a `tsconfig` change.
 * The two values taken from it are both predicates returning `boolean`, and
 * `isIdentifier()` below re-types that result, so the `any` goes no further.
 */
 ...
 * @param {any} [ecmaVersion] The ECMAScript version if specified in the parserOptions config. It arrives unvalidated from `languageOptions`, and one caller omits it entirely, which falls through to the ES5 check.
...
module.exports = {
	...
	create(context) {
		...
		return {
			/**
			 * Compares the name of a function expression with the variable it initializes.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Compares the name of a function expression with the target it is assigned to.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Compares the name of a function expression with the property it is the value of.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
