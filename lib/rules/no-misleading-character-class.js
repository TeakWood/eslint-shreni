...
/**
 * A code unit produced by a string literal or a template token, as returned by
 * the `char-source` helpers. The class itself is not exported, so the type is
 * recovered from the return type of one of the functions that build it.
 * @typedef {ReturnType<typeof parseStringLiteral>[number]} CodeUnit
 */

/**
 * Scans one character sequence taken from a character class and yields every
 * run of characters that is misleading in one specific way.
 *
 * `chars` is the sequence with each character that the `allowEscape` option
 * excuses replaced by `null`; `unfilteredChars` is the same sequence with
 * those characters left in place, for the checks whose base character may be
 * escaped without making the combination any less misleading.
 * @typedef {(chars: Array<Character | null>, unfilteredChars: Array<Character>) => Generator<Array<Character>>} CharacterSequenceFinder
 */

/**
 * What `getStaticValueOrRegex()` knows about a node. Exactly one member is
 * present: `regex` when the node is a regular expression literal, `value` when
 * the node evaluates to some other constant.
 * @typedef {Object} StaticValueOrRegex
 * @property {{ pattern: string, flags: string }} [regex] The `regex` property of a regular expression literal node.
 * @property {any} [value] The static value the node evaluates to.
 */
 ...
 * @param {Array<CharacterClassElement>} nodes The node list to iterate character sequences.
 * @returns {Generator<Array<Character>>} The list of character sequences.
 */
 ...
 * The keys are also the message IDs the matches are reported under.
 * @type {Record<string, CharacterSequenceFinder>}
 */
	...
	*zwj(chars) {
		...
		for (const [index, char] of chars.entries()) {
			...
			if (
				...
				// The three sliced elements are `previous`, `char` and `next`, all just guarded as non-null.
				if (sequence) {
					if (sequence.at(-1) === previous) {
						sequence.push(char, next); // append to the sequence
					} else {
						yield sequence;
						sequence = /** @type {Array<Character>} */ (
							chars.slice(index - 1, index + 2)
						);
					}
				} else {
					sequence = /** @type {Array<Character>} */ (
 ...
 * @param {ASTNode | undefined} node The node to be inspected. A `RegExp` call may have no arguments at all, in which case there is no node to inspect.
 ...
 * @returns {StaticValueOrRegex | null} The static value of the node, or `null`.
 */
 ...
 * @param {Array<CodeUnit>} codeUnits List of CodeUnit objects of the literal that produces the character.
 * @returns {boolean} Whether the specified regexpp character is represented as an acceptable escape sequence.
 */
...
module.exports = {
	 ...
	 * @returns {RuleVisitor} The visitor that checks every character class in the file.
	 */
	create(context) {
		 ...
		 * @param {FixFunction} unicodeFixer Fixer for missing "u" flag.
		 * @returns {void}
		 */
		function verify(node, pattern, flags, unicodeFixer) {
			...
			/** @type {Map<string, Array<Array<Character>>>} */
			const foundKindMatches = new Map();

			visitRegExpAST(patternNode, {
				/**
				 * Collects every misleading character sequence of a character class.
				 * @param {CharacterClass} ccNode The character class to inspect.
				 * @returns {void}
				 */
				onCharacterClassEnter(ccNode) {
					for (const unfilteredChars of iterateCharacterSequence(
						...
						for (const kind of kinds) {
							...
							/*
							 * Every value stored in the map is an array, so
							 * finding one is the same test as `has(kind)`.
							 */
							const foundMatches = foundKindMatches.get(kind);
			 ...
			 * @param {Array<Array<Character>>} matches Lists of individual characters being reported on.
			 * @returns {Array<SourceLocation>} locs for context.report.
			...
			function getNodeReportLocations(matches) {
				...
				return matches.map(chars => {
					...
					// Every match is a non-empty run of characters, so `at(-1)` finds one.
					const lastIndex =
						/** @type {Character} */ (chars.at(-1)).end - 1;
					/** @type {number} */
					let start;
					/** @type {number} */
					let end;
			...
			for (const [kind, matches] of foundKindMatches) {
				/** @type {Array<SuggestionDescriptor> | undefined} */
				let suggest;
		...
		return {
			/**
			 * Checks a regular expression literal.
			 * @param {ASTNode} node The regular expression literal node.
			 * @returns {void}
			 */
			"Literal[regex]"(node) {
				...
				verify(
					...
					node.regex.flags,
					/**
					 * Adds the `u` flag to the regular expression literal.
					 * @param {RuleFixer} fixer The fixer to build the edit with.
					 * @returns {EditInfo | null} The edit, or `null` if the pattern would not be valid with the flag.
					 */
					fixer => {
						if (
			...
			/**
			 * Checks every `RegExp()` and `new RegExp()` call in the file.
				...
				for (const { node: refNode } of tracker.iterateGlobalReferences(
					...
					if (typeof flags === "string") {
						verify(
							...
							flags,
							/**
							 * Adds the `u` flag to the `RegExp` call's flags argument, inserting one if the call has none.
							 * @param {RuleFixer} fixer The fixer to build the edit with.
							 * @returns {EditInfo | null} The edit, or `null` if the flags argument is not a constant this rule can edit.
							 */
							fixer => {
								if (
									!isValidWithUnicodeFlag(
								...
								if (refNode.arguments.length === 1) {
									// The last token of a call is its closing parenthesis, so skipping it always finds another token.
