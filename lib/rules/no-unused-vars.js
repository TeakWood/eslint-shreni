 ...
 * attaches to it after scope analysis has run. `eslintUsed` is set by
 * `markVariableAsUsed()` and by `/* exported *\/` directives;
 * `eslintExplicitGlobalComments` records the `/* globals *\/` comments that
 * declared the variable, and is absent for variables declared in code.
 * @typedef {import("eslint-scope").Variable & { eslintUsed?: boolean, eslintExplicitGlobalComments?: Array<Comment> }} Variable
 */

/**
 * The kinds of variable this rule distinguishes between when it picks which
 * ignore pattern applies and how to describe the variable in the report.
 * @typedef {"array-destructure" | "catch-clause" | "parameter" | "variable"} VariableType
 */

/**
 * The rule's options, after the raw schema-validated config has been folded
 * into the defaults. The four `*IgnorePattern` members arrive from the schema
 * as strings and are compiled to regular expressions before `create()`
 * returns, so every read of them below sees a `RegExp`. They stay optional
 * because the rule tests each for presence before using it.
 * @typedef {Object} UnusedVarsConfig
 * @property {"all" | "local"} vars Which variables to report: all of them, or only ones local to a function.
 * @property {"all" | "after-used" | "none"} args Which function parameters to report.
 * @property {boolean} ignoreRestSiblings Whether to skip properties that sit alongside a rest property.
 * @property {"all" | "none"} caughtErrors Whether to report unused `catch` bindings.
 * @property {boolean} ignoreClassWithStaticInitBlock Whether to skip classes that contain a static initialization block.
 * @property {boolean} ignoreUsingDeclarations Whether to skip `using` and `await using` declarations.
 * @property {boolean} reportUsedIgnorePattern Whether to report variables that match an ignore pattern but are used.
 * @property {RegExp} [varsIgnorePattern] Variables whose name matches this are not reported.
 * @property {RegExp} [argsIgnorePattern] Parameters whose name matches this are not reported.
 * @property {RegExp} [caughtErrorsIgnorePattern] `catch` bindings whose name matches this are not reported.
 * @property {RegExp} [destructuredArrayIgnorePattern] Array destructuring elements whose name matches this are not reported.
 */
 ...
 * `Variable#identifiers`, `Definition#name`, `Definition#node` and
 * `Scope#block` — as bare ESTree nodes: no `parent`, and `range` and `loc`
	...
	create(context) {
		...
		if (typeof firstOption === "string") {
			config = {
				...
				// the schema's `enum` is what restricts the string to these two.
				vars: /** @type {"all" | "local"} */ (firstOption),
			};
		} else {
			config = {
				...firstOption,
		 ...
		 * @param {Definition} def the declaration to check
		...
		function defToVariableType(def) {
			...
			if (
				...
				asNode(def.name).parent.type === "ArrayPattern"
			) {
		 ...
		 * @throws (Unreachable)
		 * @returns {[string, string | undefined]} the given variable's description and
		 * ignore pattern
		 */
		 ...
		 * @returns {Record<string, string>} The message data to be used with this unused variable.
		 */
		 ...
		 * @param {VariableType} variableType a simple name for the types of variables that this rule supports
		 * @returns {Record<string, string>} The message data to be used with
		...
		function isExported(variable) {
			...
			if (definition) {
				let node = asNode(definition.node);
		...
		function hasRestSpreadSibling(variable) {
			if (config.ignoreRestSiblings) {
				const hasRestSiblingDefinition = variable.defs.some(def =>
					hasRestSibling(asNode(def.name).parent),
				);
				const hasRestSiblingReference = variable.references.some(ref =>
					hasRestSibling(asNode(ref.identifier).parent),
				);
		 ...
		 * @param {Array<ASTNode>} nodes The candidate function nodes.
		...
		function isSelfReference(ref, nodes) {
			...
			while (scope) {
				if (nodes.includes(asNode(scope.block))) {
					return true;
				}
		 ...
		 * @param {Variable} variable eslint-scope variable object.
		 * @returns {Array<ASTNode>} Function nodes.
		 * @private
		 */
		...
		function getRhsNode(ref, prevRhsNode) {
			const id = asNode(ref.identifier);
			const parent = id.parent;
			const refScope = ref.from.variableScope;

			/*
			 * The only caller walks `variable.references`, every one of which
			 * `eslint-scope` resolved to that variable, so `resolved` is never
			 * null here.
			 */
			const varScope = /** @type {Variable} */ (ref.resolved).scope
				.variableScope;
		...
		function isInsideOfStorableFunction(id, rhsNode) {
			...
			/*
			 * `funcNode !== null` rather than a truthiness test so this returns
			 * a boolean. The sole caller reads the result in a boolean
			 * position, so the two are indistinguishable at runtime.
			 */
			return (
				funcNode !== null &&
		 ...
		 * @param {Reference} ref A reference to check.
		 * @param {ASTNode | null} rhsNode The RHS node of the previous assignment.
		...
		function isForInOfRef(ref) {
			let target = asNode(ref.identifier).parent;
		...
		function isAfterLastUsedArg(variable) {
			...
			const params = /** @type {Array<Variable>} */ (
				sourceCode.getDeclaredVariables(asNode(def.node))
			);
		 ...
		 * @param {Array<Variable>} unusedVars an array that saving result.
		 * @returns {Array<Variable>} unused variables of the scope and descendant scopes.
		 * @private
		 */
		function collectUnusedVariables(scope, unusedVars) {
			// `eslint-scope` types these without the properties the linter adds.
			const variables = /** @type {Array<Variable>} */ (scope.variables);
			...
			if (scope.type !== "global" || config.vars === "all") {
				for (i = 0, l = variables.length; i < l; ++i) {
					...
					if (
						scope.type === "class" &&
						asNode(scope.block).id === variable.identifiers[0]
					) {
						continue;
					}
					...
					if (def) {
						const type = def.type;
						const defName = asNode(def.name);
						const defNode = asNode(def.node);
						const refUsedInArrayPatterns = variable.references.some(
							ref =>
								asNode(ref.identifier).parent.type ===
								"ArrayPattern",
						);

						// skip elements of array destructuring patterns
						if (
							(defName.parent.type === "ArrayPattern" ||
						...
						if (type === "ClassName") {
							const hasStaticBlock = defNode.body.body.some(
								(/** @type {ASTNode} */ node) =>
						...
						if (type === "CatchClause") {
							...
							if (
								(defNode.parent.type === "Property" ||
									defNode.parent.type ===
										"MethodDefinition") &&
								defNode.parent.kind === "set"
							) {
								continue;
							}
							...
							if (
								...
								config.argsIgnorePattern.test(defName.name)
							) {
								if (
									config.reportUsedIgnorePattern &&
									isUsedVariable(variable)
							...
							if (
								...
								astUtils.isFunction(defName.parent) &&
							...
							if (
								...
								config.varsIgnorePattern.test(defName.name)
							) {
								if (
									config.reportUsedIgnorePattern &&
									isUsedVariable(variable)
		 ...
		 * @param {RuleFixer} fixer fixer object
		 * @param {Variable} unusedVar unused variable to fix
		 * @returns {EditInfo | null} fixer object
		 */
		function handleFixes(fixer, unusedVar) {
			const id = asNode(unusedVar.identifiers[0]);
			const parent = id.parent;
			...
			/*
			 * Every lookup below asks for a token adjacent to a node the rule
			 * has already matched against the grammar, so the surrounding
			 * token always exists. `tokenBefore` and `tokenAfter` above are
			 * the two exceptions - an identifier can start or end the file -
			 * and are guarded at each use.
			 */
			...
			function getPreviousTokenStart(node, skips) {
				return /** @type {Token} */ (
					sourceCode.getTokenBefore(node, skips)
				).range[0];
			}
			...
			function getNextTokenEnd(node, skips) {
				return /** @type {Token} */ (
					sourceCode.getTokenAfter(node, skips)
				).range[1];
			}
			...
			function getTokenBeforeValue(node) {
				return /** @type {Token} */ (sourceCode.getTokenBefore(node))
					.value;
			}
			...
			function getTokenAfterValue(node) {
				return /** @type {Token} */ (sourceCode.getTokenAfter(node))
					.value;
			}
			...
			function isClosingBraceOfBlock(token) {
				...
				/*
				 * The token was produced by the parser for this file, so the
				 * node that spans its start offset is always found.
				 */
			 ...
			 * @param {Token} nextToken next token of unused variable
			 * @param {Token | null} prevToken previous token of unused variable
				...
				if (parentNode.type === "VariableDeclarator") {
					...
					if (getTokenBeforeValue(parentNode) === ",") {
						...
						if (
							...
							const lastTokenOfDecl = /** @type {Token} */ (
								sourceCode.getLastToken(parentNode.parent)
							);
			...
			if (
				allWriteReferences.some(
					ref => asNode(ref.identifier).range[0] !== id.range[0],
				)
			) {
				return null;
			}
			...
			if (parentType === "VariableDeclarator") {
				...
				if (/** @type {Token} */ (tokenBefore).value === ",") {
					// If this is the last declarator, removing it may create an ASI hazard
					if (parent === parent.parent.declarations.at(-1)) {
						const lastTokenOfDecl = /** @type {Token} */ (
							sourceCode.getLastToken(parent.parent)
						);
					...
					return fixer.removeRange([
						/** @type {Token} */ (tokenBefore).range[0],
						parent.range[1],
					]);
				}
			...
			if (parentType === "ArrayPattern") {
				...
				if (
					/** @type {Token} */ (tokenBefore).value === "," &&
					/** @type {Token} */ (tokenAfter).value === ","
				) {
			...
			if (/** @type {Token} */ (tokenAfter).value === ",") {
