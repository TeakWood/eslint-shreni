...
module.exports = {
	...
	create(context) {
		...
		const functionStack = /** @type {Array<number>} */ ([]),
			...
			ignoreTopLevelFunctions =
				...
				false,
			topLevelFunctions =
				/** @type {Array<{ node: ASTNode, count: number }>} */ ([]);
		...
		function endFunction(node) {
			// The matching enter handler pushed an entry for this node.
			const count = /** @type {number} */ (functionStack.pop());
		...
		return {
			...
			/**
			 * Reports the top-level functions that were deferred, unless there
