...
/** @typedef {import("@eslint-community/regexpp").AST.Node} RegExpNode */
/** @typedef {import("@eslint-community/regexpp").AST.Backreference} Backreference */
/** @typedef {import("@eslint-community/regexpp").AST.CapturingGroup} CapturingGroup */
/** @typedef {import("@eslint-community/regexpp").AST.LookaroundAssertion} LookaroundAssertion */

/**
 * A reason why one of the groups a backreference resolves to cannot have
 * matched by the time the backreference is evaluated.
 * @typedef {Object} BackreferenceProblem
 * @property {string} messageId The ID of the message describing the reason.
 * @property {CapturingGroup} group The group the backreference resolves to.
 */
 ...
 * @param {RegExpNode} node Node.
 * @returns {Array<RegExpNode>} Array that starts with the given node and ends with the root node.
 */
function getPathToRoot(node) {
	const path = [];

	/** @type {RegExpNode | null} */
	let current = node;
 ...
 * @param {RegExpNode} node Node.
 * @returns {node is LookaroundAssertion} `true` if it is a lookaround node.
 */
	...
	create(context) {
		...
		function checkRegex(node, pattern, flags) {
			...
			visitRegExpAST(regExpAST, {
				/**
				 * Reports the backreference if none of the groups it resolves to can have matched.
				 * @param {Backreference} bref The backreference node.
				 * @returns {void}
				 */
				onBackreferenceEnter(bref) {
					...
					const problems = groups.map(group => {
						...
						/*
						 * `groupCut` runs from `group` itself up to just below
						 * the lowest common ancestor, so it is never empty.
						 */
						const lowestGroupAncestor = /** @type {RegExpNode} */ (
							groupCut.at(-1)
						);

						if (lowestGroupAncestor.type === "Alternative") {
					...
					/*
					 * The guard above returned unless every group produced a
					 * problem, so nothing below has to account for `null`.
					 */
					const definiteProblems =
						/** @type {Array<BackreferenceProblem>} */ (problems);

					// Gets problems that appear in the same disjunction.
					const problemsInSameDisjunction = definiteProblems.filter(
					...
					if (problemsInSameDisjunction.length) {
						...
						problemsToReport = definiteProblems;
					}
		...
		return {
			...
			/**
			 * Checks every `RegExp()` call and `new RegExp()` expression in the file.
