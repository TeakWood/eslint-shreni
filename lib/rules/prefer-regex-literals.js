...
/** @typedef {import("@eslint-community/regexpp/ecma-versions").EcmaVersion} EcmaVersion */
...
module.exports = {
	...
	create(context) {
		 ...
		 * @returns {string | null} The value of the node, or `null` if it is not a static string.
		 */
		 ...
		 * @returns {EcmaVersion} The resulting ecmaVersion compatible for regexpp.
		 */
		function getRegexppEcmaVersion(ecmaVersion) {
			...
			/*
			 * regexpp types `ecmaVersion` as a union of the versions it knows.
			 * `languageOptions.ecmaVersion` is normalized to a year, and clamping
			 * to `REGEXPP_LATEST_ECMA_VERSION` keeps the value inside that union,
			 * but tsc only sees `number` coming out of `min`.
			 */
			return /** @type {EcmaVersion} */ (
		 ...
		 * @returns {string | null} The resulting escaped character, or `null` if it needs no escaping.
		 */
		 ...
		 * @param {string | null | undefined} flags The regex flags to check.
		 ...
		 * @param {string} pattern The regex pattern to check.
		 * @param {string | null | undefined} flags The regex flags
			...
			Program(node) {
				...
				for (const { node: refNode } of tracker.iterateGlobalReferences(
					...
					if (
						...
						if (refNode.arguments.length === 2) {
							...
							context.report({
								...
								suggest: suggests.map(
									({ flags, pattern, messageId }) => ({
										...
										/**
										 * Replaces the `RegExp` constructor call with an equivalent regex literal.
							...
							context.report({
								...
								suggest: outputs.map(output => ({
									...
									/**
									 * Unwraps the regex literal from the `RegExp` constructor call.
									 * @param {RuleFixer} fixer The fixer.
									 * @returns {EditInfo} The fix.
									 */
									fix(fixer) {
						...
						/*
						 * `hasOnlyStaticStringArguments()` checked every argument with
						 * `isStaticString()`, which is the same three-way test
						 * `getStringValue()` dispatches on, so the `null` fallback is
						 * unreachable for the first argument here.
						 */
						let regexContent = /** @type {string} */ (
							getStringValue(refNode.arguments[0])
						);
						let noFix = false;
						let flags;
						...
						if (regexContent && !noFix) {
							...
							visitRegExpAST(ast, {
								/**
								 * Escapes a character that a regex literal cannot carry verbatim.
								 * @param {Character} characterNode The character node being visited.
								 * @returns {void}
								 */
