 ...
 * @property {boolean} superIsConstructor Whether the `extends` clause can evaluate to a constructor.
 ...
 * @param {ASTNode | null | undefined} node A node to check. The absent cases are
 *   a class without an `extends` clause and, in the recursive calls, an operand
 *   position the parser may leave empty.
...
module.exports = {
	...
	create(context) {
		 ...
		 * - hasExtends:         A flag which shows whether own class has a valid
		 *                       `extends` part.
		 * - superIsConstructor: A flag which shows whether the `extends` part can
		 *                       evaluate to a constructor.
		 * - codePath:           The code path object of the constructor.
		 * - currentSegments:    The code path segments being traversed.
		 *
		...
		return {
			 ...
			 * @param {ASTNode} node Node that starts the segment.
			 * @returns {void}
			 */
			...
			onCodePathSegmentLoop(fromSegment, toSegment) {
				...
				funcInfo.codePath.traverseSegments(
					...
					/**
					 * Propagates the `super()` flags forward through the loop.
					 * @param {CodePathSegment} segment The segment being visited.
					 * @param {TraverseController} controller Steers the traversal.
					 * @returns {void}
					 */
					(segment, controller) => {
