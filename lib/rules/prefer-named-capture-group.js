 ...
 * @returns {Array<SuggestionDescriptor> | null} Fixer suggestions for the regex, if statically determinable.
 */
function suggestIfPossible(groupStart, pattern, rawText, regexNode) {
	...
	return [
		{
			/**
			 * Inserts a generated `?<tempN>` name at the start of the group.
		...
		{
			/**
			 * Inserts `?:` at the start of the group, making it non-capturing.
			 * @param {RuleFixer} fixer The fixer.
			 * @returns {EditInfo} The fix.
			 */
			fix(fixer) {
	...
	create(context) {
		...
		function checkRegex(pattern, node, regexNode, flags) {
			...
			regexpp.visitRegExpAST(ast, {
				/**
				 * Reports a capturing group that has no name.
				 * @param {CapturingGroup} group The capturing group.
				 * @returns {void}
				 */
