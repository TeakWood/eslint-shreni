		...
		function check(node) {
			if (hasUselessComputedKey(node)) {
				...
				context.report({
					...
					/**
					 * Replaces the computed key with its non-computed form.
					 * @param {RuleFixer} fixer The fixer to create the edit with.
					 * @returns {EditInfo | null} The edit, or `null` if the key carries comments.
					 */
					fix(fixer) {
						/*
						 * `node.computed` is `true`, so the key is wrapped in a
						 * `[`/`]` pair and something always precedes the `[`.
						 * `key` is a `Literal`, so it has a first token too.
						 */
						const leftSquareBracket = /** @type {Token} */ (
							sourceCode.getTokenBefore(
								key,
								astUtils.isOpeningBracketToken,
							)
						);
						const rightSquareBracket = /** @type {Token} */ (
						...
						const tokenBeforeLeftBracket = /** @type {Token} */ (
