...
module.exports = {
	...
	create(context) {
		...
		function pushBlock(node) {
			...
			if (len > maxDepth) {
				context.report({
					...
					// Every node this rule visits is a statement, so it has a first token.
		...
		return {
			...
			/**
			 * Counts an `if` statement's block unless it is an `else if`.
			...
			/**
			 * Uncounts an `if` statement's block unless it is an `else if`.
			 * @param {ASTNode} node The `IfStatement` node.
			 * @returns {void}
			 */
