...
/**
 * A problem the rule has decided to report but is holding back, because an
 * enclosing construct may yet prove the parentheses necessary. `finishReport`
 * is the deferred `context.report()` call.
 * @typedef {{ node: ASTNode, finishReport: () => void }} BufferedReport
 */

/**
 * A frame of the reports buffer. The rule pushes one when it enters a
 * `for` statement's initializer, where removing parentheses around an `in`
 * expression could turn the loop into an ill-formed for-in loop, and pops it
 * on the way out. `upper` is the enclosing frame, if any.
 * @typedef {Object} ReportsBuffer
 * @property {ReportsBuffer | undefined} upper The enclosing buffer frame.
 * @property {Array<ASTNode>} inExpressionNodes The `in` expressions seen inside this frame.
 * @property {Array<BufferedReport>} reports The problems withheld by this frame.
 */
...
module.exports = {
	...
	create(context) {
		...
		/*
		 * `getPrecedence()` only ever reads `type` (and `operator`, for the
		 * node types that have one), so a bare shape stands in for a node.
		 */
		const PRECEDENCE_OF_ASSIGNMENT_EXPR = precedence(
			/** @type {ASTNode} */ ({ type: "AssignmentExpression" }),
		);
		const PRECEDENCE_OF_UPDATE_EXPR = precedence(
			/** @type {ASTNode} */ ({ type: "UpdateExpression" }),
		);

		/** @type {ReportsBuffer | undefined} */
		let reportsBuffer;
		...
		function isImmediateFunctionPrototypeMethodCall(node) {
			...
			return (
				...
				["call", "apply"].includes(
					// `null`, returned for a dynamic key, is neither name.
					/** @type {string} */ (
		...
		function isParenthesised(node) {
			/*
			 * `eslint-utils` types this parameter as an ESTree node, which the
			 * rules-layer `ASTNode` deliberately is not — it carries an index
			 * signature so that rules can read properties contributed by
			 * other parsers. Every node reaching here comes from the ESTree
			 * AST this rule visits.
			 */
			return isParenthesizedRaw(
				1,
				/** @type {ESTreeNode} */ (node),
				sourceCode,
			);
		}
		...
		function isParenthesisedTwice(node) {
			// See `isParenthesised` above for why the cast is sound.
			return isParenthesizedRaw(
				2,
				/** @type {ESTreeNode} */ (node),
				sourceCode,
		...
		function isNewExpressionWithParens(newExpression) {
			// A `new` expression always spans at least `new` and its callee.
			const lastToken = /** @type {Token} */ (
				sourceCode.getLastToken(newExpression)
			);
			const penultimateToken = /** @type {Token} */ (
		...
		function requiresLeadingSpace(node) {
			/*
			 * The node is surrounded by parentheses, so the opening paren and
			 * the node's own first token both exist. Only the token before the
			 * opening paren may be missing, at the start of the file.
			 */
			...
			const tokenAfterLeftParen = /** @type {Token} */ (
				sourceCode.getTokenAfter(leftParenToken, {
					includeComments: true,
				})
			);

			return (
				tokenBeforeLeftParen !== null &&
		...
		function requiresTrailingSpace(node) {
			...
			// The node is surrounded by parentheses, so it has a last token.
		 ...
		 * @param {ASTNode | null} [node] The node to check. Array patterns have holes, so this can be `null`.
		...
		function report(node) {
			/*
			 * Every caller checks that the node is parenthesised first, so the
			 * surrounding parens - and the node's own first token - all exist.
			 */
			const leftParenToken = /** @type {Token} */ (
				sourceCode.getTokenBefore(node)
			);
		...
		function checkExpressionOrExportStatement(node) {
			// The statement is non-empty, so it has a first token either way.
			...
			if (
				...
				/*
				 * A statement starting with `(` has a matching `)`, so there is
				 * always a token after the opening parentheses to inspect.
				 */
				secondToken &&
		 ...
		 * @returns {Array<ASTNode>} Path, including both nodes.
		 * @throws If the given node does not have the specified ancestor.
		 */
		 ...
		 * @returns {Array<ASTNode>} Path, including both nodes.
		 * @throws If the given node does not have the specified descendant.
		 */
		...
		/**
		 * Returns the buffer the rule is currently filling. Every caller runs
		 * between a `startNewReportsBuffering()` and its matching
		 * `endCurrentReportsBuffering()`, so a buffer is always open.
		 * @returns {ReportsBuffer} The open buffer.
		 */
		function currentReportsBuffer() {
			return /** @type {ReportsBuffer} */ (reportsBuffer);
		}
		...
		function endCurrentReportsBuffering() {
			const { upper, inExpressionNodes, reports } =
				currentReportsBuffer();
		...
		function isInCurrentReportsBuffer(node) {
			return currentReportsBuffer().reports.some(r => r.node === node);
		}

		/**
		 * Removes the given node from the current reports buffer.
		 * @param {ASTNode} [node] Node to remove. `undefined` removes nothing.
		 * @returns {void}
		 */
		function removeFromCurrentReportsBuffer(node) {
			const buffer = currentReportsBuffer();
		...
		return {
			/**
			 * Checks the parentheses around an array literal's elements.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around an array pattern's elements.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ArrayPattern(node) {
				node.elements
					.filter(
						(/** @type {ASTNode | null} */ e) =>
							...
							hasExcessParens(/** @type {ASTNode} */ (e)),
					)
					.forEach(report);
			},

			/**
			 * Checks the parentheses around an arrow function's expression body.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ArrowFunctionExpression(node) {
				...
				if (node.body.type !== "BlockStatement") {
					/*
					 * The body is an expression, so it has a first token, and
					 * the arrow itself always precedes it.
					 */
					...
					const tokenBeforeFirst = /** @type {Token} */ (
			...
			/**
			 * Checks the parentheses around both sides of an assignment.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks a binary expression's operands, recording `in` expressions so an enclosing `for` initializer can keep their parentheses.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a conditional expression's three operands.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a `do`-`while` test.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around both sides of a `for`-`in` loop.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ForInStatement(node) {
				if (node.left.type !== "VariableDeclaration") {
					// A for-in left-hand side is non-empty and is followed by `in`.
					const firstLeftToken = /** @type {Token} */ (
						sourceCode.getFirstToken(
							node.left,
							astUtils.isNotOpeningParenToken,
						)
			...
			/**
			 * Checks the parentheses around both sides of a `for`-`of` loop.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ForOfStatement(node) {
				if (node.left.type !== "VariableDeclaration") {
					// A for-of left-hand side is non-empty.
					const firstLeftToken = /** @type {Token} */ (
						sourceCode.getFirstToken(
							node.left,
							astUtils.isNotOpeningParenToken,
			...
			/**
			 * Checks the parentheses around a `for` loop's test and update, and starts buffering reports for its initializer.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ForStatement(node) {
				...
				if (node.init) {
					if (node.init.type !== "VariableDeclaration") {
						// The initializer is present, so it has a first token, and the `;` follows it.
						const firstToken = /** @type {Token} */ (
							sourceCode.getFirstToken(
								node.init,
								astUtils.isNotOpeningParenToken,
			...
			/**
			 * Flushes the buffered reports for a `for` loop's initializer, keeping the parentheses that stop an `in` expression from being read as an ill-formed for-in loop.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			"ForStatement > *.init:exit"(node) {
				...
				if (buffer.reports.length) {
					buffer.inExpressionNodes.forEach(inExpressionNode => {
			...
			/**
			 * Checks the parentheses around an `if` test.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			IfStatement(node) {
			...
			/**
			 * Checks the parentheses around a dynamic import's source.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a member expression's object and computed property.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a computed method name.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around an object literal's property values.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ObjectExpression(node) {
				node.properties
					...
					.forEach((/** @type {ASTNode} */ property) =>
						report(property.value),
					);
			},

			/**
			 * Checks the parentheses around an object pattern's property values.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ObjectPattern(node) {
				node.properties
					.filter((/** @type {ASTNode} */ property) => {
			...
			/**
			 * Checks the parentheses around a computed property key.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			Property(node) {
			...
			/**
			 * Checks the parentheses around a class field's computed key and its value.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a rest element's argument.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			RestElement(node) {
			...
			/**
			 * Checks the parentheses around a `return` statement's argument.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ReturnStatement(node) {
				// A `return` statement always starts with the `return` keyword.
				const returnToken = /** @type {Token} */ (
					sourceCode.getFirstToken(node)
				);
			...
			/**
			 * Checks the parentheses around a sequence expression's operands.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a `case` test.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			SwitchCase(node) {
			...
			/**
			 * Checks the parentheses around a `switch` discriminant.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a `throw` statement's argument.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			ThrowStatement(node) {
				// A `throw` statement always starts with the `throw` keyword.
				const throwToken = /** @type {Token} */ (
					sourceCode.getFirstToken(node)
				);
			...
			/**
			 * Checks the parentheses around an update expression's argument.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			UpdateExpression(node) {
				if (node.prefix) {
					...
					const { argument } = node;
					// A postfix update expression ends with its operator.
			...
			/**
			 * Checks the parentheses around a declarator's initializer.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			...
			/**
			 * Checks the parentheses around a `while` test.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			WhileStatement(node) {
			...
			/**
			 * Checks the parentheses around a `with` object.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			WithStatement(node) {
			...
			/**
			 * Checks the parentheses around a `yield` expression's argument.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			YieldExpression(node) {
				if (node.argument) {
					// A `yield` expression always starts with the `yield` keyword.
					const yieldToken = /** @type {Token} */ (
						sourceCode.getFirstToken(node)
					);
			...
			/**
			 * Checks the parentheses around a template literal's substitutions.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
			TemplateLiteral(node) {
				node.expressions
					.filter(
						(/** @type {ASTNode} */ e) => e && hasExcessParens(e),
					)
					.forEach(report);
			},

			/**
			 * Checks the parentheses around both sides of a default-value pattern.
			 * @param {ASTNode} node The node to check.
			 * @returns {void}
			 */
