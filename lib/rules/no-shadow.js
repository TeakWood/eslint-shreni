...
/**
 * The rule's options. Every member is present by the time `create()` runs,
 * because `meta.defaultOptions` supplies the whole object and `meta.schema`
 * has already validated whatever the user merged into it.
 * @typedef {Object} NoShadowOptions
 * @property {Array<string>} allow Variable names that are allowed to shadow.
 * @property {boolean} builtinGlobals Whether to report shadowing of built-in globals.
 * @property {"all" | "functions" | "never" | "types" | "functions-and-types"} hoist Which kinds of declaration count as hoisted above a use that precedes them.
 * @property {boolean} ignoreOnInitialization Whether to ignore a shadow that can only be evaluated before the outer variable is initialized.
 * @property {boolean} ignoreTypeValueShadow Whether to ignore a type shadowing a value, and vice versa.
 * @property {boolean} ignoreFunctionTypeParameterNameValueShadow Whether to ignore a function type's parameter name shadowing a value.
 */

/**
 * A variable as `typescript-eslint`'s scope analyser produces it. That analyser
 * splits a binding into its value and type halves and records which half a
 * given variable is; `eslint-scope` itself never sets the field, so it is
 * optional here and every read of it is guarded.
 * @typedef {Variable & { isValueVariable?: boolean }} TSVariable
 */

/**
 * Where a shadowed variable was declared. A variable that came from the
 * environment rather than from this file has no identifier to point at, so it
 * reports only that it is global.
 * @typedef {Object} DeclaredLocation
 * @property {boolean} global Whether the shadowed variable is a global with no declaration in this file.
 * @property {number} [line] The 1-based line the shadowed variable is declared on.
 * @property {number} [column] The 1-based column the shadowed variable is declared on.
 */
...
/*
 * Definition kinds whose `node` carries the `declare` modifier in a definition
 * file. This is a set lookup rather than a chain of `Definition#type`
 * comparisons because `eslint-scope` types that field as only the kinds it
 * creates itself, while `TSEnumName` and `TSModuleName` are contributed by
 * `typescript-eslint`'s scope analyser.
 */
const DECLARE_ON_NODE_DEF_TYPES = new Set([
	"ClassName",
	"TSEnumName",
	"TSModuleName",
]);
 ...
 *
 * `eslint-scope` types the AST it stores — `Variable#identifiers[n]`,
 * `Definition#name`, `Definition#node`, `Definition#parent` and `Scope#block` —
 * as bare ESTree nodes: no `parent`, and `range` and `loc` optional. The linter
...
module.exports = {
	...
	create(context) {
		...
		] = /** @type {[NoShadowOptions]} */ (context.options);
		...
		function isGlobalAugmentation(scope) {
			return (
				asNode(scope.block).kind === "global" ||
		 ...
		 * @param {TSVariable} variable The variable to check
		 * @param {TSVariable} shadowedVariable The shadowed variable
		...
		function isTypeValueShadow(variable, shadowedVariable) {
			...
			const isTypeImport =
				...
				(asNode(firstDefinition.parent).importKind === "type" ||
					asNode(firstDefinition.parent).specifiers.some(
						(/** @type {ASTNode} */ s) => s.importKind === "type",
					));
		...
		function isTypeParameterOfStaticMethod(variable) {
			const typeParameter = asNode(variable.identifiers[0]).parent;
		 ...
		 *
		 * Every call site reaches this through an `any`-typed property such as
		 * `VariableDeclarator#init`, which may be `null` — hence the leading
		 * `node &&` guard, which the parameter type deliberately does not restate.
		 ...
		 * @returns {Scope | null} Function's outer scope.
		 */
		 ...
		 * @param {Variable} shadowedVariable a shadowedVariable to check.
		...
		function isInitPatternNode(variable, shadowedVariable) {
			...
			const fun = asNode(variableScope.block);
			const { parent } = fun;
		...
		function isDuplicatedClassNameVariable(variable) {
			const block = asNode(variable.scope.block);
		 ...
		 * @param {Variable} innerVariable The inner variable to check.
		 * @param {Variable} outerVariable The outer variable.
		...
		function isFunctionNameInitializerException(
			...
			const outerIdentifier = asNode(outerDef.name);
			let initializerNode;
			...
			const nodeToCheck = asNode(innerDef.node); // FunctionExpression or ClassExpression node
		 ...
		 *
		 * The `def &&` guard is defensive: `eslint-scope` types `defs[0]` as
		 * always present, which is why it does not widen the return type.
		 * @param {Variable} variable The variable to get.
		 * @returns {Range} The range of the variable's identifier node.
		 */
		function getNameRange(variable) {
			const def = variable.defs[0];

			return def && asNode(def.name).range;
		}

		/**
		 * Get declared line and column of a variable.
		 * @param {Variable} variable The variable to get.
		 * @returns {DeclaredLocation} The declared line and column of the variable.
		 */
		function getDeclaredLocation(variable) {
			...
			if (identifier) {
				obj = {
					global: false,
					line: asNode(identifier).loc.start.line,
					column: asNode(identifier).loc.start.column + 1,
				};
			} else {
				obj = {
					global: true,
		 ...
		 * @param {Variable} variable The variable to check.
		 * @param {Variable} scopeVar The variable of TDZ.
		...
		function isDeclareInDTSFile(variable) {
			...
			return variable.defs.some(
				def =>
					(def.type === "Variable" && asNode(def.parent).declare) ||
					(DECLARE_ON_NODE_DEF_TYPES.has(def.type) &&
						asNode(def.node).declare),
			);
		}
		 ...
		 * @param {Variable} variable Current variable
		 * @param {Variable} shadowedVariable Shadowed variable
		...
		function isExternalDeclarationMerging(
			...
			const moduleDecl = findSelfOrAncestor(
				asNode(variable.identifiers[0])?.parent,
			 ...
			 */
			const importSource = asNode(firstDefinition.parent).source.value;
			const moduleName =
		...
		function checkForShadows(scope) {
			...
			for (let i = 0; i < variables.length; ++i) {
				...
				// `checkForShadows()` only ever runs on a scope below the global scope.
				...
				if (
					...
					!isGenericOfAStaticMethodShadow(variable) &&
		...
		return {
			/**
			 * Checks every scope in the file for shadowed variables.
			...
			"Program:exit"(node) {
				...
				while (stack.length) {
					// The loop condition guarantees the stack is not empty.
