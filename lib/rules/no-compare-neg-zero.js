					...
					suggest: [
						{
							...
							/**
							 * Replaces each `-0` operand with `0`.
							 * @param {RuleFixer} fixer The fixer to build the edits with.
						...
						},
						{
							messageId:
								node.operator === "==="
									...
									: "suggestNotObjectIs",
							/**
							 * Rewrites the comparison as a call to `Object.is()`.
							 * @param {RuleFixer} fixer The fixer to build the edit with.
							 * @returns {EditInfo | null} The edit, or `null` when the rewrite is not safe.
							 */
							fix(fixer) {
								if (
									(node.operator !== "===" &&
