...
module.exports = {
	...
	create(context) {
		...
		function checkComputedProperty(node, value) {
			/*
			 * `RegExp#test` stringifies its argument, so the `boolean` and
			 * `null` keys this rule also accepts are matched by their source
			 * text just as a string key is.
			 */
			if (
				validIdentifier.test(/** @type {string} */ (value)) &&
				(allowKeywords || !keywords.includes(String(value))) &&
				!(
					allowPattern &&
					allowPattern.test(/** @type {string} */ (value))
				)
			) {
				...
				context.report({
					...
					/**
					 * Rewrites the computed member access as dot notation.
					 * @param {RuleFixer} fixer The fixer to use.
					 * @returns {Generator<EditInfo>} The fixes to apply.
					 */
					*fix(fixer) {
						// The property is computed, so the node is bracketed.
		...
		return {
			/**
			 * Checks the member access for the property notation to use.
			...
			MemberExpression(node) {
				...
				if (
					...
					context.report({
						...
						/**
						 * Rewrites the dot notation as a computed member access.
						 * @param {RuleFixer} fixer The fixer to use.
						 * @returns {Generator<EditInfo>} The fixes to apply.
						 */
						*fix(fixer) {
							// A non-computed property is always preceded by `.` or `?.`.
