...
module.exports = {
	...
	create(context) {
		...
		function getClosingParenOfParams(node) {
			// Only called once the parens are known to be present.
			return /** @type {Token} */ (
		...
		return {
			/**
			 * Checks the parens around a single-parameter arrow function.
			...
			"ArrowFunctionExpression[params.length=1]"(node) {
				...
				if (shouldHaveParens && !hasParens) {
					context.report({
						...
						loc: param.loc,
						/**
						 * Adds the missing parens around the parameter.
				...
				if (
					...
					context.report({
						...
						loc: param.loc,
						/**
						 * Removes the unnecessary parens around the parameter.
						 * @param {RuleFixer} fixer The fixer to use.
						 * @returns {Generator<EditInfo>} The fixes to apply.
						 */
						*fix(fixer) {
							...
							if (
								...
								!astUtils.canTokensBeAdjacent(
									tokenBeforeOpeningParen,
									// A parameter always has a first token.
									/** @type {Token} */ (
