...
/** @import { ASTNode, Comment, NodeOrToken, Position, Reference, Scope, SourceCode, SourceLocation, SourceRange, Token, Variable } from "../../types/core.js" */

/**
 * A node, as this module sees one.
 *
 * ESCAPE HATCH, and the only structural one in this file. The AST vocabulary
 * decided by the phase-0 spike is a closed union of ~89 node interfaces, and
 * `lib/types/core.d.ts` currently carries only its agreed base — `ASTNode`,
 * with `type`, `range`, `loc` and `parent` and no per-node members (see "The
 * AST seam" in `.shreni/design/typescript-types-from-jsdoc.md`). The union
 * itself is authored by a later bead.
 *
 * `Node` is that base intersected with the members this module actually reads,
 * so every signature below still speaks `ASTNode` — this is a widening of the
 * vocabulary, deliberately not a second one. It buys no narrowing: `ASTNode`
 * declares `type` as a bare `string`, so `node.type === "Literal"` cannot
 * discriminate, and members that exist on only some node kinds are declared
 * with the widest shape any of them uses.
 *
 * RETIREMENT: when the node union lands, delete `Node` and `NodeMembers` and
 * replace every `Node` below with `ASTNode`. The signatures do not change.
 ...
 *
 * `RegExp` and `bigint` literals carry `null` here in environments that cannot
 * represent them, which is why `isNullLiteral` tests `regex`/`bigint` rather
 * than `value` — see the comment there.
 * @typedef {string | number | boolean | null | RegExp | bigint} LiteralValue
 */
 ...
 * intersecting the two narrows it to `Node`. That is the one claim here that
 * overstates the runtime: `Program.parent` really is `null`. It is declared
 * this way because the alternative is ~40 non-null assertions at sites that all
 * read `parent` off a node the caller already knows is not the root, and the
 * union will state it exactly (`parent: null` on `Program` alone). The two
 * walks that do reach the root — `getUpperFunction` and
 * `isStartOfExpressionStatement` — still test it at runtime, and those guards
 * are load-bearing: do not remove them because the type says they cannot fire.
 * Every member below is declared non-optional for the same reason. The union
 * will make each of them present exactly on the node kinds that have it; an
 * interim view that cannot discriminate can only offer them unconditionally or
 * make all ~35 of them `| undefined`, and the second choice buys no safety —
 * it just moves the assertion to every read. Nullability that the runtime
 * really does express through a member's VALUE (`id`, `test`, `init`,
 * `alternate`, `elements`, `options`) is kept.
 * @typedef {Object} NodeMembers
 * @property {Node} parent The node's parent. See above.
 * @property {Node | null} id `Identifier` naming a function or class.
 * @property {string} name The name of an `Identifier` or `PrivateIdentifier`.
 * @property {LiteralValue | Node} value A `Literal`'s value, or the value half
 * of a `Property`, `PropertyDefinition` or `MethodDefinition`.
 * @property {string} raw A `Literal`'s source text.
 * @property {{ pattern: string; flags: string }} regex Set on `RegExp` literals only.
 * @property {string} bigint Set on `BigInt` literals only.
 * @property {Node} expression `ChainExpression#expression` or `ExpressionStatement#expression`.
 * @property {Node[]} expressions `SequenceExpression` or `TemplateLiteral` sub-expressions.
 * @property {(ASTNode & { value: { cooked: string; raw: string } })[]} quasis `TemplateLiteral` string parts.
 * @property {Node} key The key of a `Property`, `PropertyDefinition`, `MethodDefinition` or `ImportAttribute`.
 * @property {boolean} computed Whether a key or member access is computed.
 * @property {Node} object `MemberExpression#object`.
 * @property {Node} property `MemberExpression#property`.
 * @property {Node} callee `CallExpression#callee` or `NewExpression#callee`.
 * @property {Node[]} arguments Call and `new` arguments.
 * @property {Node} left The left operand of a binary, logical or assignment form.
 * @property {Node} right The right operand of a binary, logical or assignment form.
 * @property {string} operator The operator of a binary, logical, unary, update or assignment form.
 * @property {Node | Node[]} body A statement list (`Program`, `BlockStatement`) or a single body node (a function's or a loop's).
 * @property {Node[]} params A function's parameters.
 * @property {Node} argument The operand of a unary, update, spread, yield or await form.
 * @property {(Node | null)[]} elements `ArrayExpression` elements. `null` for the holes in a sparse array.
 * @property {Node | null} test The test of a conditional, a loop or a `SwitchCase`. `null` for `default:`.
 * @property {Node | null} init `VariableDeclarator#init` or `ForStatement#init`.
 * @property {string} kind A declaration's kind, or an accessor's (`get`/`set`/`constructor`).
 * @property {boolean} static Whether a class member is static.
 * @property {boolean} shorthand Whether a `Property` uses shorthand syntax.
 * @property {boolean} method Whether a `Property` is a method.
 * @property {boolean} generator Whether a function is a generator.
 * @property {boolean} async Whether a function is async.
 * @property {string} directive The directive text of an `ExpressionStatement` in a directive prologue.
 * @property {Node} consequent `ConditionalExpression#consequent`.
 * @property {Node | null} alternate `ConditionalExpression#alternate` or `IfStatement#alternate`.
 * @property {Node} typeAnnotation The annotated type of a TypeScript type node.
 * @property {Node | null} options `ImportExpression#options`.
 */

/**
 * A token or a comment, which is what every token predicate here accepts.
 *
 * The predicates are handed to the token store as filters, and
 * `SourceCode`'s getters pass comments through those filters as well as tokens
 * (`TokenFilter` in `lib/types/core.d.ts`), so accepting only `Token` would
 * make them unassignable at the call sites that pass them.
 * @typedef {Token | Comment} AnyToken
 */

/**
 * A token or comment as `canTokensBeAdjacent` compares them.
 *
 * That method is the one place here that tokenizes on its own rather than
 * reading the token store, so it sees espree's shapes rather than ESLint's.
 * espree declares `tokenize`'s result for every option set it accepts —
 * `value` is `any`, `range` and `loc` are optional, `comments` is optional,
 * and comment `type`s are a narrower union than token `type`s — while the
 * options passed there fix all of it. This is what those options guarantee.
 * @typedef {{ type: string; value: string; range: SourceRange }} AdjacencyToken
 */
 ...
 *
 * Key is the name of the variable. Value is `true` if the variable is
 * considered writable, `false` otherwise.
 *
 * The cast is needed because `conf/globals.js` is not annotated yet, so its
 * inferred type is an object literal with one property per ECMAScript edition
 * and no index signature — which a computed edition name cannot select from.
 * @type {Record<string, boolean>}
 */
const ECMASCRIPT_GLOBALS =
	/** @type {Record<string, Record<string, boolean>>} */ (globals)[
		`es${LATEST_ECMA_VERSION}`
	];
 ...
 * @returns {boolean | null} Whether or not a node is a constructor. `null`
 * when the function has no name, which every caller treats as falsy.
 */
 ...
 * @param {Node} node A start node to find.
 ...
 * @param {Node} node The node to address.
 * @returns {Node} The `ChainExpression#expression` value if the node is a `ChainExpression` node. Otherwise, the node.
 */
 ...
 * @param {Node} left The Literal node to compare.
 * @param {Node} right The other Literal node to compare.
 ...
 *     a[0] = a[0]
 *     a['b'] = a['b']
 * @param {Node} left The left side of the comparison.
 * @param {Node} right The right side of the comparison.
 ...
 * @param {(token: AnyToken) => boolean} f The function to negate.
 * @returns {(token: AnyToken) => boolean} Negated function.
 */
function negate(f) {
...
function isParenthesised(sourceCode, node) {
	/*
	 * The token store returns `null` past either end of the file. The
	 * `Boolean(previousToken && nextToken)` conjunct below is the real guard
	 * and must stay; asserting here keeps the four guarded reads from each
	 * needing their own assertion.
	 */
	const previousToken = /** @type {AnyToken} */ (
			sourceCode.getTokenBefore(node)
		),
		nextToken = /** @type {AnyToken} */ (sourceCode.getTokenAfter(node));
 ...
 * @param {AnyToken} token The token to check.
 ...
 * @param {Node} node The node to get the comment for.
 * @param {SourceCode} sourceCode A SourceCode instance to get comments.
 * @returns {Comment | null} The Block comment token containing the JSDoc comment for the given node or null if not found.
 * @private
 */
...
function getOpeningParenOfParams(node, sourceCode) {
	...
	if (node.type === "ArrowFunctionExpression" && node.params.length === 1) {
		/*
		 * The token store's getters are declared nullable because they return
		 * `null` past the ends of the file. A function node always has a first
		 * token, a parameter always has one, and a parameter list always has an
		 * opening paren unless this is the single-bare-param arrow the branch
		 * above handles — so none of the four reads here can be `null`.
		 */
		const argToken = /** @type {AnyToken} */ (
			sourceCode.getFirstToken(node.params[0])
		);
		const maybeParenToken = /** @type {AnyToken} */ (
			sourceCode.getTokenBefore(argToken)
		);
 ...
 * @param {Node} left A node 1 to compare.
 * @param {Node} right A node 2 to compare.
 * @param {SourceCode} sourceCode The ESLint source code object.
 ...
 * @param {Node} left A node to check.
 * @param {Node} right Another node to check.
 ...
 * @param {SourceCode} sourceCode The source code object to get tokens.
 * @returns {AnyToken} The colon token of the node.
 */
function getSwitchCaseColonToken(node, sourceCode) {
	/*
	 * Both getters are declared nullable for the ends of the file. A
	 * `SwitchCase` always has its colon, so neither can be `null` here.
	 */
	if (node.test) {
		return /** @type {AnyToken} */ (
			sourceCode.getTokenAfter(node.test, isColonToken)
		);
	}
	return /** @type {AnyToken} */ (sourceCode.getFirstToken(node, 1));
}

/**
 * Gets ESM module export name represented by the given node.
 * @param {Node} node `Identifier` or string `Literal` node in a position
...
function getModuleExportName(node) {
	...
	/*
	 * A string literal. The interim node view cannot discriminate on `type`,
	 * so `value` carries every literal's value type at once; the doc comment
	 * above is what constrains the caller to pass a string literal.
	 */
 ...
 * @param {Node} node any `Literal` node
 ...
 * @param {Node} node The branch of main condition which needs to be checked
 ...
 * @param {Node | null | undefined} node The AST node to check. `null` for the holes in a
 * sparse array, `undefined` for the last element of an empty list. Both are
 * answered by the `!node` guard on the first line.
 ...
 * @param {Node} node A node at the position where an opening parenthesis, bracket, or asterisk will be inserted.
 * @returns {boolean} Whether a semicolon is required before the opening parenthesis or bracket.
 * @type {(sourceCode: SourceCode, node: Node) => boolean}
 */
...

{
	...
	needsPrecedingSemicolon = function (sourceCode, node) {
		...
		/*
		 * `getNodeByRangeIndex` is nullable for offsets outside the tree;
		 * `prevToken` came from the tree, so its start is always inside one.
		 */
		const prevNode = /** @type {Node} */ (
			sourceCode.getNodeByRangeIndex(prevToken.range[0])
		);
		...
		if (IDENTIFIER_OR_KEYWORD.has(prevToken.type)) {
			...
			/*
			 * `NODE_TYPES_BY_KEYWORD` is a `__proto__: null` table, so it has
			 * no prototype and admits only index access. Its inferred type is
			 * an object literal — one property per keyword, plus `__proto__`
			 * typed `null` — which is why the widening goes through
			 * `unknown`: `null` is not a `string`, so a direct assertion to
			 * `Record<string, string>` is rejected.
			 */
			const nodeType = /** @type {Record<string, string>} */ (
				/** @type {unknown} */ (NODE_TYPES_BY_KEYWORD)
			)[keyword];
...
module.exports = {
	 ...
	 * @param {NodeOrToken} left The left token object.
	 * @param {NodeOrToken} right The right token object.
	 ...
	 * @param {Comment} node The comment token to be checked
	 ...
	 * @param {import("esutils").StatementNode} node A node to get.
	 * @returns {import("esutils").StatementNode | null} The trailing statement's node.
	 */
	 ...
	 * @param {Node} node A function node to check. It also can be an implicit function, like `StaticBlock`
	 * or any expression that is `PropertyDefinition#value` node.
	 * @param {SourceCode} sourceCode A SourceCode instance to get comments.
	 * @param {object} [options] Options.
	 * @param {boolean} [options.capIsConstructor] `false` disables the assumption that functions which name starts
			...
			switch (parent.type) {
				...
				case "VariableDeclarator":
					return !(
						...
						// A `VariableDeclarator` always has an `id`; only a function's is nullable.
						/** @type {Node} */ (parent.id).type === "Identifier" &&
						startsWithUpperCase(
							/** @type {Node} */ (parent.id).name,
						)
					);
	...
	isEmptyBlock(node) {
		return Boolean(
			...
			// A `BlockStatement`'s body is a statement list.
			/** @type {Node[]} */ (node.body).length === 0,
		);
	},
	...
	isEmptyFunction(node) {
		return (
			isFunction(node) &&
			// A function's body is a single node, not a statement list.
			module.exports.isEmptyBlock(/** @type {Node} */ (node.body))
		);
	},
	 ...
	 * @param {Node} node The node to check.
	 * @returns {Node[]} The directives found in the directive prologue.
	 */
	getDirectivePrologue(node) {
		...
		if (
			...
			(node.type === "ArrowFunctionExpression" &&
				/** @type {Node} */ (node.body).type === "BlockStatement")
		) {
			/*
			 * `body` is a statement list on `Program` and a single
			 * `BlockStatement` on the three function forms, which is the one
			 * place in this file where the interim view's widest-shape rule
			 * for a member costs a pair of assertions.
			 */
			const statements =
				node.type === "Program"
					? /** @type {Node[]} */ (node.body)
					: /** @type {Node[]} */ (
							/** @type {Node} */ (node.body).body
						);
	 ...
	 * @param {Node} node The function node to get.
	 * @param {SourceCode} sourceCode The source code object to get tokens.
	 * @returns {SourceLocation} The location of the function node for reporting.
	 */
	getFunctionHeadLoc(node, sourceCode) {
		...
		if (
			...
			// An arrow function always has its `=>` before the body.
			const arrowToken = /** @type {AnyToken} */ (
	 ...
	 * @returns {Position | null} Next location
	 */
	...
	getParenthesisedText(sourceCode, node) {
		let leftToken = /** @type {AnyToken} */ (
			sourceCode.getFirstToken(node)
		);
		let rightToken = /** @type {AnyToken} */ (
			sourceCode.getLastToken(node)
		);

		/*
		 * Every read below is already guarded by the preceding conjunct in the
		 * same condition, but TypeScript cannot correlate two calls to the
		 * same method, so each guarded read is asserted. The guards are the
		 * load-bearing half — a node at either end of the file has no
		 * surrounding token.
		 */
		while (
			sourceCode.getTokenBefore(leftToken) &&
			/** @type {AnyToken} */ (sourceCode.getTokenBefore(leftToken))
				.type === "Punctuator" &&
			/** @type {AnyToken} */ (sourceCode.getTokenBefore(leftToken))
				.value === "(" &&
			sourceCode.getTokenAfter(rightToken) &&
			/** @type {AnyToken} */ (sourceCode.getTokenAfter(rightToken))
				.type === "Punctuator" &&
			/** @type {AnyToken} */ (sourceCode.getTokenAfter(rightToken))
				.value === ")"
		) {
			leftToken = /** @type {AnyToken} */ (
				sourceCode.getTokenBefore(leftToken)
			);
			rightToken = /** @type {AnyToken} */ (
				sourceCode.getTokenAfter(rightToken)
			);
		}
	 ...
	 * @param {Node} node ASTNode to check
	...
	couldBeError(node) {
		switch (node.type) {
			...
			case "SequenceExpression": {
				...
				return (
					exprs.length !== 0 &&
					// Guarded by the `length` test on the line above.
					module.exports.couldBeError(
						/** @type {Node} */ (exprs.at(-1)),
					)
				);
			}
			...
			case "ConditionalExpression":
				return (
					...
					// A `ConditionalExpression` always has an alternate; only an `IfStatement`'s is nullable.
					module.exports.couldBeError(
						/** @type {Node} */ (node.alternate),
					)
				);
	 ...
	 * @param {string | AnyToken} leftValue The left token. If this is a string, it will be tokenized and the last token will be used.
	 * @param {string | AnyToken} rightValue The right token. If this is a string, it will be tokenized and the first token will be used.
	...
	canTokensBeAdjacent(leftValue, rightValue) {
		...
		if (typeof leftValue === "string") {
			...
			try {
				/*
				 * `espreeOptions` sets `range: true` and `comment: true`, so
				 * the ranges and the `comments` array espree declares as
				 * optional are all present. The widening goes through
				 * `unknown` because espree's shape and `AdjacencyToken`
				 * disagree in both directions — see the typedef.
				 */
				tokens =
					/** @type {AdjacencyToken[] & { comments: AdjacencyToken[] }} */ (
						/** @type {unknown} */ (
							espree.tokenize(leftValue, espreeOptions)
						)
					);
			} catch {
				return false;
			...
			// `at` is nullable for an empty list; the `!leftToken` guard below covers that.
			leftToken = /** @type {AdjacencyToken} */ (tokens.at(-1));
			if (comments.length) {
				const lastComment = /** @type {AdjacencyToken} */ (
					comments.at(-1)
				);
		...
		if (typeof rightValue === "string") {
			...
			try {
				// Same widening as for the left operand above.
				tokens =
					/** @type {AdjacencyToken[] & { comments: AdjacencyToken[] }} */ (
						/** @type {unknown} */ (
							espree.tokenize(rightValue, espreeOptions)
						)
					);
			} catch {
				return false;
	 ...
	 *     else
	 *         bar();
	 * @param {Node} node `BlockStatement` body with exactly one statement directly inside. The statement can have its own nested statements.
	...
	areBracesNecessary(node, sourceCode) {
		...
		function isFollowedByElseKeyword(nodeToCheck) {
			...
			return (
				Boolean(nextToken) &&
				// Guarded by the `Boolean` test on the line above.
				isElseKeywordToken(/** @type {AnyToken} */ (nextToken))
			);
		}
		 ...
		 *        else
		 *            bar();
		 * @param {Node} nodeToCheck Node representing the code to check.
		...
		function hasUnsafeIf(nodeToCheck) {
			switch (nodeToCheck.type) {
				...
				case "WithStatement":
				case "WhileStatement":
					// A loop's or label's body is a single node, not a statement list.
					return hasUnsafeIf(/** @type {Node} */ (nodeToCheck.body));
				default:
					return false;
			}
		}

		// The doc comment above constrains `node` to a `BlockStatement`.
		const statement = /** @type {Node[]} */ (node.body)[0];
