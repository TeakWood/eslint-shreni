 ...
 * @type Key is the name of the variable. Value is `true` if the variable is considered writable, `false` otherwise.
 */
 ...
 * @param node The node to check.
 * @param objectName The expected name or the expected pattern of the object name. If this is nullish, this method doesn't check object.
 * @param propertyName The expected name or the expected pattern of the property name. If this is nullish, this method doesn't check property.
 ...
 * @param [disableStaticComputedKey] Don't address `a.b` and `a["b"]` are the same if `true`. For backward compatibility.
 ...
 * @param node The node to check.
 * @param sourceCode Source code to which the node belongs.
 * @param objectName Name of the global object.
 ...
 * @param node A node at the position where an opening parenthesis, bracket, or asterisk will be inserted.
...
module.exports = {
	 ...
	 *     else
	 *         bar();
	 * @param node `BlockStatement` body with exactly one statement directly inside. The statement can have its own nested statements.
	...
	areBracesNecessary(node, sourceCode) {
		 ...
		 *        else
		 *            bar();
		 * @param nodeToCheck Node representing the code to check.
