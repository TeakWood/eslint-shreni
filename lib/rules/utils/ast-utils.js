...
/*
 * `esutils` ships no type declarations and has no `@types` package, so the
 * require is the one unavoidable implicit `any` in this file. The single value
 * taken from it is re-typed where it is re-exported (`getTrailingStatement`),
 * so the `any` does not reach the emitted declarations.
 */
...
/**
 * A node of the AST as the rules layer sees it.
 *
 * This is deliberately looser than `estree.Node`. The tree a rule receives is
 * not the bare ESTree union: the linter links every node to its `parent` and
 * always populates `range` and `loc`, `Literal` carries the `regex` and
 * `bigint` fields as a single node type rather than as three, and third-party
 * parsers feed in node types ESTree does not describe at all (JSX, and the
 * TypeScript nodes that `isImportAttributeKey` and `getStaticPropertyName`
 * below match on by name). Modelling the parameter as the ESTree union would
 * therefore force a cast at nearly every property read in this file — and at
 * every call site in the 289 rule files that depend on it — without making a
 * single one of those reads safer.
 *
 * What is pinned here is the part that genuinely holds for every node the
 * linter hands to a rule; type-specific fields stay open.
 * @typedef {{ type: string, parent: ASTNode, range: Range, loc: SourceLocation, [key: string]: any }} ASTNode
 */

/**
 * A token produced by the parser, including comment tokens.
 * @typedef {{ type: string, value: string, range: Range, loc: SourceLocation, [key: string]: any }} Token
 */

/**
 * A comment token.
 * @typedef {{ type: "Line" | "Block", value: string, range: Range, loc: SourceLocation }} Comment
 */
 ...
 * @returns {ASTNode | null} A found function node.
 */
 ...
 * @param {ASTNode} node The node to check.
 * @param {string | RegExp | null | undefined} objectName The expected name or the expected pattern of the object name. If this is nullish, this method doesn't check object.
 * @param {string | RegExp | null | undefined} propertyName The expected name or the expected pattern of the property name. If this is nullish, this method doesn't check property.
 ...
 * @param {ASTNode} node A node to check.
 * @returns Whether or not the node is a `Array.fromAsync`.
 */
 ...
 * @param {ASTNode} node A node to check.
 * @returns Whether or not the node is a method which expects a function as a first argument, and `thisArg` as a second argument.
 */
 ...
 * @param {string | RegExp | null | undefined} objectName Name of the global object.
 * @param {string | RegExp} methodName Name of the method.
 * @param {number} index The given position.
 * @returns `true` if the node is argument at the given position.
 * @private
 */
 ...
 * @template T
 * @param {(value: T) => boolean} f The function to negate.
 * @returns {(value: T) => boolean} Negated function.
 */
function negate(f) {
 ...
 *
 * `SourceCode`'s token getters return `null` when the search runs past the end
 * of the file. At the call sites below the grammar guarantees a token is
 * there — a function has a `(`, a `SwitchCase` has a `:`, an arrow function
 * has a `=>` — so the lookup is reinterpreted rather than re-checked.
 ...
 * @param {ASTNode} node A node to check.
 * @returns `true` if the node is an export declaration.
 * @private
 */
 ...
 * @returns The Block comment token containing the JSDoc comment for the given node or null if not found.
 * @private
 */
...
function getOpeningParenOfParams(node, sourceCode) {
	...
	if (node.type === "ArrowFunctionExpression" && node.params.length === 1) {
		const argToken = asToken(sourceCode.getFirstToken(node.params[0]));
		const maybeParenToken = asToken(sourceCode.getTokenBefore(argToken));
	...
	return node.id
		? asToken(sourceCode.getTokenAfter(node.id, isOpeningParenToken))
		: asToken(sourceCode.getFirstToken(node, isOpeningParenToken));
}
...
function getSwitchCaseColonToken(node, sourceCode) {
	if (node.test) {
		return asToken(sourceCode.getTokenAfter(node.test, isColonToken));
	}
	return asToken(sourceCode.getFirstToken(node, 1));
}
...
function isReferenceToGlobalVariable(scope, node) {
	const reference = scope.references.find(
		ref => /** @type {unknown} */ (ref.identifier) === node,
	);
		...
		case "TemplateLiteral":
			return (
				(inBooleanPosition &&
					node.quasis.some(
						/**
						 * Checks whether a template element contributes text.
						 * @param {ASTNode} quasi The template element.
						 * @returns {number} The length of the element's text.
						 */
						quasi => quasi.value.cooked.length,
					)) ||
				node.expressions.every(
					/**
					 * Checks whether an interpolated expression is constant.
					 * @param {ASTNode} exp The interpolated expression.
					 * @returns {boolean} `true` if the expression is constant.
					 */
					exp => isConstant(scope, exp, false),
				)
			);
		...
		case "ArrayExpression": {
			if (!inBooleanPosition) {
				return node.elements.every(
					/**
					 * Checks whether an array element is constant.
					 * @param {ASTNode} element The array element.
					 * @returns {boolean} `true` if the element is constant.
					 */
...
/**
 * Tokenizes source text, keeping the comments espree collects alongside the
 * tokens.
 *
 * espree's declarations describe the option-independent result, so they say
 * nothing about `comments` or about each token carrying a `range`. Both are
 * present here because the caller passes `comment: true` and `range: true`.
 * @param {string} text The source text to tokenize.
 * @param {Object} options The espree options to tokenize with.
 * @returns {Array<Token> & { comments: Array<Token> }} The tokens, with the comments attached.
 */
function tokenizeWithComments(text, options) {
	return /** @type {Array<Token> & { comments: Array<Token> }} */ (
		/** @type {unknown} */ (espree.tokenize(text, options))
	);
}
 ...
 * @param {Token} token The token to check.
 * @returns Whether the token can cause continuation of a preceding expression.
 */
 ...
 *
 * Assigned from the block below, which keeps its lookup tables out of the
 * module scope. The signature lives here because a bare `let` gives tsc
 * nothing to infer the parameters from.
...

{
	 ...
	 * @param {ASTNode} node The node to check.
	 * @returns Whether the node is inside a TypeScript type context.
	 */
	...
	/**
	 * Determines whether a semicolon is required before the given node.
	...
	needsPrecedingSemicolon = function (sourceCode, node) {
		...
		/*
		 * `prevToken` is a token of the program being linted, so the node
		 * containing it always exists. The code below has always dereferenced
		 * `prevNode` unconditionally; the cast records that rather than
		 * introducing a branch that cannot be reached.
		 */
		const prevNode = /** @type {ASTNode} */ (
			/** @type {unknown} */ (
		...
		if (IDENTIFIER_OR_KEYWORD.has(prevToken.type)) {
			...
			// `undefined` for any keyword the map does not list.
			/** @type {string | undefined} */
			const nodeType =
				NODE_TYPES_BY_KEYWORD[
					/** @type {Exclude<keyof typeof NODE_TYPES_BY_KEYWORD, "__proto__">} */ (
						keyword
					)
				];
 ...
 * @param {ASTNode} node The node to check.
 * @returns Whether the node is used as an import attribute key.
 */
...
module.exports = {
	 ...
	 * @param {Array<Reference>} references An array of references.
	 ...
	 * @param {ASTNode} node The comment token to be checked
	 ...
	 *
	 * Re-typed on the way out because `esutils` ships no declarations; see the
	 * note on its `require` above.
	 * @type {(node: ASTNode) => ASTNode | null}
	 */
	 ...
	 * @param {SourceCode} sourceCode A SourceCode instance to get comments.
	 * @param [capIsConstructor = true] `false` disables the assumption that functions which name starts
	...
	isDefaultThisBinding(node, sourceCode, { capIsConstructor = true } = {}) {
		...
		if (
			...
			node.params.some(
				/**
				 * Checks whether a parameter is named `this`.
				 * @param {ASTNode} param A parameter of the function.
				 * @returns {boolean} `true` if the parameter is named `this`.
				 */
	 ...
	 * @param {ASTNode} node The node to check.
	 * @returns {Array<ASTNode>} The directives found in the directive prologue.
	 */
	...
	getParenthesisedText(sourceCode, node) {
		let leftToken = asToken(sourceCode.getFirstToken(node));
		let rightToken = asToken(sourceCode.getLastToken(node));

		/*
		 * Each surrounding token is looked up once and held, rather than
		 * re-fetched for every clause: a lookup that has already been checked
		 * for `null` is the only thing the clauses below need.
		 */
		for (;;) {
			const nextLeft = sourceCode.getTokenBefore(leftToken);
			const nextRight = sourceCode.getTokenAfter(rightToken);

			if (
				!nextLeft ||
				nextLeft.type !== "Punctuator" ||
				nextLeft.value !== "(" ||
				!nextRight ||
				nextRight.type !== "Punctuator" ||
				nextRight.value !== ")"
			) {
				break;
			}
	...
	canTokensBeAdjacent(leftValue, rightValue) {
		...
		/*
		 * `leftToken` and `rightToken` are dereferenced unconditionally from
		 * the hashbang check onwards, which only holds because callers pass
		 * text that tokenizes to at least one token or comment. Declaring them
		 * as `Token` records that assumption in one place instead of adding a
		 * branch for a case this function has never handled.
		 */
		/** @type {Token} */
		let leftToken;
		...
		if (typeof leftValue === "string") {
			...
			try {
				tokens = tokenizeWithComments(leftValue, espreeOptions);
			} catch {
				return false;
			}
			...
			leftToken = /** @type {Token} */ (tokens.at(-1));
			if (comments.length) {
				// Non-empty by the check above.
				const lastComment = /** @type {Token} */ (comments.at(-1));
		...
		if (typeof rightValue === "string") {
			...
			try {
				tokens = tokenizeWithComments(rightValue, espreeOptions);
			} catch {
				return false;
			}
	 ...
	 * @returns `true` if the braces are necessary - removing them (replacing the given `BlockStatement` body with its single statement content)
	...
	areBracesNecessary(node, sourceCode) {
		 ...
		 * @returns True if the node is a lexical declaration
		 * @private
		 */
		 ...
		 * @param {Token} token The token to check.
		 * @returns `true` if the token is an `else` token.
		 */
		 ...
		 * @param {ASTNode} nodeToCheck The node to check.
		 * @returns `true` if the node is followed by an `else` keyword token.
		 */
		function isFollowedByElseKeyword(nodeToCheck) {
			const nextToken = sourceCode.getTokenAfter(nodeToCheck);

			return nextToken !== null && isElseKeywordToken(nextToken);
		}
		 ...
		 * @returns `true` if an `if` statement within the code would become associated with an `else` appended to that code.
		 */
