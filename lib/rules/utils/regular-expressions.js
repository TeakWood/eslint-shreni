...
function isValidWithUnicodeFlag(ecmaVersion, pattern, flag = "u") {
	...
	const validator = new RegExpValidator({
		 ...
		 * Clamping to `REGEXPP_LATEST_ECMA_VERSION` is what keeps the value
		 * inside that union, but tsc only sees `number` coming out of `min`.
		 */
		ecmaVersion:
			/** @type {import("@eslint-community/regexpp/ecma-versions").EcmaVersion} */ (
