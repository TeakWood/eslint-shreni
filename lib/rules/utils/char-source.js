...
class CodeUnit {
	 ...
	 * @param {number} start The offset in the source text where this code unit starts.
	 * @param {string} source The source text that produced this code unit.
	 */
	...
	/**
	 * The number of source characters that produced this code unit.
	 * @type {number}
	 */
	get length() {
...
class TextReader {
	/**
	 * Creates a reader positioned at the start of the given text.
	 * @param {string} source The source text to read.
	 */
	constructor(source) {
		this.source = source;
		this.pos = 0;
...
function readUnicodeSequence(reader) {
	...
	if (match) {
		// The pattern above always defines the `hexDigits` group.
...
function readOctalSequence(reader, maxLength) {
	/*
	 * The caller only reaches here after reading an octal digit, which the
	 * reader is positioned just after, so the match always succeeds.
	 */
...
function readEscapeSequenceOrLineContinuation(reader) {
	...
	reader.advance(2);
	// `undefined` for any character the table does not list.
	/** @type {string | undefined} */
	const unitChar =
		SIMPLE_ESCAPE_SEQUENCES[
			/** @type {Exclude<keyof typeof SIMPLE_ESCAPE_SEQUENCES, "__proto__">} */ (
				char
			)
		];
 ...
 * @returns {Array<CodeUnit>} A list of code units produced by the string literal.
 */
 ...
 * @returns {Array<CodeUnit>} A list of code units produced by the template token.
 */
