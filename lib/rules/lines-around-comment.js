...
/**
 * Which sides of a comment this rule requires a blank line on.
 * @typedef {Object} EmptyLineOptions
 * @property {boolean} after Whether an empty line is required after the comment.
 * @property {boolean} before Whether an empty line is required before the comment.
 */
 ...
 * @param {Array<string>} lines An array of each line of the file.
 * @returns {Array<number>} An array of line numbers.
 */
 ...
 * @param {Array<Comment>} comments An array of comment tokens.
 * @returns {Array<number>} An array of line numbers.
 */
...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {Comment} token The comment token to check.
		 * @returns {boolean} True if the comment is not alone.
		 */
		function codeAroundComment(token) {
			// The walks below run off the ends of the file, so the cursor has to admit `null`.
			/** @type {Token | null} */
		 ...
		 * @param {Comment} token The token to check.
		...
		function getParentNodeOfToken(token) {
			 ...
			 */
			/*
			 * A `StaticBlock` node always has a `static` keyword followed by an
			 * opening brace, so the token lookups below cannot come up empty.
			 */
		 ...
		 * @param {Comment} token The Comment token.
		 * @param {string} nodeType The parent type to check against.
		...
		function isCommentAtParentStart(token, nodeType) {
			...
			if (parent && isParentNodeType(parent, nodeType)) {
				...
				/*
				 * The grammar guarantees the brace each lookup below asks for:
				 * a static block is `static {`, and a switch is `switch (x) {`.
				 */
				if (parent.type === "StaticBlock") {
					parentStartNodeOrToken = /** @type {Token} */ (
					...
					parentStartNodeOrToken = /** @type {Token} */ (
						sourceCode.getTokenAfter(parent.discriminant, {
		 ...
		 * @param {Comment} token The Comment token.
		 * @param {EmptyLineOptions} opts Options to determine the newline.
		...
		function checkForEmptyLine(token, opts) {
			...
			/*
			 * These two are only read under `before` / `after`, and the
			 * `prevLineNum` / `nextLineNum` guards above already cleared those
			 * flags at the very top and the very bottom of the file, so neither
			 * lookup can be `null` where it matters.
			 */
			const previousTokenOrComment = /** @type {Token} */ (
				sourceCode.getTokenBefore(token, {
					includeComments: true,
				})
			);
			const nextTokenOrComment = /** @type {Token} */ (
			...
			if (
				...
				const range = /** @type {Range} */ ([lineStart, lineStart]);
				...
				context.report({
					...
					/**
					 * Inserts a newline before the comment's line.
					 * @param {RuleFixer} fixer The fixer to apply the fix with.
			...
			if (
				...
				context.report({
					...
					/**
					 * Inserts a newline after the comment.
					 * @param {RuleFixer} fixer The fixer to apply the fix with.
					 * @returns {EditInfo} The fix.
					 */
					fix(fixer) {
		...
		return {
			/**
			 * Checks every comment in the file once the program is visited.
			 * @returns {void} No return value.
			 */
			Program() {
				comments.forEach(token => {
