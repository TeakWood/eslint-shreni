...
/**
 * What this rule tracks for one reachable code path segment. The segments of a
 * code path are visited in order, so a segment's information is built from the
 * information of the segments that precede it.
 * @typedef {Object} SegmentInfo
 * @property {Array<ASTNode>} uselessReturns The `return` statements that are still candidates for a report on this segment.
 * @property {boolean} returned Whether a `return` statement terminates this segment.
 */
 ...
 * @property {ScopeInfo} upper The information for the enclosing code path.
 * @property {Array<ASTNode>} uselessReturns The `return` statements of this code path that are still candidates for a report.
 * @property {Array<ASTNode>} traversedTryBlockStatements The `try` blocks currently being traversed, innermost last.
 ...
 * @param {Array<ASTNode>} array The source array to remove.
 * @param {ASTNode} element The target item to remove.
 * @returns {void}
 */
...
module.exports = {
	...
	create(context) {
		 ...
		 * @param {Array<ASTNode>} uselessReturns The collected return statements.
		 * @param {Array<CodePathSegment>} prevSegments The previous segments to traverse.
		 * @param {WeakSet<CodePathSegment>} [providedTraversedSegments] A set of segments that have already been traversed in this call
		 * @returns {Array<ASTNode>} `uselessReturns`.
		 */
		function getUselessReturns(
			...
			for (const segment of prevSegments) {
				...
				if (segmentInfoMap.has(segment)) {
					// `has()` above guarantees the `get()` below finds the info.
					const info = /** @type {SegmentInfo} */ (
						segmentInfoMap.get(segment)
					);

					uselessReturns.push(...info.uselessReturns);
				}
			}
		...
		return {
			...
			/**
			 * Tracks the `try` block that is being left.
			 * @param {ASTNode} node The `try` block's `BlockStatement` node.
			 * @returns {void}
			 */
			...
			/**
			 * Stops tracking the `try` statement that is being left.
			 * @returns {void}
			 */
			"TryStatement:exit"() {
