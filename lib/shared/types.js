...
/*
 * The three shapes below are deliberately looser than their ESTree
 * counterparts, for the same reason the rules layer keeps its own node
 * vocabulary in `lib/rules/utils/ast-utils.js`: the tree ESLint actually walks
 * is not the bare ESTree union. Third-party parsers contribute node and token
 * types ESTree does not describe, the linter links every node to its `parent`,
 * and `SourceCode` rewrites the first comment's `type` to `"Shebang"`. Pinning
 * `type` to a closed union here would force a cast at nearly every read in the
 * languages layer without making a single one of those reads safer.
 *
 * What is pinned is the part that holds for everything the token store and
 * `SourceCode` are handed: a `range`, and for tokens a `type` and a `value`.
 */

/**
 * A token produced by the parser.
 * @typedef {{ type: string, value: string, range: Range, loc: SourceLocation, [key: string]: any }} Token
 */

/**
 * A comment token. `type` is `"Line"` or `"Block"` as the parser produces it,
 * but `SourceCode` rewrites a leading hashbang comment's type to `"Shebang"`.
 * @typedef {{ type: string, value: string, range: Range, loc: SourceLocation, [key: string]: any }} Comment
 */

/**
 * Anything the token store can locate in the source text: an AST node, a
 * token, or a comment. Only `range` is read, so only `range` is required.
 * @typedef {{ range: Range, [key: string]: any }} NodeOrToken
 */
...
/**
 * The timing information collected when the `stats` option is enabled. One
 * `passes` entry is recorded per autofix pass, each holding the parse, fix and
 * per-rule lint times for that pass.
 * @typedef {Object} LintStats
 * @property {{ passes: Array<Record<string, any>> }} [times] The times spent parsing, fixing and linting the file.
...
/**
 * The second argument handed to a formatter, carrying everything about the run
 * that is not itself a lint result. Every property is optional because the
 * argument is assembled from the CLI options that were actually supplied, and a
 * formatter invoked through the API may be given nothing at all.
 * @typedef {Object} FormatterData
 * @property {string} [cwd] The current working directory.
 * @property {Record<string, RuleMeta | undefined>} [rulesMeta] The metadata of each rule that was executed, keyed by rule ID.
 * @property {boolean} [color] Whether to colorize the output; when absent the formatter decides from the output stream.
 * @property {{ maxWarnings: number, foundWarnings: number }} [maxWarningsExceeded] The warning counts, present only when `--max-warnings` was exceeded.
 */
