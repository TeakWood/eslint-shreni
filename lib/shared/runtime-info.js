...
function environment() {
	...
	function execCommand(cmd, args) {
		...
		if (cache.has(key)) {
			// `has` has already established that the entry is present.
	 ...
	 * @param {boolean} [options.global] Whether to check globally or not.
	 * @throws As may be collected by `cross-spawn.sync`.
	 * @returns {string} The normalized version returned by the command.
	 */
	function getNpmPackageVersion(pkg, { global = false } = {}) {
		...
		try {
			/**
			 * Only the one branch of `npm ls --json` that this function reads
			 * is described. A global lookup returns `{}`, which is why every
			 * member is optional.
			 * @type {{ dependencies?: { eslint?: { version: string } } }}
			 */
