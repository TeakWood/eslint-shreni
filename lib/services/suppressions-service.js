...
/**
 * How many violations of one rule are suppressed in one file.
 * @typedef {Object} SuppressionCount
 * @property {number} count The number of suppressed violations.
 */

/**
 * The contents of a suppressions file: rule counts, keyed first by the file's
 * path relative to the cwd and then by rule ID.
 * @typedef {Record<string, Record<string, SuppressionCount>>} Suppressions
 */
...
class SuppressionsService {
	 ...
	 * @param {Array<string>} [rules] The rules to suppress.
	 * @returns {Promise<void>} A promise that fulfills once the file is written.
	 */
	 ...
	 * @param {Array<LintResult>} results The lint results.
	 * @param {Suppressions} suppressions The suppressions.
	 * @returns {{ results: Array<LintResult>, unused: Suppressions }} The updated
	 *      results and the unused suppressions.
	 */
		...
		for (const result of filtered) {
			...
			for (const ruleId in violationsByRule) {
				...
				if (violationsCount < suppressionsCount) {
					...
					// The count the cast promises is assigned on the next line.
	 ...
	 * @returns {Promise<Suppressions>} The suppressions.
	 */
	async load() {
		try {
	 ...
	 * @param {Suppressions} suppressions The suppressions to save.
	 * @returns {Promise<void>} A promise that fulfills once the file is written.
	 * @private
	 */
	save(suppressions) {
	 ...
	 * @param {Array<LintMessage>} messages The messages to count.
	 * @returns {Record<string, SuppressionCount>} The number of violations by rule.
	 */
	static countViolationsByRule(messages) {
		return messages.reduce((totals, message) => {
			...
			return totals;
		}, /** @type {Record<string, SuppressionCount>} */ ({}));
	}
	...
	static suppressMessagesByRule(result, ruleId) {
		...
		result.suppressedMessages = result.suppressedMessages.concat(
			suppressedMessages.map(message => {
				/*
				 * The message becomes a suppressed one by being given the
				 * `suppressions` the cast promises, on the very next line.
				 */
				const suppressed =
