...
module.exports = class ForwardTokenCommentCursor extends Cursor {
	...
	moveNext() {
		...
		/*
		 * ESCAPE HATCH: TypeScript does not narrow through a `Boolean()` call,
		 * so `current` is still `Token | Comment | null` in the second operand.
		 * The `Boolean(this.current)` guard on the left of the `&&` is what
		 * makes the assertion safe — it is load-bearing, not decorative.
		 */
		return (
