...
/**
 * The map from a character offset in the source text to an index in `tokens`.
 *
 * It is built with `Object.create(null)`, and its keys are offsets stringified
 * by property access, so the numeric key type describes how it is read rather
 * than how it is stored.
 * @typedef {Record<number, number>} IndexMap
 */
 ...
 * @param {Array<Token | Comment>} tokens It searches the token in this list.
 ...
 * @param {Array<Token | Comment>} tokens The tokens to find an index.
