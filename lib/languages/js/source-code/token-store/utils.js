...
/**
 * The map from source locations to indices in the token array.
 *
 * `token-store/index.js` builds this with `Object.create(null)`, so it has no
 * prototype and is only ever reached through the bracket form. A miss is a
 * genuine possibility at every read — the callers below guard with `in` first —
 * which is why this is an index signature rather than a `Map`.
 *
 * Declared here because this module is the only one that reads the map;
 * `index.js` builds it and the four token cursors merely forward it.
 * @typedef {{ [location: number]: number }} IndexMap
 */
 ...
 * @param {readonly (Token | Comment)[]} tokens It searches the token in this list.
 ...
 * @param {readonly Token[]} tokens The tokens to find an index.
 * @param {IndexMap} indexMap The map from locations to indices.
