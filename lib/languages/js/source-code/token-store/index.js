...
/**
 * The two factory methods this module reaches on `cursors.forward` and
 * `cursors.backward`.
 *
 * `cursors.js` is deliberately left un-annotated by this change: bead y6r.11
 * owns it, because `createCursor` chooses its base class from an instance field
 * at runtime and then conditionally wraps it, so its honest return type is a
 * six-way union. Un-annotated, both methods infer as returning `any`, which
 * would spread `any` through every getter below. Naming the contract this
 * module relies on, and typing the `factory` parameters against it, stops that
 * `any` at the boundary — one JSDoc cast (in `getTokenByRangeStart`, the only
 * place a factory is reached directly rather than passed as an argument).
 *
 * RETIREMENT: once y6r.11 annotates `cursors.js`, delete this typedef and the
 * cast, and take `typeof cursors.forward` directly.
 * @typedef {object} CursorFactory
 * @property {(tokens: Token[], comments: Comment[], indexMap: IndexMap, startLoc: number, endLoc: number, includeComments?: boolean) => Cursor} createBaseCursor Creates an undecorated cursor.
 * @property {(tokens: Token[], comments: Comment[], indexMap: IndexMap, startLoc: number, endLoc: number, includeComments: boolean, filter: TokenFilter | null, skip: number, count: number) => Cursor} createCursor Creates a cursor with the filter/skip/limit decorators applied.
 */
...
module.exports = class TokenStore {
	...
	getTokenByRangeStart(offset, options) {
		...
		/*
		 * ESCAPE HATCH: this is the one place a factory is reached directly
		 * rather than passed to a helper whose parameter is already typed. See
		 * the `CursorFactory` typedef for why the un-annotated `cursors.js`
		 * makes the cast necessary, and when it can be deleted.
		 */
		const token = /** @type {CursorFactory} */ (cursors.forward)
			.createBaseCursor(
				this[TOKENS],
				this[COMMENTS],
				this[INDEX_MAP],
