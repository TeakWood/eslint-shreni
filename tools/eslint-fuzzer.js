 ...
 * @param options Config options for fuzzing
 * @param options.count The number of fuzz iterations.
 * @param options.linter The linter object to test with.
 * @param [options.codeGenerator=eslump.generateRandomJS] A function to use to generate random
 ...
 * @param [options.checkAutofixes=true] `true` if the fuzzer should check for autofix bugs. The fuzzer runs
 * roughly 4 times slower with autofix checking enabled.
 * @param [options.progressCallback] A function that gets called once for each code sample, with the total number of errors found so far
 * @returns A list of problems found. Each problem has the following properties:
...
function fuzz(options) {
	 ...
	 * @param config A config object that causes a crash or autofix error
	 * @param problemType The type of problem that occurred
	 * @returns A config object with only one rule enabled that produces the same crash or autofix error, if possible.
	 ...
	 * @param originalText Syntactically valid source code that results in a syntax error or crash when autofixing with `config`
	 * @param config The config to lint with
	 * @returns A possibly-modified version of originalText that results in the same syntax error or crash after only one pass
	 */
